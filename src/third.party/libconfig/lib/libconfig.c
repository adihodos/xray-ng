...
int config_setting_set_string_with_len(config_setting_t* setting, const char* value, const size_t len) {
  config_assert(setting != NULL);
  config_assert(value != NULL);
  ...
  if (value) {
	  setting->value.sval = (char*)malloc(len + 1);
	  if (!setting->value.sval) {
		  return(CONFIG_FALSE);
	  }
	  memcpy(setting->value.sval, value, len);
	  setting->value.sval[len] = 0;
  }
...
int config_setting_set_string(config_setting_t* setting, const char* value) {
	return config_setting_set_string_with_len(setting, value, strlen(value));
}
...
/* ------------------------------------------------------------------------- */
config_setting_t* config_setting_set_string_elem_with_len(
	config_setting_t* setting, int idx, const char* value, const size_t len
) {
  config_setting_t *element = NULL;
  ...
  if(! config_setting_set_string_with_len(element, value, len))
    return(NULL);
...
config_setting_t* config_setting_set_string_elem(config_setting_t* setting, int idx, const char* value) {
	return config_setting_set_string_elem_with_len(setting, idx, value, strlen(value));
}
