...
int main(int, char**) {
  ...
  struct key_map_info {
    ...
    const uint32_t mask;
  } static constexpr mappings[] = {
      {"symtables/x11.keymap.0", "X11_MISC_FUNCTION_KEYS_MAPPING_TABLE", 0xFF},
      {"symtables/x11.keymap.1", "X11_LATIN1_KEYS_MAPPING_TABLE", 0xFF00}};

  vector<string> gen_code;

  auto keymapfn = [](const key_map_info& kmi) {
    unique_pointer<FILE, decltype(&fclose)> fp{fopen(kmi.file_name, "r"),
                                               &fclose};
  ...
  transform(begin(mappings), end(mappings), back_inserter(gen_code),
            [](const auto& mp) {
