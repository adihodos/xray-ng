...
main(int, char**)
{
    ...
    platformstl::readdir_sequence rss{ "enumdefs",
                                       platformstl::readdir_sequence::files | platformstl::readdir_sequence::fullPath };
    ...
    for (auto is = begin(rss), ie = end(rss); is != ie; ++is) {
        ...
        if (!deffile) {
            printf("\nFailed to open file %s, error %s", *is, deffile.error().c_str());
            continue;
        }
        ...
        auto fn_gen_namespaces = [&deffile](string& ns_begin, string& ns_end) {
            ...
            for (uint32_t i = 0, entries_count = ns_entry.length(); i < entries_count; ++i) {
                namespaces.push_back(ns_entry[i].as_string());
            }

            ns_begin =
                accumulate(begin(namespaces), end(namespaces), std::string(""), [](const string& s, const string& ns) {
                    return s + "namespace " + ns + " { ";
                });

            ns_end = accumulate(rbegin(namespaces),
                                rend(namespaces),
                                std::string(""),
                                [](const string& s, const string& ns) { return s + "} // namespace " + ns + "\n"; });
        };

        {
            ...
            outtext = replace_all_occurences(
                "{enum_underlying_type_spec}", underlying_type ? fmt::format(": {}", underlying_type) : "", outtext);

            outtext = replace_all_occurences("{enum_underlying_type}",
                                             underlying_type ? underlying_type
                                                             : fmt::format("std::underlying_type<{}::e>::type", name),
                                             outtext);

            outtext =
                replace_all_occurences("{type_traits_hdr}", underlying_type ? "" : "#include <type_traits>", outtext);
...

            {
                ...
                outtext = str_ext::replace_all("{enum_members_and_values}", mws.c_str(), outtext);

                outtext = str_ext::replace_all("{enum_length}", fmt::format("{}", mem_sec.length()).c_str(), outtext);
            }

            {
                for_each(begin(members_with_values), end(members_with_values), [](std::string& s) {
                    str_ext::trim(s);
                ...
                const auto mem_lst = std::accumulate(
                    std::next(begin(members_with_values), 1),
                    end(members_with_values),
                    fmt::format("e::{}", *begin(members_with_values)),
                    [](const std::string& a, const std::string& b) { return a + fmt::format(", e::{}", b); });

                outtext = str_ext::replace_all("{enum_members}", mem_lst.c_str(), outtext);
            }

            {
                ...
                if (bitwise) {
                    constexpr auto bitwise_or = "constexpr {0}::e operator|(const {0}::e a, const {0}::e b) "
                                                "noexcept {{"
                    ...
                    constexpr auto bitwise_and = "constexpr {0}::e operator&(const {0}::e a, const {0}::e b) "
                                                 "noexcept {{"
                                                 "  return static_cast<{0}::e>("
                                                 "      static_cast<{0}::underlying_type>(a) &"
                                                 "      static_cast<{0}::underlying_type>(b));"
                    ...
                    outtext = str_ext::replace_all("{bitwise_and}", fmt::format(bitwise_and, name).c_str(), outtext);

                    outtext = str_ext::replace_all("{bitwise_or}", fmt::format(bitwise_or, name).c_str(), outtext);
                } else {
            ...
            outtext = str_ext::replace_all("{enum_file_hpp}", fmt::format("{}.hpp", name), outtext);
            outtext = str_ext::replace_all("{enum_name}", name, outtext);

            const auto qualified_names_code = accumulate(
                begin(members_with_values),
                end(members_with_values),
                std::string{},
                [name](const std::string& a, const std::string& b) {
                    return a + fmt::format("case {1}::e::{0} :\n return \"{1}::e::{0}\";\nbreak;\n", b, name);
                });

            outtext = str_ext::replace_all("{qualified_name_code}", qualified_names_code, outtext);

            const auto names_code =
                accumulate(begin(members_with_values),
                           end(members_with_values),
                           std::string{},
                           [name](const std::string& a, const std::string& b) {
                               return a + fmt::format("case {1}::e::{0} :\n return \"{0}\"; break;\n", b, name);
            ...
            outtext = str_ext::replace_all("{unqualified_name_code}", names_code, outtext);
