...
inline bool
operator==(const tinyobj_opt::index_t& a, const tinyobj_opt::index_t& b)
{
                     ...
                     std::vector<uint32_t>* indices)
{
    ...
    const auto res = parseObj(&attrs, &shapes, &mtls, (const char*)mmfile.memory(), mmfile.size(), {});
    ...
    if (attrs.normals.empty()) {
        ...
        for (size_t i = 0, cnt = indices->size() / 3; i < cnt; ++i) {
            ...
            const auto n = cross(v1.position - v0.position, v2.position - v0.position);
        ...
        for_each(begin(*vertices), end(*vertices), [](vertex_pnt& v) { v.normal = normalize(v.normal); });
    }
...
main(int argc, char** argv)
{
    ...
    readdir_sequence dir_contents_iter{ argv[1], readdir_sequence::files | readdir_sequence::fullPath };

    for_each(begin(dir_contents_iter), end(dir_contents_iter), [&vertices, &indices](const char* file) {
        ...
        mbounds.axis_aligned_bbox =
            bounding_box3_axis_aligned((const vec3f*)vertices.data(), vertices.size(), sizeof(vertex_pnt));

        mbounds.bounding_sphere =
            bounding_sphere<float>(vertices.begin(), vertices.end(), [](const vertex_pnt& v) { return v.position; });
        ...
        mhdr.index_offset = mhdr.vertex_offset + mhdr.vertex_count * mhdr.vertex_size;
