...
tl::expected<TerrainSlabTextures, VulkanError>
create_terrain_slab_render_resources(const TerrainParams terrain_params,
                                     ...
                                     std::span<vec4ui8> colormap)
{
    ...
    auto heightmap_texture = VulkanImage::from_memory(*renderer,
    ...
    auto colormap_texture = VulkanImage::from_memory(*renderer,
                                                     VulkanImageCreateInfo{
                                                         .tag_name = scratch_buffer,
                                                         .wpkg = terrain_images_job->buffer,
                                                         .type = VK_IMAGE_TYPE_2D,
...
concurrencpp::result<tl::expected<TerrainGenTaskResult, VulkanError>>
task_generate_terrain_chunk(concurrencpp::executor_tag,
...
B5::Terrain::create(const InitContext& ctx)
{
    ...
    return tl::expected<Terrain, VulkanError>{
        tl::in_place, PrivateConstructionToken{}, std::move(*vertex_buffer), std::move(*index_buffer),
        instances,    std::move(lod_lvls),        std::move(terrain_chunks), std::move(noise_gen),
        params,
    };
}

void
copy_render_resources(xray::rendering::VulkanRenderer* renderer,
                      ...
                      const TerrainParams& params)
{
    ...
    for (auto&& [copy_data, buffer_copy, mem_barriers, texture] :
        ...
        vkCmdCopyBufferToImage(queued_job.buffer,
...
B5::Terrain::loop_event(const RenderEvent& re)
{
    ...
    _renderstate.slabs_visible_last_frame.insert(std::cbegin(slabs_visible_current_frame),
                                                 std::cend(slabs_visible_current_frame));
    ...
    vkfn::CmdSetPolygonModeEXT(re.frame_data->cmd_buf,
                               _uistate.draw_opts[DrawOptions::TerrainWireframeBit] ? VK_POLYGON_MODE_LINE
    ...
    UniqueMemoryMapping::map_memory(re.renderer->device(),
        ...
        .map([this,
              &re,
              visible_range = lz::chain(slabs_visible_current_frame)
                                  .take(std::min<uint32_t>(slabs_visible_current_frame.size(), MAX_VISIBLE_SLABS))](
...
B5::Terrain::user_interface(xray::ui::user_interface* ui, const RenderEvent& re)
{
    if (ImGui::CollapsingHeader("Terrain")) {
        ...
        for (auto&& [slab_center, dontcare] : _renderstate.slabs_table) {
            ...
            draw_list->AddRectFilled(ImVec2{ c.x - cell_size * 0.5f, c.y - cell_size * 0.5f },
        ...
        draw_list->AddTriangleFilled(
            { v0.x, v0.y }, { v1.x, v1.y }, { dir_endpt.x, dir_endpt.y }, IM_COL32(0, 32, 255, 255));
