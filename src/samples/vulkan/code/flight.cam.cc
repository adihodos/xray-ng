...
{
    // rfl::libconfig::save("cam.urmom.conf", FlightCameraParams{});
...
FlightCamera::update(const JPH::Body& target) noexcept
{
    ...
    const JPH::Mat44 cam_rot =
        JPH::Mat44::sRotation(ship_orientation * rotation_from_input * rotation_from_angular_vel);

    const JPH::Vec3 cam_pos =
    ...
    MatrixWithInvertedMatrixPair4f view_matrix;
    world_to_camera.Transposed().StoreFloat4x4(reinterpret_cast<JPH::Float4*>(view_matrix.transform.components));
    (JPH::Mat44::sTranslation(cam_pos) * cam_rot)
        .Transposed()
        .StoreFloat4x4(reinterpret_cast<JPH::Float4*>(view_matrix.inverted.components));
