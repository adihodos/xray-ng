...
public:
  ...
  gpu_buffer(renderer* r_ptr, const uint32_t bind_flags, const T (&arr_ref)[N],
...
template <typename T, size_t N>
gpu_buffer::gpu_buffer(renderer* r_ptr, const uint32_t   bind_flags,
                       const T (&arr_ref)[N], const bool gpu_write,
                       const bool cpu_acc)
    : gpu_buffer{r_ptr,   sizeof(arr_ref[0]), N,      bind_flags,
                 arr_ref, gpu_write,          cpu_acc} {}

inline gpu_buffer::handle_type raw_handle(const gpu_buffer& buff) noexcept {
  return buff.handle();
}
