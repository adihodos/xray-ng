...
struct shader_traits<shader_type::vertex>
{
    ...
    static scoped_shader_type create(ID3D10Device* device, ID3D10Blob* blob)
    {
        ...
        device->CreateVertexShader(blob->GetBufferPointer(), blob->GetBufferSize(), raw_ptr_ptr(new_shader));

        return new_shader;
    }

    static void bind(ID3D10Device* device, ID3D10VertexShader* shader) noexcept { device->VSSetShader(shader); }
