...
bool
shader_collect_uniforms(ID3D10Blob* compiled_bytecode,
...
scoped_blob_handle
compile_shader_from_file(const shader_type type, const char* file_name, const uint32_t compile_flags = 0);

scoped_blob_handle
compile_shader(const shader_type type, const char* src_code, const size_t code_len, const uint32_t compile_flags = 0);
...
class gpu_shader_common_core
{
  ...
  public:
    ...
    gpu_shader_common_core(const shader_type stype,
                           ID3D10Device* dev,
