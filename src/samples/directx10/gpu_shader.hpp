...
public:
  ...
  gpu_shader(renderer* r_ptr, const char* file_name,
             const uint32_t compile_flags = 0);

  gpu_shader(renderer* r_ptr, const char* src_code, const size_t code_len,
...
xray::rendering::directx10::gpu_shader<stype>::gpu_shader(
    renderer* r_ptr, const char* file_name, const uint32_t compile_flags)
...
template <shader_type stype>
xray::rendering::directx10::gpu_shader<stype>::gpu_shader(
    renderer* r_ptr, const char* src_code, const size_t code_len,
    const uint32_t compile_flags)
...
template <shader_type stype>
template <typename T>
void xray::rendering::directx10::gpu_shader<stype>::set_uniform(
    const char* name, const T& value) noexcept {
  assert(valid());
...
template <shader_type stype>
template <typename T>
void xray::rendering::directx10::gpu_shader<stype>::set_uniform_block(
    const char* name, const T& value) noexcept {
  assert(valid());
...
template <shader_type stype>
void xray::rendering::directx10::gpu_shader<stype>::bind(renderer* r_ptr) {
  assert(valid());
