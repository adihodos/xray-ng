...
static const vertex_pc Triangle_Vertices[] = { { { -0.8f, -0.8f, 0.0f }, { 1.0f, 0.0f, 0.0f, 1.0f } },
...
void
app::simple_object::init(xray::rendering::directx10::renderer* r_ptr)
{
    assert(!valid());
    ...
    {
        vertex_buff_ = gpu_buffer{ r_ptr, D3D10_BIND_VERTEX_BUFFER, Triangle_Vertices };
...

    {
        ...
        cbuffer_ = gpu_buffer{ r_ptr, 32, 1, D3D10_BIND_CONSTANT_BUFFER, nullptr, false, true };
        if (!cbuffer_)
            return;
    }

    {
        constexpr D3D10_INPUT_ELEMENT_DESC VertexPC_Layout[] = { { "POSITION",
                                                                   0,
        ...
        r_ptr->dev_ctx()->CreateInputLayout(VertexPC_Layout,
                                            XR_U32_COUNTOF__(VertexPC_Layout),
                                            vert_shader_.bytecode()->GetBufferPointer(),
                                            vert_shader_.bytecode()->GetBufferSize(),
                                            raw_ptr_ptr(layout_decl_));
...
void
app::simple_object::update(const float /* delta */) noexcept
{
...
void
app::simple_object::draw(const xray::rendering::draw_context_t&)
{
    assert(valid());
    ...
    // update constant buffers
    {
        ...
        const float rotation_matrix_data[] = { rotation_matrix.a00, rotation_matrix.a01, rotation_matrix.a02, 0.0f,

                                               rotation_matrix.a10, rotation_matrix.a11, rotation_matrix.a12, 0.0f };

        vert_shader_.set_uniform("rotation", rotation_matrix_data, sizeof(rotation_matrix_data));
