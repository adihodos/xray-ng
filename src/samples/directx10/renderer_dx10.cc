...
xray::rendering::directx10::renderer::renderer(
    const render_params_t& r_param) noexcept {

  {
    ...
    chain_desc.BufferDesc.ScanlineOrdering =
    ...
    constexpr auto kCreateFlags =
        D3D10_CREATE_DEVICE_SINGLETHREADED | D3D10_CREATE_DEVICE_DEBUG;

    D3D10CreateDeviceAndSwapChain(
        nullptr, kDriverType, nullptr, kCreateFlags, D3D10_SDK_VERSION,
        &chain_desc, raw_ptr_ptr(swapchain_), raw_ptr_ptr(device_));
...

  {
    ...
    device_->CreateRenderTargetView(raw_ptr(backbuffer_tex), nullptr,
                                    raw_ptr_ptr(rtv_));
    if (!rtv_) {
...

  {
    const auto view_port =
        D3D10_VIEWPORT{0, 0, r_param.width, r_param.height, 0.0f, 1.0f};
...
void xray::rendering::directx10::renderer::clear_backbuffer(
...
void xray::rendering::directx10::renderer::swap_buffers() noexcept {
  assert(valid());
