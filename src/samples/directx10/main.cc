...
{
  public:
    basic_scene(const uint32_t wnd_width, const uint32_t wnd_height, xray::rendering::renderer* r_ptr);

    void window_resized(const int32_t new_height, const int32_t new_width) noexcept;

    void key_event(const int32_t key_code, const int32_t action, const int32_t modifiers) noexcept;
  ...
  private:
    ...
    xray::scene::camera_controller_spherical_coords cam_control_{ &cam_, controller_cfg_file_path };
...
basic_scene::basic_scene(const uint32_t wnd_width, const uint32_t wnd_height, xray::rendering::renderer* r_ptr)
    : renderer_{ r_ptr }
    , obj_{ r_ptr }
{
...
void
basic_scene::window_resized(const int32_t new_height, const int32_t new_width) noexcept
{
    cam_.set_projection(projection::perspective_symmetric(
        static_cast<float>(new_width), static_cast<float>(new_height), radians(70.0f), 0.3f, 100.0f));
}

void
basic_scene::key_event(const int32_t key_code, const int32_t action, const int32_t modifiers) noexcept
{
    cam_control_.key_event(key_code, action, modifiers);
}

void
basic_scene::mouse_moved(const float delta_x, const float delta_y) noexcept
{
    cam_control_.mouse_moved(delta_x, delta_y);
}

void
basic_scene::scroll_event(const float x_off, const float y_off) noexcept
{
    cam_control_.scroll_event(x_off, y_off);
}

void
basic_scene::tick_event(const float delta)
{
    cam_control_.update();
...
void
basic_scene::draw(const xray::ui::window_context& wnd_ctx)
{
...
{
    return enter_exit_action<enter_fn, exit_fn>{ std::forward<enter_fn>(func_enter), std::forward<exit_fn>(func_exit) };
}
...
main(int, char**)
{
    ...
    const auto r_params = render_params_t{ app_wnd.width(), app_wnd.height(), app_wnd.handle(), true };
    ...
    app_wnd.events.mouse_scroll = make_delegate(scene, &app::basic_scene::scroll_event);
    app_wnd.events.mouse_move = make_delegate(scene, &app::basic_scene::mouse_moved);
    app_wnd.events.window_resize = make_delegate(scene, &app::basic_scene::window_resized);
