...
class basic_scene {
public:
  explicit basic_scene(application_window* app_wnd,
...
basic_scene::~basic_scene() { _perf_stats_thread.signal_stop(); }

void basic_scene::ui_logic() {
...
void basic_scene::collect_stats(const float delta_ms) {
  _stats.s_frame_time = delta_ms;
}

void basic_scene::tick_event(const float delta_ms) noexcept {
  _cam_control.update();
...
void basic_scene::resize_event(const uint32_t width,
...
void basic_scene::input_event(const input_event_t& in_event) noexcept {
...
maybe<scalar2u32> get_primary_monitor_resolution() {
...
void test_scene::input_event(const xray::ui::input_event_t& input_evt) {
...
void test_scene::draw_event(const xray::ui::window_draw_event& wde) {
  // if (!_brush) {
  //  LOGBRUSH lb;
  //  lb.lbStyle = BS_SOLID;
...
void test_scene::resize_event(const uint32_t w, const uint32_t h) {
  _renderer->handle_resize(w, h);
}

void test_scene::tick_event(const float) {}

HRESULT test_scene::swap_buffers_event(const bool) { return S_OK; }

void test_scene::toggle_fullscreen_event() {}
