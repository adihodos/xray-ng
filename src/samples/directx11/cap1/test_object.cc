...
ripple_grid(xray::rendering::geometry_data_t* mesh, const ripple_params& rp) noexcept
{
    ...
    for (uint32_t i = 0, idx_count = static_cast<uint32_t>(indices.length() / 3); i < idx_count; i += 3) {
...
app::simple_object::simple_object(ID3D11Device* device)
{
    init(device);
}
...
void
app::simple_object::draw(const xray::rendering::draw_context_t& dc)
{
    assert(valid());
...

    {
        ...
        renderer->context()->IASetVertexBuffers(0, 1, &vertex_buff, &stride, &offset);
        renderer->context()->IASetIndexBuffer(raw_ptr(_index_buffer), DXGI_FORMAT_R32_UINT, 0);
...
void
app::simple_object::init(ID3D11Device* device)
{
    _rasterstate = [device]() {
...

    {
        ...
        transform(raw_ptr(mesh.geometry),
                  ...
                  [](const auto& in_vertex) {
                      return vertex_pnt{ in_vertex.position, in_vertex.normal, in_vertex.texcoords };
                  });

        _vertex_buffer = [&vertices, device]() {
            D3D11_BUFFER_DESC buff_desc;
            buff_desc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
            buff_desc.ByteWidth = static_cast<uint32_t>(vertices.size() * sizeof(vertices[0]));
        ...
        _vertex_shader = vertex_shader{ device, "shaders/cap1/vert.hlsl", "main_pnt", shader_compile_flags::debug };
        if (!_vertex_shader)
            return;

        _pixel_shader = pixel_shader{ device, "shaders/cap1/pixel.hlsl", "main_pnt", shader_compile_flags::debug };
        if (!_pixel_shader)
            return;

        _layout = [signature = _vertex_shader.bytecode(), device]() {
            const D3D11_INPUT_ELEMENT_DESC format_desc[] = { { "POSITION",
                                                               0,
            ...
            device->CreateInputLayout(format_desc,
                                      XR_U32_COUNTOF__(format_desc),
    ...
    {
        platformstl::memory_mapped_file tex_file{ app_config::instance()->texture_path("uv_grids/ash_uvgrid02.jpg") };

        com_ptr<ID3D11Resource> loaded_tex;
        DirectX::CreateWICTextureFromMemoryEx(device,
                                              static_cast<const uint8_t*>(tex_file.memory()),
                                              tex_file.size(),
                                              0,
                                              ...
                                              0,
                                              true,
                                              raw_ptr_ptr(loaded_tex),
                                              nullptr);

        device->CreateShaderResourceView(raw_ptr(loaded_tex), nullptr, raw_ptr_ptr(_diffuse_mat));
        if (!_diffuse_mat) {
