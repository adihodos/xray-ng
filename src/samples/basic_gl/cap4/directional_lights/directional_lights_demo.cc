...
void
app::directional_light_demo::init()
{
    {
        ...
        _index_buffer = [&mesh]() {
            GLuint handle{};
            gl::CreateBuffers(1, &handle);
            gl::NamedBufferStorage(handle, mesh.index_count * sizeof(uint32_t), raw_ptr(mesh.indices), 0);
    ...
    {
        const GLuint shaders[] = { make_shader(gl::VERTEX_SHADER, "shaders/cap4/directional_lights/vert_shader.glsl"),
...
void
app::directional_light_demo::draw(const xray::rendering::draw_context_t& dc) noexcept
{
...
void
app::directional_light_demo::update(const float /*delta_ms*/) noexcept
{
}

void
app::directional_light_demo::key_event(const int32_t /*key_code*/,
