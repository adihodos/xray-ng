...
app::per_fragment_lighting_demo::per_fragment_lighting_demo() { init(); }

app::per_fragment_lighting_demo::~per_fragment_lighting_demo() {}

void app::per_fragment_lighting_demo::init() {
  //
  // Shaders & program.
  {
    const GLuint compiled_shaders[] = {
        make_shader(gl::VERTEX_SHADER,
                    "shaders/cap4/per_fragment_lighting/vert_shader.glsl"),
        make_shader(gl::FRAGMENT_SHADER,
                    "shaders/cap4/per_fragment_lighting/frag_shader.glsl")};
  ...
  // Vertex array object.
  _vertex_arr_obj =
      [ vbh = raw_handle(_vertex_buff), ibh = raw_handle(_index_buff) ]() {
    GLuint vao{};
  ...
  // Setup lights.
  {
    for (uint32_t idx = 0; idx < per_fragment_lighting_demo::NUM_LIGHTS;
         ++idx) {
      const auto x_pos =
...
void app::per_fragment_lighting_demo::update(
    const float /*delta_ms*/) noexcept {
...
void app::per_fragment_lighting_demo::key_event(
    const int32_t /*key_code*/, const int32_t /*action*/,
