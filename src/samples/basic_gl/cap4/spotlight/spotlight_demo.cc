...
app::spotlight_demo::spotlight_demo() { init(); }
...
void app::spotlight_demo::init() {
  ...
  //
  {
    ...
    const GLuint compiled_shaders[] = {
        make_shader(gl::VERTEX_SHADER, "shaders/cap4/spotlight/shader.vert"),
  ...
  // Vertex + index buffers
  {
    ...
    copy(raw_ptr(mesh.indices), raw_ptr(mesh.indices) + mesh.index_count,
         back_inserter(indices));
    copy(raw_ptr(mesh_grid.indices),
         raw_ptr(mesh_grid.indices) + mesh_grid.index_count,
         back_inserter(indices));
...
void app::spotlight_demo::update(const float /*delta_ms*/) {
  if (!_rotate_mesh)
    return;
...
void app::spotlight_demo::key_event(const int32_t key_code,
