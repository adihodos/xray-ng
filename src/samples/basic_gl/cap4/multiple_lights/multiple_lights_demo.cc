...
void
app::multiple_lights_demo::update(const float /*delta_ms*/) noexcept
{
    _rot_angle_x += 0.03f;
...
void
app::multiple_lights_demo::key_event(const int32_t /*key_code*/,
...
void
app::multiple_lights_demo::init()
{

    geometry_data_t obj_meshes[multiple_lights_demo::NUM_MESHES];

    if (!geometry_factory::load_model(&obj_meshes[0], MODEL_FILE_SA23, mesh_import_options::remove_points_lines)) {
    ...
    draw_info_[1].index_end = obj_meshes[0].index_count + obj_meshes[1].index_count;
    ...
    gsl::span<vertex_pntt> input_vert_spans[] = {
        { raw_ptr(obj_meshes[0].geometry), static_cast<ptrdiff_t>(obj_meshes[0].vertex_count) },
        { raw_ptr(obj_meshes[1].geometry), static_cast<ptrdiff_t>(obj_meshes[1].vertex_count) }
    };

    for (const auto& in_vtx_span : input_vert_spans) {
        transform(begin(in_vtx_span), end(in_vtx_span), back_inserter(vertices), [](const auto& in_vertex) {
...

    {
        ...
        {
            const auto in_span = gsl::span<uint32_t>{ raw_ptr(obj_meshes[0].indices),
                                                      static_cast<ptrdiff_t>(obj_meshes[0].index_count) };
            copy(begin(in_span), end(in_span), back_inserter(indices));
        }

        {
            const auto in_span = gsl::span<uint32_t>{ raw_ptr(obj_meshes[1].indices),
...

    {
        ...
        gl::VertexArrayVertexBuffer(vao, 0, raw_handle(vertex_buff_), 0, sizeof(vertex_pn));
...

    {
        const GLuint compiled_shaders[] = {
            make_shader(gl::VERTEX_SHADER, "shaders/cap4/multiple_lights/vert_shader.glsl"),
            make_shader(gl::FRAGMENT_SHADER, "shaders/cap4/multiple_lights/frag_shader.glsl")
        };
...
{
  public:
    explicit scoped_capability_toggle(const GLenum cap) noexcept
        : _cap{ cap }
    {
        mode::on(_cap);
    }
...
void
app::multiple_lights_demo::draw(const xray::rendering::draw_context_t& dc) noexcept
{
    ...
    // Set shared uniforms
    {
        ...
        transform(begin(lights_), end(lights_), begin(lights), [vmtx = dc.view_matrix](const light_source2& in_light) {
...

    {
        ...
        const auto tf_uniform =
            obj_transforms{ dc.view_matrix * to_world, dc.view_matrix, dc.proj_view_matrix * to_world };
        ...
        gl::DrawElementsBaseVertex(gl::TRIANGLES,
                                   static_cast<GLsizei>(draw_info_[1].index_end - draw_info_[1].index_start),
                                   gl::UNSIGNED_INT,
    ...
    {
        const auto to_world = mat4f{ R3::rotate_x(_rot_angle_x) * R3::rotate_y(_rot_angle_y) };

        const auto tf_uniform =
            obj_transforms{ dc.view_matrix * to_world, dc.view_matrix * to_world, dc.proj_view_matrix * to_world };
        ...
        gl::DrawElementsBaseVertex(gl::TRIANGLES,
                                   static_cast<GLsizei>(draw_info_[0].index_end - draw_info_[0].index_start),
                                   gl::UNSIGNED_INT,
