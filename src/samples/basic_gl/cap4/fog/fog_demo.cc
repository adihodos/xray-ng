...
static constexpr const char* const FOG_IMPLEMENTATIONS[] = { "fog_exponential", "fog_linear" };
...
void
app::fog_demo::draw(const xray::rendering::draw_context_t& dc)
{
    assert(valid());
    ...
    const light_source scene_light{ mul_point(dc.view_matrix, _light.position),
                                    0.0f,
                                    color_palette::material::black,
                                    color_palette::material::white,
                                    color_palette::material::white };

    _draw_prog.set_uniform_block("scene_lights", scene_light);
    _draw_prog.set_subroutine_uniform(graphics_pipeline_stage::fragment, "fog_type", FOG_IMPLEMENTATIONS[_linear_fog]);
    ...
    for (const auto& mesh : _meshes) {
        ...
        _draw_prog.set_uniform_block("object_material", material::stdc::red_plastic);
...
void
app::fog_demo::update(const float /*delta_ms*/)
{
}

void
app::fog_demo::key_event(const int32_t key_code, const int32_t action, const int32_t /*mods*/)
{
