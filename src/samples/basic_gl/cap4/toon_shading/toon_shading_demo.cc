...
void
app::toon_shading_demo::init()
{
    //
    // Shaders
    {
        const GLuint compiled_shaders[] = {
            make_shader(gl::VERTEX_SHADER, "shaders/cap4/toon_shading/vert_shader.glsl"),
            make_shader(gl::FRAGMENT_SHADER, "shaders/cap4/toon_shading/frag_shader.glsl")
        };
    ...
    // Vertex + index buffers
    {
        ...
        _index_buffer = [&mesh]() {
            ...
            gl::NamedBufferStorage(ibuff, mesh.index_count * sizeof(uint32_t), raw_ptr(mesh.indices), 0);
...
void
app::toon_shading_demo::draw(const xray::rendering::draw_context_t& dc) noexcept
{
...

    {
        ...
        scene_light.position = { 10.0f * std::cos(_light_angle), 10.0f, 10.0f * std::sin(_light_angle) };
        ...
        } const obj_transforms_pack{ dc.view_matrix, dc.view_matrix, dc.proj_view_matrix };
...
void
app::toon_shading_demo::update(const float delta_ms) noexcept
{
    _light_angle += 0.015f;
...
void
app::toon_shading_demo::key_event(const int32_t key_code, const int32_t action, const int32_t mods) noexcept
{
}
