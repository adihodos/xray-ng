...
app::colored_circle_demo::colored_circle_demo()
{
    init();
}

void
app::colored_circle_demo::init() noexcept
{
    ...
    vertex_buff_ = []() {
        ...
        gl::NamedBufferStorage(vbh, (GLsizei)sizeof(quad_vertices), quad_vertices, 0);
    ...
    index_buff_ = []() {
        ...
        gl::NamedBufferStorage(ibh, (GLsizei)sizeof(quad_indices), quad_indices, 0);
        return ibh;
    }();

    layout_desc_ = [vb = raw_handle(vertex_buff_), ib = raw_handle(index_buff_)]() {
        GLuint vao{};
        gl::CreateVertexArrays(1, &vao);
        gl::VertexArrayVertexBuffer(vao, 0, vb, 0, (GLsizei)sizeof(vertex_pt));
        gl::VertexArrayElementBuffer(vao, ib);

        gl::VertexArrayAttribFormat(vao, 0, 3, gl::FLOAT, gl::FALSE_, XR_U32_OFFSETOF(vertex_pt, position));
        gl::VertexArrayAttribFormat(vao, 1, 2, gl::FLOAT, gl::FALSE_, XR_U32_OFFSETOF(vertex_pt, texcoord));
...
void
app::colored_circle_demo::draw(const xray::rendering::draw_context_t& ctx)
{
    ...
    gl::ViewportIndexedf(0, 0.0f, 0.0f, (float)ctx.window_width, (float)ctx.window_height);
    ...
    gl::DrawElements(gl::TRIANGLES, XR_U32_COUNTOF(quad_indices), gl::UNSIGNED_SHORT, nullptr);
}

void
app::colored_circle_demo::update(const float /* delta_ms */)
{
}

void
app::colored_circle_demo::event_handler(const xray::ui::window_event& /* evt */)
{
}

void
app::colored_circle_demo::compose_ui()
{
