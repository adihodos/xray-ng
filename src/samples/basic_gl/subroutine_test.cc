...
app::basic_object::basic_object() { init(); }

app::basic_object::~basic_object() {}

void app::basic_object::init() {
  assert(!valid());
...

  {
    ...
    transform(begin(mesh_data.geometry()), end(mesh_data.geometry()),
              begin(mesh_verts), [](const vertex_pntt_t& vs_in) {
                return vertex_pn{vs_in.position, vs_in.normal};
              });

    vertex_buffer_ =
        make_buffer(gl::ARRAY_BUFFER, 0, &mesh_verts[0], mesh_verts.size());
    ...
    index_buffer_ =
        make_buffer(gl::ELEMENT_ARRAY_BUFFER, 0, raw_ptr(mesh_data.indices),
...

  {
    scoped_shader_handle vertex_shader{
        make_shader(gl::VERTEX_SHADER, "less4/lighting.glsl")};
    ...
    scoped_shader_handle fragment_shader{
        make_shader(gl::FRAGMENT_SHADER, "less4/frag_shader.glsl")};
    ...
    const GLuint shader_handles[] = {raw_handle(vertex_shader),
...
void app::basic_object::draw(
    const xray::rendering::draw_context_t& draw_context) {
  gl::BindVertexArray(raw_handle(layout_));
...
void app::basic_object::update(const float /* delta */) {}