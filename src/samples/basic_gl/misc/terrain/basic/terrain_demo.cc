...
template<typename HeightGenerator>
void
terrain_generator::make_terrain(const int32_t tiles_x,
                                ...
                                std::vector<uint32_t>& terrain_indices)
{
    ...
    const auto delta_uv = vec2f{ 1.0f / static_cast<float>(tiles_x), 1.0f / static_cast<float>(tiles_z) };
    ...
    for (int32_t z = 0; z <= tiles_z; ++z) {
        for (int32_t x = 0; x <= tiles_x; ++x) {
            vertex_pnt v;
            v.position = vec3f{ static_cast<float>(x) * tile_step - hw, 0.0f, static_cast<float>(z) * tile_step - hh };
            v.normal = vec3f::stdc::unit_y;
            v.texcoord = vec2f{ static_cast<float>(x) * delta_uv.x, static_cast<float>(z) * delta_uv.y };
...
app::terrain_demo::init()
{
    ...
    terrain_generator::make_terrain(1023, 1023, 1.0f, null_height_gen{}, vertices, indices);
    ...
    _vs = gpu_program_builder{}.add_file("shaders/misc/terrain/basic/vs.glsl").build<render_stage::e::vertex>();

    if (!_vs)
        return;

    _fs = gpu_program_builder{}.add_file("shaders/misc/terrain/basic/fs.glsl").build<render_stage::e::fragment>();
    ...
    {
        texture_loader tldr{ xr_app_config->texture_path("worlds/test/heightmap01.png").c_str() };
        ...
        gl::TextureStorage2D(raw_handle(_heightmap), 1, tldr.internal_format(), tldr.width(), tldr.height());
...

    {
        ...
        gl::TextureStorage2D(raw_handle(_colormap), 1, tldr.internal_format(), tldr.width(), tldr.height());
        gl::TextureSubImage2D(
            raw_handle(_colormap), 0, 0, 0, tldr.width(), tldr.height(), tldr.format(), gl::UNSIGNED_BYTE, tldr.data());
    }
...
void
app::terrain_demo::draw(const float surface_width, const float surface_height)
{
    assert(valid());
...

    {
        ...
        const GLuint bound_textures[] = { raw_handle(_heightmap), raw_handle(_colormap) };
        ...
        _vs.set_uniform("world_view_proj_mtx", _camera.projection_view() * mat4f{ R3::scale_y(_terrain_opts.scaling) });
        ...
        if (_terrain_opts.wireframe) {
            ...
            gl::DrawElements(gl::TRIANGLES, _mesh.index_count(), gl::UNSIGNED_INT, nullptr);
        } else {
            gl::DrawElements(gl::TRIANGLES, _mesh.index_count(), gl::UNSIGNED_INT, nullptr);
        }
    }
...
void
app::terrain_demo::draw_ui(const int32_t surface_width, const int32_t surface_height)
{
...
void
app::terrain_demo::event_handler(const xray::ui::window_event& evt)
{
    if (evt.type == event_type::configure) {
        ...
        _camera.set_projection(projections_rh::perspective_symmetric(
            static_cast<float>(cfg->width) / static_cast<float>(cfg->height), radians(70.0f), 0.1f, 1000.0f));
...
void
app::terrain_demo::loop_event(const xray::ui::window_loop_event& wle)
{
    _camcontrol.update();
    _ui->tick(1.0f / 60.0f);
