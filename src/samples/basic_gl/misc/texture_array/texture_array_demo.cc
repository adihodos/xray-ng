...
static constexpr const char* TEXTURE_FILES[] = { "ash_uvgrid01.jpg", "ash_uvgrid02.jpg", "ash_uvgrid03.jpg",
                                                 "ash_uvgrid04.jpg", "ash_uvgrid05.jpg", "ash_uvgrid06.jpg",
                                                 "ash_uvgrid07.jpg", "ash_uvgrid08.jpg", "ash_uvgrid09.png",
                                                 "ash_uvgrid10.jpg" };
    ...
    : app::DemoBase{ init_ctx }
{
    _quad_vb = []() {
        ...
        gl::NamedBufferStorage(buffer, (GLsizeiptr)sizeof(QUAD_VERTICES), QUAD_VERTICES, 0);
        return buffer;
    }();
    ...
    _quad_ib = []() {
        ...
        gl::NamedBufferStorage(index_buff, (GLsizeiptr)sizeof(QUAD_INDICES), QUAD_INDICES, 0);
        return index_buff;
    }();
    ...
    _quad_layout = [vb = raw_handle(_quad_vb), ib = raw_handle(_quad_ib)]() {
        ...
        gl::VertexArrayAttribFormat(vao, 0, 2, gl::FLOAT, gl::FALSE_, XR_U32_OFFSETOF(vertex2pt, pos));
        gl::VertexArrayAttribFormat(vao, 1, 2, gl::FLOAT, gl::FALSE_, XR_U32_OFFSETOF(vertex2pt, texc));
    ...
    _vs =
        gpu_program_builder{}.add_file("shaders/misc/texture_array/shader.vert.glsl").build<render_stage::e::vertex>();
    ...
    _texture_array = []() {
        ...
        gl::TextureStorage3D(texture, 4, gl::RGBA8, 1024, 1024, XR_I32_COUNTOF(TEXTURE_FILES));
    ...
    gl::SamplerParameteri(raw_handle(_sampler), gl::TEXTURE_MIN_FILTER, _min_filter);
    gl::SamplerParameteri(raw_handle(_sampler), gl::TEXTURE_MAG_FILTER, _mag_filter);
    ...
    gl::SamplerParameterfv(raw_handle(_sampler), gl::TEXTURE_BORDER_COLOR, _border_color.components);
...
void
app::texture_array_demo::event_handler(const xray::ui::window_event& evt)
{
...
static constexpr texture_param_info TEXTURE_WRAP_MODES[] = { TEX_PARAM_INFO_ENTRY(gl::REPEAT),
...
static constexpr texture_param_info TEXTURE_SAMPLING_MODES[] = { TEX_PARAM_INFO_ENTRY(gl::NEAREST),
...
void
app::texture_array_demo::draw_ui(const int32_t surface_width, const int32_t surface_height)
{
    ...
    if (ImGui::Begin("Options", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::Button("<< Previous image")) {
            ...
            _image_index = clamp(_image_index - 1, 0, XR_I32_COUNTOF(TEXTURE_FILES) - 1);
        }
...

        {
            ...
            for_each(begin(TEXTURE_WRAP_MODES), end(TEXTURE_WRAP_MODES), [this](const auto& twm) {
                const auto selected = ImGui::RadioButton(twm.desc, _wrap_mode == twm.mode);
            ...
            if (_wrap_mode == gl::CLAMP_TO_BORDER) {
                const bool color_changed = ImGui::ColorEdit4("Border color", _border_color.components, false);
                if (color_changed) {
                    gl::SamplerParameterfv(raw_handle(_sampler), gl::TEXTURE_BORDER_COLOR, _border_color.components);
                }
            }
        ...
        ImGui::Separator();
        {
            ...
            for_each(begin(TEXTURE_SAMPLING_MODES), end(TEXTURE_SAMPLING_MODES), [this](const auto& samp_mode) {
                const auto sel_changed = ImGui::RadioButton(samp_mode.desc, samp_mode.mode == _min_filter);
        ...
        ImGui::Separator();
        {
            ...
            int32_t id{};
            for_each(
                begin(TEXTURE_SAMPLING_MODES), begin(TEXTURE_SAMPLING_MODES) + 2, [this, &id](const auto& samp_mode) {
                    ImGui::PushID(id++);

                    const auto sel_changed = ImGui::RadioButton(samp_mode.desc, samp_mode.mode == _mag_filter);
