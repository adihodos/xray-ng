...
static const rgb_color INSTANCE_COLORS[] = {
    color_palette::flat::alizarin300,     color_palette::flat::amethyst100,    color_palette::flat::asbestos800,
    color_palette::flat::belizehole400,   color_palette::flat::carrot300,      color_palette::flat::clouds500,
    color_palette::flat::concrete900,     color_palette::flat::emerald400,     color_palette::flat::greensea200,
    color_palette::flat::midnightblue200, color_palette::flat::nephritis400,   color_palette::flat::orange400,
    color_palette::flat::peterriver100,   color_palette::flat::pomegranate400, color_palette::flat::pumpkin600,
    color_palette::flat::silver800,       color_palette::flat::sunflower200,   color_palette::flat::turquoise300,
...
app::geometric_shapes_demo::geometric_shapes_demo(const init_context_t& init_ctx)
    : app::demo_base{ init_ctx }
{
...

    {
        ...
        gl::NamedBufferStorage(
            raw_handle(_render.vertices_sphere), container_bytes_size(sphere_vertices), sphere_vertices.data(), 0);

        gl::CreateBuffers(1, raw_handle_ptr(_render.indices_sphere));
        gl::NamedBufferStorage(
            raw_handle(_render.indices_sphere), sphere.index_count * sizeof(uint32_t), raw_ptr(sphere.indices), 0);
...

    {
        ...
        for (;;) {
            ...
            pg.color = INSTANCE_COLORS[rnd.next_uint(0, XR_U32_COUNTOF(INSTANCE_COLORS))];
    ...
    // Floor plane
    {
        ...
        for (size_t y = 0; y < 1024; ++y) {
            for (size_t x = 0; x < 1024; ++x) {
                ...
                v.color = color ? color_palette::web::black : color_palette::flat::silver500;
        ...
        gl::NamedBufferStorage(raw_handle(_render.vertices), container_bytes_size(vertices), vertices.data(), 0);

        gl::CreateBuffers(1, raw_handle_ptr(_render.indices));
        gl::NamedBufferStorage(
            raw_handle(_render.indices), grid.index_count * sizeof(uint32_t), raw_ptr(grid.indices), 0);
    }

    gl::CreateVertexArrays(1, raw_handle_ptr(_render.vertexarray));
    const auto va = raw_handle(_render.vertexarray);
    gl::VertexArrayVertexBuffer(va, 0, raw_handle(_render.vertices), 0, sizeof(vertex_pnc));
    gl::VertexArrayElementBuffer(va, raw_handle(_render.indices));
    gl::VertexArrayAttribFormat(va, 0, 3, gl::FLOAT, gl::FALSE_, offsetof(vertex_pnc, pos));
    gl::VertexArrayAttribFormat(va, 1, 3, gl::FLOAT, gl::FALSE_, offsetof(vertex_pnc, normal));
    gl::VertexArrayAttribFormat(va, 2, 4, gl::FLOAT, gl::FALSE_, offsetof(vertex_pnc, color));
    ...
    _render.fs =
        gpu_program_builder{}.add_file("shaders/misc/geometric_shapes/fs.glsl").build<render_stage::e::fragment>();
    ...
    _render.pipeline.use_vertex_program(_render.vs).use_fragment_program(_render.fs);

    _scene.camera.set_projection(projections_rh::perspective_symmetric(static_cast<float>(init_ctx.surface_width),
...

    {
        ...
        transform(begin(rs), end(rs), back_inserter(fonts_to_load), [](const char* ffile) {
            return font_info{ ffile, 14.0f };
        });

        _ui = xray::base::make_unique<imgui_backend>(fonts_to_load.data(), fonts_to_load.size());
...
void
app::geometric_shapes_demo::draw()
{
    ...
    {
        scoped_resource_mapping instbuff{ raw_handle(_render.instances_ssbo),
        ...
        auto gpu_inst_span = gsl::span<gpu_instance_info>{ static_cast<gpu_instance_info*>(instbuff.memory()),
        ...
        for (size_t i = 0, cnt = _obj_instances.instances.size(); i < cnt; ++i) {
            ...
            gpu_inst_span[i].world_view_proj = transpose(_scene.camera.projection_view() * world_tf);
    ...
    gl::VertexArrayVertexBuffer(
        raw_handle(_render.vertexarray), 0, raw_handle(_render.vertices_sphere), 0, sizeof(vertex_pnc));
    gl::VertexArrayElementBuffer(raw_handle(_render.vertexarray), raw_handle(_render.indices_sphere));

    gl::BindBufferBase(gl::SHADER_STORAGE_BUFFER, 0, raw_handle(_render.instances_ssbo));

    light_source ls_view[XR_COUNTOF(_scene.lights)];
    transform(begin(_scene.lights), end(_scene.lights), begin(ls_view), [s = &_scene](const light_source& in_light) {
        light_source out_light{ in_light };
        out_light.position = normalize(mul_vec(s->camera.view(), in_light.position));
        return out_light;
    });
...

    {
        ...
        gl::DrawElementsInstanced(
            gl::TRIANGLES, _render.index_count_sphere, gl::UNSIGNED_INT, nullptr, _obj_instances.instances.size());
    }

    gl::VertexArrayVertexBuffer(
        raw_handle(_render.vertexarray), 0, raw_handle(_render.vertices), 0, sizeof(vertex_pnc));
    gl::VertexArrayElementBuffer(raw_handle(_render.vertexarray), raw_handle(_render.indices));
    ...
    struct
    {
        ...
        mat4f world_view;
        mat4f normals;
    } const transform_matrices{ _scene.camera.projection_view(), _scene.camera.view(), _scene.camera.view() };
    ...
    {
        scoped_winding_order_setting set_cw{ gl::CW };
        gl::DrawElements(gl::TRIANGLES, _render.index_count, gl::UNSIGNED_INT, nullptr);
    }
}

void
app::geometric_shapes_demo::update(const float delta_ms)
{
    ...
    for_each(begin(_obj_instances.instances),
             ...
             [inst = &_obj_instances, &wind_direction, &idx](particle& p) {
                 p.compute_loads(object_instances::drag_coefficient, wind_direction, object_instances::wind_speed);
...
void
app::geometric_shapes_demo::event_handler(const xray::ui::window_event& evt)
{

    if (evt.type == event_type::configure) {
        _scene.camera.set_projection(
            projections_rh::perspective_symmetric(static_cast<float>(evt.event.configure.width),
...
void
app::geometric_shapes_demo::poll_start(const xray::ui::poll_start_event&)
{
}

void
app::geometric_shapes_demo::poll_end(const xray::ui::poll_end_event&)
{
}

void
app::geometric_shapes_demo::loop_event(const xray::ui::window_loop_event& wle)
{
...
void
app::geometric_shapes_demo::draw_ui(const int32_t wnd_width, const int32_t wnd_height)
{
    ...
    if (ImGui::Begin("Options", nullptr,  ImGuiWindowFlags_AlwaysAutoResize)) {
        ...
        if (ImGui::CollapsingHeader("Lights setup", ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_Framed)) {
            for (size_t i = 0; i < XR_COUNTOF(_scene.lights); ++i) {
                ...
                ImGui::DragFloat3(
                    "Direction", _scene.lights[i].position.components, 1.0f, -100.0f, +100.0f, "%3.3f", 1.0f);
                ImGui::DragFloat3("Diffuse color", _scene.lights[i].kd.components, 0.1f, 0.0f, 1.0f, "%3.3f");
        ...
        if (ImGui::CollapsingHeader("Particle setup", ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_Framed)) {
