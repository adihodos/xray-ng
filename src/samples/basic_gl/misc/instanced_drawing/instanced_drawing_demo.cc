...
app::simple_world::simple_world()
{
    ...
    gl::TextureStorage2D(raw_handle(_heightmap), 1, tl.internal_format(), tl.width(), tl.height());
    gl::TextureSubImage2D(
        raw_handle(_heightmap), 0, 0, 0, tl.width(), tl.height(), tl.format(), tl.pixel_size(), tl.data());
    ...
    const geometry_data_t geometry{ geometry_factory::grid(_worldsize.x, _worldsize.y, 1.0f, 1.0f) };
    ...
    _world = basic_mesh{ std::span{ vertices },
                         std::span{ raw_ptr(geometry.indices), raw_ptr(geometry.indices) + geometry.index_count } };

    _vs = gpu_program_builder{}.add_file("shaders/misc/instanced_draw/vs.world.glsl").build<render_stage::e::vertex>();

    _fs =
        gpu_program_builder{}.add_file("shaders/misc/instanced_draw/fs.world.glsl").build<render_stage::e::fragment>();
...
void
app::simple_world::update(const float)
{
}

void
app::simple_world::draw(const xray::scene::camera* cam)
{
...
app::instanced_drawing_demo::instanced_drawing_demo(const app::init_context_t& init_ctx)
    : app::demo_base{ init_ctx }
{
    ...
    for (const char* f : files) {
        mesh_loader::load(init_ctx.cfg->model_path(f).c_str()).map([&mloaders](mesh_loader&& loaded_mdl) {
    ...
    const auto vertex_bytes =
        accumulate(begin(mloaders), end(mloaders), 0u, [](const size_t curr_bytes, const mesh_loader& ml) {
...

    {
        ...
        gl::NamedBufferStorage(raw_handle(_vertices), vertex_bytes, 0, gl::MAP_WRITE_BIT);

        scoped_resource_mapping bmap{ raw_handle(_vertices), gl::MAP_WRITE_BIT, vertex_bytes };
...

    {
        ...
        gl::NamedBufferStorage(raw_handle(_indices), index_bytes, nullptr, gl::MAP_WRITE_BIT);

        scoped_resource_mapping imap{ raw_handle(_indices), gl::MAP_WRITE_BIT, index_bytes };
...

    {
        ...
        generate_n(back_inserter(draw_call_ids), object_instances::instance_count, [&instance_id]() {
            const auto id = instance_id++;
            return id;
        });

        gl::CreateBuffers(1, raw_handle_ptr(_draw_ids));
        gl::NamedBufferStorage(raw_handle(_draw_ids), container_bytes_size(draw_call_ids), draw_call_ids.data(), 0);

        GL_MARK_BUFFER(raw_handle(_draw_ids), "Instance id buffer");
    }

    _vertexarray = [vb = raw_handle(_vertices), ib = raw_handle(_indices), di = raw_handle(_draw_ids)]() {
        GLuint vao{};
...

    {
        ...
        gl::NamedBufferStorage(raw_handle(_indirect_draw_cmd_buffer), sizeof(draw_cmds), draw_cmds, gl::MAP_WRITE_BIT);

        GL_MARK_BUFFER(raw_handle(_indirect_draw_cmd_buffer), "Indirect draw commands buffer");
    }

    _vs = gpu_program_builder{}.add_file("shaders/misc/instanced_draw/vs.glsl").build<render_stage::e::vertex>();

    if (!_vs) {
        return;
    }

    _fs = gpu_program_builder{}.add_file("shaders/misc/instanced_draw/fs.glsl").build<render_stage::e::fragment>();
    ...
    _textures = []() {
        ...
        platformstl::readdir_sequence dir_contents_reader{ xr_app_config->texture_path("uv_grids").c_str(),
                                                           platformstl::readdir_sequence::files |
                                                               platformstl::readdir_sequence::fullPath };

        const auto textures_count = std::distance(dir_contents_reader.begin(), dir_contents_reader.end());

        XR_LOG_INFO("Texture count {}", textures_count);

        GLint tex_idx{};
        for_each(
            begin(dir_contents_reader),
            end(dir_contents_reader),
            [tex, &store_allocated, textures_count, &tex_idx](const auto& texture_file) {
                ...
                if (!store_allocated) {
                    gl::TextureStorage3D(tex, 1, tl.internal_format(), tl.width(), tl.height(), textures_count);
    ...
    // Create instances
    uint32_t idx{};
    generate_n(back_inserter(_obj_instances.instances), object_instances::instance_count, [r = &_rand, &idx]() {
    ...
    _obj_instances.buffer_transforms = [objs = &_obj_instances]() {
        ...
        gl::NamedBufferStorage(
            buff, sizeof(InstanceXData) * object_instances::instance_count, nullptr, gl::MAP_WRITE_BIT);
    ...
    _obj_instances.buffer_texture_ids = [objs = &_obj_instances]() {
        GLuint buff{};
        gl::CreateBuffers(1, &buff);
        stlsoft::auto_buffer<uint32_t, 32> texid_buff{ object_instances::instance_count };

        for (uint32_t i = 0; i < object_instances::instance_count; ++i) {
            texid_buff.data()[i] = objs->instances[i].texture_id;
        }

        gl::NamedBufferStorage(buff, texid_buff.size() * sizeof(uint32_t), texid_buff.data(), 0);
...
void
app::instanced_drawing_demo::loop_event(const xray::ui::window_loop_event& wle)
{
    ...
    {
        for_each(begin(_obj_instances.instances), end(_obj_instances.instances), [](instance_info& ii) {
        ...
        transform(begin(_obj_instances.instances),
                  ...
                  [this](instance_info& ii) {
                      ...
                      return InstanceXData{ _scene.camera.projection_view() * R4::translate(ii.position) *
                                                obj_rotation * mat4f{ R3::scale(ii.scale) },
                                            obj_rotation };
                  });

        scoped_resource_mapping buffermap{ raw_handle(_obj_instances.buffer_transforms),
                                           gl::MAP_WRITE_BIT,
        ...
        memcpy(buffermap.memory(), instance_transforms.data(), container_bytes_size(instance_transforms));
    }
    ...
    scoped_indirect_draw_buffer_binding draw_indirect_buffer_binding{ raw_handle(_indirect_draw_cmd_buffer) };
...
void
app::instanced_drawing_demo::event_handler(const xray::ui::window_event& evt)
{
