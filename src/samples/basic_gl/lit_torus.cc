...
app::lit_object::init()
{
...

    {
...

        {
            ...
            if (!geometry_factory::load_model(
                    &torus_mesh, MODEL_FILE_VIPER, mesh_import_options::remove_points_lines)) {
        ...
        vbuff_ = make_buffer(
            gl::ARRAY_BUFFER, gl::MAP_READ_BIT, torus_vertices.size() * sizeof(torus_vertices[0]), &torus_vertices[0]);

        if (!vbuff_)
            return;

        ibuff_ = make_buffer(gl::ELEMENT_ARRAY_BUFFER,
                             gl::MAP_READ_BIT,
...

    {
        ...
        gl::VertexArrayVertexBuffer(raw_handle(layout_), 0, raw_handle(vbuff_), 0, sizeof(vertex_pn));

        gl::EnableVertexArrayAttrib(raw_handle(layout_), 0);
        gl::EnableVertexArrayAttrib(raw_handle(layout_), 1);
        gl::VertexArrayAttribFormat(
            raw_handle(layout_), 0, 3, gl::FLOAT, gl::FALSE_, XR_U32_OFFSETOF(vertex_pn, position));
        gl::VertexArrayAttribFormat(
            raw_handle(layout_), 1, 3, gl::FLOAT, gl::FALSE_, XR_U32_OFFSETOF(vertex_pn, normal));
    ...
    {
        scoped_shader_handle vertex_shader{ make_shader(gl::VERTEX_SHADER, "shaders/less4/phong_lighting.glsl") };

        if (!vertex_shader)
            return;

        scoped_shader_handle fragment_shader{ make_shader(gl::FRAGMENT_SHADER, "shaders/less4/frag_passthrough.glsl") };

        if (!fragment_shader)
            return;

        const GLuint shaders[] = { raw_handle(vertex_shader), raw_handle(fragment_shader) };
...
void
app::lit_object::draw(const xray::rendering::draw_context_t& draw_ctx)
{
...

    {
        ...
        const auto model2view = draw_ctx.view_matrix * mat4f{ R3::rotate_y(ry_) * R3::rotate_x(rx_) };
...
void
app::lit_object::update(const float /* delta */)
{
    rx_ += 0.02f;
    if (rx_ > two_pi<float>)
        rx_ = 0.0f;
...
void
app::lit_object::key_event(const int32_t /*key_code*/, const int32_t /*action*/, const int32_t /*mods*/) noexcept
{
}
