...
subroutine vec4 lighting_model_type(in const vec3 vertex_pos, in const vec3 normal);
...
subroutine uniform lighting_model_type lighting_model;
...
subroutine(lighting_model_type)
...
subroutine(lighting_model_type)
vec4 diffuse_only_lighting(in const vec3 vertex_pos, in const vec3 normal) {
    vec4 vertex_color = light_ka * mat_ka;

    const vec3 s = normalize(light_eye_pos - vertex_pos);
