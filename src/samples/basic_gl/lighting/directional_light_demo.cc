...
app::DirectionalLightDemo::DirectionalLightDemo(PrivateConstructionToken,
    ...
    , mScene{ std::move(s) }
{
    lz::chain(mRenderState.geometryObjects)
        .map([](basic_mesh& bm) {
            return GraphicsObject{
                &bm, xray::math::vec3f::stdc::zero, xray::math::vec3f::stdc::zero, mat4f::stdc::identity, 1.0f
            };
        })
        .copyTo(std::back_inserter(mRenderState.graphicsObjects));
    ...
    mFsWatcher.add_watch(ctx.cfg->shader_root(), cpp::bind<&DirectionalLightDemo::filesys_event_handler>(this), true);
...
make_lights()
{
    ...
    itlib::small_vector<vec3f, 8> lightsDirections = lz::chain(lz::range(4))
                                                         .map([=](const int quadrant) {
                                                             const float rotation_angle =
                                                                 static_cast<float>(quadrant) * rotation_step;
    ...
    uniform_int_distribution<uint32_t> rngDistrib{ 0, 127 };
    ...
    itlib::small_vector<KaKdKs> lightColors =
        lz::chain(lightsDirections)
            .map([&](const vec3f&) mutable {
            ...
            .to<itlib::small_vector<KaKdKs>>();

    mt19937 randEngineLights{ 0xbeefdead };
    uniform_int_distribution<uint32_t> lightStrength{ 20, 32 };

    vector<app::DirectionalLight> lightsCollection{ lz::chain(lz::zip(lightsDirections, lightColors))
                                                        .map([&](const std::tuple<vec3f&, KaKdKs&> dir_color_pair) {
                                                            const auto& [dir, colors] = dir_color_pair;
                                                            ...
                                                            return app::DirectionalLight{
                                                                ...
                                                                .direction = dir,
                                                                .shininess =
                                                                    static_cast<float>(lightStrength(randEngineLights)),
                                                                .half_vector = vec3f::stdc::zero,
                                                                .strength =
                                                                    static_cast<float>(lightStrength(randEngineLights)),
                                                            };
                                                        })
                                                        .to<vector<app::DirectionalLight>>() };
...
app::DirectionalLightDemo::create(const app::init_context_t& initContext)
{
    ...
    while (!pathQueue.empty()) {
        ...
        auto meshFileFilterFn = [](const fs::path& path) { return path.extension() == ".bin"; };
    ...
    lz::chain(s.modelFiles).forEach([](const ModelInfo& mi) { XR_LOG_DEBUG("::: {} :::", mi.path.generic_string()); });
    ...
    auto viperMdlItr = find_if(cbegin(s.modelFiles), cend(s.modelFiles), [](const ModelInfo& mdl) {
        return mdl.path.generic_string().find("viper") != string_view::npos;
    });
    ...
    const auto vs_file = initContext.cfg->shader_path("lighting/directional.vert").generic_string();
    vertex_program vertShader = gpu_program_builder{}.add_file(vs_file.c_str()).build<render_stage::e::vertex>();

    if (!vertShader)
        return nullptr;

    const auto fs_file = initContext.cfg->shader_path("lighting/directional.frag").generic_string();
    fragment_program fragShader = gpu_program_builder{}.add_file(fs_file.c_str()).build<render_stage::e::fragment>();
    ...
    struct TextureIdWithColorPair
    {
        uint32_t id;
        rgb_color color;
    };
    const TextureIdWithColorPair texture_size_color_pair[] = { { 256u, color_palette::web::dark_orange },
                                                               { 256u, color_palette::web::navy } };

    itlib::small_vector<scoped_texture> colorTextures{
        lz::chain(texture_size_color_pair)
            .map([](const TextureIdWithColorPair& texture_color_pair) {
                GLuint newTexture{};

                gl::CreateTextures(gl::TEXTURE_2D, 1, &newTexture);

                const auto [texDimensions, texColor] = texture_color_pair;
            ...
            .to<itlib::small_vector<scoped_texture>>()
    };
...
app::DirectionalLightDemo::draw()
{
    ...
    lz::chain(mScene.lights)
        .take(mScene.lightsCount)
        .map([this](const DirectionalLight& inputLight) {
        ...
        .copyTo(std::begin(scene_lights.lights));
    ...
    lz::chain(mRenderState.graphicsObjects)
        .forEach([r = &mRenderState, s = &mScene, &scene_lights](GraphicsObject& obj) {
    ...
    lz::chain(mScene.lights)
        .take(mScene.lightsCount)
        .forEach([dbgDraw = &mRenderState.mDebugDraw](const DirectionalLight& dl) {
            dbgDraw->draw_directional_light(dl.direction, 32.0f, dl.kd);
        });

    lz::chain(mRenderState.graphicsObjects)
        .filter([](const GraphicsObject& obj) {
        ...
        .forEach([cam = &mScene.cam, dbg_draw = &mRenderState.mDebugDraw](const GraphicsObject& obj) {
    ...
    static bool dont_remember_what_was_the_idea_here{ false };
    if (!dont_remember_what_was_the_idea_here) {
        cam_data = mScene.cam.projection_view_bijection();
        dont_remember_what_was_the_idea_here = true;
    }
...
{
    lz::chain(mRenderState.graphicsObjects).forEach([opts = &mDemoOptions](GraphicsObject& obj) {
...
app::DirectionalLightDemo::draw_ui(const int32_t surface_w, const int32_t surface_h)
{
    ...
    if (ImGui::Begin("Options", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove)) {
        ...
        if (ImGui::CollapsingHeader("Objects", ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_Framed)) {
            lz::chain(mRenderState.graphicsObjects).forEach([](GraphicsObject& obj) {
                ImGui::PushID(&obj);

                ImGui::Text("::: Object :::");
                ImGui::Separator();

                lz::chain(rfl::get_underlying_enumerator_array<GraphicsObject::StateBits>())
                    .forEach([&obj](const pair<string_view, uint8_t> enum_name_id_pair) {
        ...
        if (ImGui::CollapsingHeader("Debug", ImGuiTreeNodeFlags_Framed)) {
            ...
            lz::chain(mRenderState.graphicsObjects).forEach([this](const GraphicsObject& graphicsObj) mutable {
...
app::DirectionalLightDemo::switch_mesh(const SwitchMeshOpt switchMeshOpt)
{
    if (switchMeshOpt == SwitchMeshOpt::LoadFromFile) {
        ...
        XR_LOG_DEBUG("Loaded model {}", mScene.modelFiles[mScene.current_mesh_idx].path.generic_string());
...
app::DirectionalLightDemo::filesys_event_handler(const xray::base::FileSystemEvent& fs_event)
{
    if (const FileModifiedEvent* e = get_if<FileModifiedEvent>(&fs_event)) {
        if (e->file.extension() == ".vert") {
            const auto fpath = e->file.generic_string();
            vertex_program prg = gpu_program_builder{}.add_file(fpath.c_str()).build<render_stage::e::vertex>();
        ...
        if (e->file.extension() == ".frag") {
            const auto fpath = e->file.generic_string();
            fragment_program prg = gpu_program_builder{}.add_file(fpath.c_str()).build<render_stage::e::fragment>();
