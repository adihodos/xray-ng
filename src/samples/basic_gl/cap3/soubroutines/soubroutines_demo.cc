...
void
app::soubroutines_demo::init()
{
    {
        geometry_data_t torus_mesh{};
        if (!geometry_factory::load_model(&torus_mesh, MODEL_FILE_VIPER, mesh_import_options::remove_points_lines)) {
...

        {
            ...
            gl::NamedBufferStorage(
                raw_handle(vertex_buffer_), mesh_verts.size() * sizeof(mesh_verts[0]), &mesh_verts[0], 0);
        }

        {
            ...
            gl::NamedBufferStorage(
                raw_handle(index_buffer_), torus_mesh.index_count * sizeof(uint32_t), raw_ptr(torus_mesh.indices), 0);
    ...
    // vertex layout
    {
        ...
        gl::VertexArrayVertexBuffer(raw_handle(vertex_layout_), 0, raw_handle(vertex_buffer_), 0, sizeof(vertex_pn));

        gl::EnableVertexArrayAttrib(raw_handle(vertex_layout_), 0);
        gl::EnableVertexArrayAttrib(raw_handle(vertex_layout_), 1);

        gl::VertexArrayAttribFormat(
            raw_handle(vertex_layout_), 0, 3, gl::FLOAT, gl::FALSE_, XR_U32_OFFSETOF(vertex_pn, position));
        gl::VertexArrayAttribFormat(
            raw_handle(vertex_layout_), 1, 3, gl::FLOAT, gl::FALSE_, XR_U32_OFFSETOF(vertex_pn, normal));
        gl::VertexArrayAttribBinding(raw_handle(vertex_layout_), 0, 0);
        gl::VertexArrayAttribBinding(raw_handle(vertex_layout_), 1, 0);
    }

    {
        constexpr const char* const VS_FILE = "shaders/cap3/soubroutines/vert_shader.glsl";
        constexpr const char* const FS_FILE = "shaders/cap3/soubroutines/frag_shader.glsl";
...
void
app::soubroutines_demo::update(const float)
{
}

void
app::soubroutines_demo::draw(const draw_context_t& draw_ctx)
{
    assert(*this);
...

    {
        ...
        static constexpr const char* const SHADING_MODEL_SUBROUTINES[] = { "light_model_diffuse", "light_model_ads" };

        draw_prog_.set_subroutine_uniform(
            graphics_pipeline_stage::vertex, "shading_model", SHADING_MODEL_SUBROUTINES[use_ads_lighting_]);
    }
...
void
app::soubroutines_demo::key_event(const int32_t key_code, const int32_t action, const int32_t /*mods*/) noexcept
{
