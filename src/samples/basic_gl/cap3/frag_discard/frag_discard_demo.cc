...
app::frag_discard_demo::frag_discard_demo() { init(); }

app::frag_discard_demo::~frag_discard_demo() {}

void app::frag_discard_demo::update(const float /*delta_ms*/) noexcept {}

void app::frag_discard_demo::init() {
  //
  // load mesh
  {
    ...
    auto input_verts = gsl::span<vertex_pntt>{
    ...
    vertex_buff_ = [&vertices]() {
      ...
      gl::NamedBufferStorage(vbuff, vertices.size() * sizeof(vertices[0]),
                             &vertices[0], gl::MAP_READ_BIT);
    ...
    index_buff_ = [&mesh]() {
      ...
      gl::NamedBufferStorage(bhandle, mesh.index_count * sizeof(uint32_t),
                             raw_ptr(mesh.indices), gl::MAP_READ_BIT);
      return bhandle;
    }();
  ...
  {
    vertex_layout_ =
        [ ibh = raw_handle(index_buff_), vbh = raw_handle(vertex_buff_) ]() {
      GLuint vao{};
...
void app::frag_discard_demo::key_event(const int32_t /*key_code*/,
