...
void
app::edge_detect_demo::compose_ui()
{
    if (ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove)) {

        if (ImGui::RadioButton("No edge detection", (_detect_method == edge_detect_none))) {
            _detect_method = edge_detect_none;
        }

        if (ImGui::RadioButton("Sobel edge detection", (_detect_method == edge_detect_sobel))) {
            _detect_method = edge_detect_sobel;
        }

        if (ImGui::RadioButton("Frei-Chen edge detection", (_detect_method == edge_detect_frei_chen))) {
            _detect_method = edge_detect_frei_chen;
        }

        if (_detect_method == edge_detect_sobel) {
            ImGui::Separator();
            ImGui::SliderFloat("Edge treshold", &_edge_treshold, 0.01f, 0.25f, "%3.3f", 0.1f);
        }
        ...
        for (auto first = detail::DEBUGRecord_records_begin(), last = detail::DEBUGRecord_records_end(); first != last;
             ++first) {
            if (first->hit_count == 0)
                continue;

            ImGui::Text("BLOCK: %s %d (%3.3f)", first->func, first->line, first->millis);
            ImGui::Separator();
        }
        ImGui::EndGroup();
    }
...
void
app::edge_detect_demo::draw(const xray::rendering::draw_context_t& draw_ctx)
{
    assert(valid());

    {
        ...
        {
            const float viewport_dim[] = {
                0.0f, 0.0f, static_cast<float>(draw_ctx.window_width), static_cast<float>(draw_ctx.window_height)
            };

            gl::ViewportIndexedfv(0, viewport_dim);
            gl::ClearNamedFramebufferfv(
                raw_handle(_fbo.fbo_object), gl::COLOR, 0, color_palette::web::black.components);
            const float def_clear_depthval{ 1.0f };
            gl::ClearNamedFramebufferfv(raw_handle(_fbo.fbo_object), gl::DEPTH, 0, &def_clear_depthval);
        }
        ...
        } const obj_transforms{ obj_to_view, obj_to_view, draw_ctx.projection_matrix * obj_to_view };

        _vertex_prg.set_uniform_block("transform_pack", obj_transforms);

        point_light lights[edge_detect_demo::max_lights];
        transform(begin(_lights), end(_lights), begin(lights), [&draw_ctx](const auto& in_light) -> point_light {
            return { in_light.ka, in_light.kd, in_light.ks, mul_point(draw_ctx.view_matrix, in_light.position) };
        });
    ...
    // second pass, edge detection
    {
        ...
        const auto surface_size =
            vec2f{ static_cast<float>(draw_ctx.window_width), static_cast<float>(draw_ctx.window_height) };

        constexpr const char* const drawing_subroutines[] = { "color_default",
                                                              "color_edges_sobel",
                                                              "color_edges_frei_chen" };
        ...
        _fs_edge_detect.set_uniform("kSourceTexture", 0)
            ...
            .set_subroutine_uniform("kDrawingStyle", drawing_subroutines[_detect_method]);
...
void
app::edge_detect_demo::update(const float /*delta_ms*/)
{
    if (!_rotate_object)
        return;
...
void
app::edge_detect_demo::key_event(const int32_t /*key_code*/, const int32_t /*action*/, const int32_t /*mods*/)
{
}

void
app::edge_detect_demo::resize_event(const resize_context_t& rctx)
{
    assert(valid());
...
void
app::edge_detect_demo::create_framebuffer(const GLsizei r_width, const GLsizei r_height)
{
    ...
    _fbo.fbo_depth = [r_width, r_height]() {
        ...
        gl::NamedRenderbufferStorage(depth_fb, gl::DEPTH_COMPONENT, r_width, r_height);
        return depth_fb;
    }();
    ...
    _fbo.fbo_object = [f_tex = raw_handle(_fbo.fbo_texture), f_depth = raw_handle(_fbo.fbo_depth)]() {
        GLuint fbo{};
        gl::CreateFramebuffers(1, &fbo);
        gl::NamedFramebufferTexture(fbo, gl::COLOR_ATTACHMENT0, f_tex, 0);
        gl::NamedFramebufferRenderbuffer(fbo, gl::DEPTH_ATTACHMENT, gl::RENDERBUFFER, f_depth);
        gl::NamedFramebufferDrawBuffer(fbo, gl::COLOR_ATTACHMENT0);

        if (gl::CheckNamedFramebufferStatus(fbo, gl::DRAW_FRAMEBUFFER) != gl::FRAMEBUFFER_COMPLETE) {
...
void
app::edge_detect_demo::init(const init_context_t& ini_ctx)
{
    ...
    {
        _vs_edge_detect = gpu_program_builder{}
                              .add_file("shaders/cap6/edge_detect/passthrough.vert.glsl")
                              .build<graphics_pipeline_stage::vertex>();

        _fs_edge_detect = gpu_program_builder{}
...

    {
        ...
        _object = simple_mesh{ vertex_format::pnt, c_str_ptr(ini_ctx.cfg->model_path(model_file)) };
...

    {
        _lightcount =
            xray::math::min<uint32_t>(scene_loader::parse_point_lights_section(
                                          app_cfg.lookup_entry("app.scene.lights"), _lights, XR_U32_COUNTOF__(_lights)),
    ...
    _obj_material = [&app_cfg, &ini_ctx]() {
        ...
        gl::TextureStorage2D(texh, 1, internal_format[tex_ldr.depth() == 4], tex_ldr.width(), tex_ldr.height());
