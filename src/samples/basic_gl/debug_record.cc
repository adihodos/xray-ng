...
void app::detail::DEBUGRecord_enter_scope(const int32_t id, const char* file,
...
void app::detail::DEBUGRecord_exit_scope(const int32_t id, const float msecs) {
  assert(id < static_cast<int32_t>(MAX_DEBUG_COUNTERS));
  DEBUG_RECORDS_STORAGE[id].millis = msecs;
}

const app::detail::debug_record* app::detail::DEBUGRecord_records_begin() {
  return DEBUG_RECORDS_STORAGE;
}

const app::detail::debug_record* app::detail::DEBUGRecord_records_end() {
