...
enum class phong_material_source_type
{
    tex,
    color
};
...
struct phong_material_component_definition
{
    ...
    phong_material_component_definition(const phong_material_source_texture& fd)
        : source_type{ phong_material_source_type::tex }
        , file{ fd }
    {
    }

    phong_material_component_definition(const phong_material_source_color& clr_)
        : source_type{ phong_material_source_type::color }
        , clr{ clr_ }
    {
    }
};
...
struct phong_material_builder
{
  public:
    ...
    phong_material_builder& set_emissive(const phong_material_component_definition& def) noexcept
    {
        set_component_definition(def, phong_material_component::e::emissive);
        return *this;
    }

    phong_material_builder& set_ambient(const phong_material_component_definition& def) noexcept
    {
        set_component_definition(def, phong_material_component::e::ambient);
        return *this;
    }

    phong_material_builder& set_diffuse(const phong_material_component_definition& def) noexcept
    {
        set_component_definition(def, phong_material_component::e::diffuse);
        return *this;
    }

    phong_material_builder& set_specular(const phong_material_component_definition& def) noexcept
    {
        set_component_definition(def, phong_material_component::e::specular);
        return *this;
    }

    phong_material_builder& set_specular_intensity(const float spec_intensity) noexcept
    {
        _spec_pwr = spec_intensity;
        return *this;
    }

    void set_component_definition(const phong_material_component_definition& cdef,
                                  const phong_material_component::e ctype) noexcept
    {
        _cdefs[phong_material_component::to_integer(phong_material_component::e::emissive)] = cdef;
        _cbits |= 1u << phong_material_component::to_integer(phong_material_component::e::emissive);
    }
    ...
    bool valid() const noexcept { return has_component(phong_material_component::e::diffuse); }

  private:
    const phong_material_component_definition& component_def(const phong_material_component::e cmp) noexcept
    {
        assert(has_component(cmp));
        return _cdefs[phong_material_component::to_integer(cmp)];
    }

    bool has_component(const phong_material_component::e cmp_type) const noexcept
    {
        return (_cbits & (1 << phong_material_component::to_integer(cmp_type))) != 0;
    }

  private:
    phong_material_component_definition _cdefs[phong_material_component::size - 1];
    uint32_t _cbits{ 0 };
...
struct mesh_create_info
{
    ...
    mesh_create_info(const char* fname, const xray::rendering::vertex_format fmt) noexcept
        : file_name{ fname }
        , format{ fmt }
    {
    }
...
class scene_loader
{
  public:
    ...
    bool parse_phong_material(const char* mtl_name, phong_material_builder* mtl_builder);

    bool parse_mesh(const char* mesh_name, mesh_create_info* mi);

    bool parse_program(const char* id, xray::rendering::gpu_program_builder* prg_bld);

    uint32_t parse_point_lights(xray::scene::point_light* lighs, const size_t lights_count);

    static uint32_t parse_point_lights_section(const xray::base::config_entry& lights_sec,
  ...
  public:
    ...
    static xray::rendering::rgb_color read_color_f32(const xray::base::config_entry& e)
    {
        xray::rendering::rgb_color clr;
    ...
    static xray::rendering::rgb_color read_color_u32(const xray::base::config_entry& e)
    {
  ...
  private:
    ...
    static uint32_t read_array(const xray::base::config_entry& entry, const size_t max_items, OutputIter out_iter);

  private:
    using path_stack_type = xray::base::fixed_stack<platformstl::basic_path<char>, 4>;
...
template<typename OutputIter>
uint32_t
scene_loader::read_array(const xray::base::config_entry& entry, const size_t max_items, OutputIter out_iter)
{

    assert(entry && "Config entry must be valid!");

    const auto elements_count = xray::math::min<uint32_t>(max_items, entry.length());
