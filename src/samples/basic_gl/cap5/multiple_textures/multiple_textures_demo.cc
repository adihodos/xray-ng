...
void
app::multiple_textures_demo::draw(const xray::rendering::draw_context_t& dc)
{
    assert(valid());
    ...
    const GLuint bound_textures[] = { raw_handle(_base_tex), raw_handle(_overlay_tex) };
    ...
    } const object_transforms{ dc.view_matrix, dc.view_matrix, dc.proj_view_matrix };
...
void
app::multiple_textures_demo::update(const float /*delta_ms*/)
{
}

void
app::multiple_textures_demo::key_event(const int32_t /*key_code*/, const int32_t /*action*/, const int32_t /*mods*/)
{
}

void
app::multiple_textures_demo::init()
{
    {
        const GLuint compiled_shaders[] = {
            make_shader(gl::VERTEX_SHADER, "shaders/cap5/multiple_textures/shader.vert"),
            make_shader(gl::FRAGMENT_SHADER, "shaders/cap5/multiple_textures/shader.frag")
        };
...

    {
        ...
        transform(raw_ptr(mesh.geometry),
                  ...
                  [](const auto& in_vx) {
                      const auto texcoord_gl = vec2f{ in_vx.texcoords.u, 1.0f - in_vx.texcoords.v };

                      return vertex_pnt{ in_vx.position, in_vx.normal * (-1.0f), texcoord_gl };
                  });
    ...
    _base_tex = []() {
        texture_loader tex_ldr{ c_str_ptr(xr_app_config->texture_path("brick1.jpg")) };
