...
public:
  ...
  ~scoped_gl_state() noexcept {
    _saved_state ? gl::Enable(_state_id) : gl::Disable(_state_id);
  }
...
app::discard_alphamap_demo::discard_alphamap_demo() { init(); }

app::discard_alphamap_demo::~discard_alphamap_demo() {}

void app::discard_alphamap_demo::draw(
  ...
  const GLuint bound_textures[] = {raw_handle(_base_texture),
                                   raw_handle(_discard_map)};
...
void app::discard_alphamap_demo::update(const float /*delta_ms*/) {
...
void app::discard_alphamap_demo::key_event(const int32_t /*key_code*/,
                                           const int32_t /*action*/,
                                           const int32_t /*mods*/) {}

void app::discard_alphamap_demo::init() {
  {
    const GLuint shaders[] = {
        make_shader(gl::VERTEX_SHADER,
                    "shaders/cap5/discard_alphamap/shader.vert"),
        make_shader(gl::FRAGMENT_SHADER,
                    "shaders/cap5/discard_alphamap/shader.frag")};
...

  {
    ...
    if (!geometry_factory::load_model(
            &mesh, xr_app_config->model_path("f4/f4phantom.obj").c_str(),
    ...
    _index_buffer = [&mesh]() {
      ...
      gl::NamedBufferStorage(
          ibh, bytes_size(raw_ptr(mesh.indices), mesh.index_count),
