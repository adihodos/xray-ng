...
void
app::reflection_demo::draw(const xray::rendering::draw_context_t& dc)
{
    assert(valid());
...

    {
        ...
        const matrix_pack_t skybox_tf_pack{
            skybox_world, mat4f::stdc::identity, dc.proj_view_matrix * skybox_world, dc.active_camera->origin()
        };

        _draw_prog.set_uniform_block("transform_pack", skybox_tf_pack);
        _draw_prog.set_subroutine_uniform(graphics_pipeline_stage::vertex, "reflection_func", "skybox_func");
        _draw_prog.set_subroutine_uniform(graphics_pipeline_stage::fragment, "reflect_surface_fn", "reflect_skybox");

        _draw_prog.bind_to_pipeline();
        gl::DrawElements(gl::TRIANGLES, _skybox_mesh_index_count, gl::UNSIGNED_INT, nullptr);
    }

    {
        const matrix_pack_t object_tf_pack{
            mat4f::stdc::identity, mat4f::stdc::identity, dc.proj_view_matrix, dc.active_camera->origin()
        };

        _draw_prog.set_uniform_block("transform_pack", object_tf_pack);
        _draw_prog.set_subroutine_uniform(graphics_pipeline_stage::vertex, "reflection_func", "reflect_surface_func");
        _draw_prog.set_subroutine_uniform(graphics_pipeline_stage::fragment, "reflect_surface_fn", "reflect_object");
...
void
app::reflection_demo::update(const float delta_ms)
{
    XR_UNUSED_ARG(delta_ms);
}

void
app::reflection_demo::key_event(const int32_t key_code, const int32_t action, const int32_t mods)
{
...
void
app::reflection_demo::init()
{
    {
        const GLuint compiled_shaders[] = { make_shader(gl::VERTEX_SHADER, "shaders/cap5/reflection/shader.vert"),
                                            make_shader(gl::FRAGMENT_SHADER, "shaders/cap5/reflection/shader.frag") };
    ...
    _vertex_arr = [vbh = raw_handle(_vertex_buffer), ibh = raw_handle(_index_buffer)]() {
        GLuint vao{};
    ...
    _skybox = []() {
        ...
        struct tex_name_and_target
        {
            ...
            GLenum target;
            GLint layer;
        } const cube_faces[] = { { "spacesky/sky_right1.png", gl::TEXTURE_CUBE_MAP_POSITIVE_X, 0 },
        ...
        for (const auto& cube_face : cube_faces) {
            texture_loader tex_ldr{ c_str_ptr(xr_app_config->texture_path(cube_face.file_name))
                                    /*texture_load_options::flip_y*/ };

            if (!tex_ldr)
                return GLuint{};

            gl::TextureSubImage3D(texh,
                                  0,
                                  0,
                                  0,
                                  cube_face.layer,
    ...
    XR_LOG_INFO("Mesh load time (old mode) (milliseconds) {}", ld_timer.elapsed_millis());

    ld_timer.start();
    _spacecraft = simple_mesh{ vertex_format::pn, xr_app_config->model_path(MODEL_FILE).c_str() };
    ld_timer.end();
    ...
    XR_LOG_INFO("Mesh load time (new mode) (milliseconds) {}", ld_timer.elapsed_millis());

    _valid = true;
}

void
app::reflection_demo::compose_ui()
{
    ...
    ImGui::SliderFloat("Reflectivity", &_surface_params._reflectivity, 0.0f, 1.0f, "%3.3f", 1.0f);
    ImGui::Separator();
    ImGui::SliderFloat("Red", &_surface_params._color.r, 0.0f, 1.0f, "%3.3f", 1.0f);
    ImGui::SliderFloat("Green", &_surface_params._color.g, 0.0f, 1.0f, "%3.3f", 1.0f);
    ImGui::SliderFloat("Blue", &_surface_params._color.b, 0.0f, 1.0f, "%3.3f", 1.0f);
    ImGui::End();
}
