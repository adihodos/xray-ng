...
void
app::refraction_demo::compose_ui()
{
    ImGui::Begin("Surface parameters", nullptr, ImGuiWindowFlags_AlwaysAutoResize);

    {
        const bool val_changed =
            ImGui::SliderFloat("Refractivity", &_surfaceparams.refract_surface, 0.0f, 1.0f, "%3.3f", 1.0f);
...

    {
        const auto val_changed =
            ImGui::SliderFloat("Reflectivity", &_surfaceparams.reflect_surface, 0.0f, 1.0f, "%3.3f", 1.0f);
...
void
app::refraction_demo::draw(const xray::rendering::draw_context_t& dc)
{
    assert(valid());
    ...
    // draw skybox
    {
        ...
        scoped_vertex_array_binding skybox_vao_binding{ raw_handle(_vertexarray_skybox) };

        const auto skybox_world_tf = R4::translate(dc.active_camera->origin());
        const matrix_pack skybox_matrices{ skybox_world_tf, skybox_world_tf, dc.proj_view_matrix * skybox_world_tf };

        _drawprogram.set_uniform_block("transforms_pack", skybox_matrices);
        _drawprogram.set_subroutine_uniform(graphics_pipeline_stage::vertex, "surface_process_func", "process_skybox");
        _drawprogram.set_subroutine_uniform(graphics_pipeline_stage::fragment, "color_surface_func", "color_skybox");
        _drawprogram.set_uniform("environment_map", 0);
        _drawprogram.bind_to_pipeline();
        gl::DrawElements(gl::TRIANGLES, _skybox_indexcount, gl::UNSIGNED_INT, nullptr);
...

    {
        ...
        const matrix_pack spacecraft_matrices{ spacecraft_world_tf,
                                               spacecraft_world_tf,
        ...
        _drawprogram.set_subroutine_uniform(graphics_pipeline_stage::vertex, "surface_process_func", "process_surface");
        _drawprogram.set_uniform("surface_reflection", _surfaceparams.reflect_surface);
        _drawprogram.set_uniform("environment_map", 0);
        _drawprogram.set_subroutine_uniform(graphics_pipeline_stage::fragment, "color_surface_func", "color_surface");
        _drawprogram.bind_to_pipeline();
...
void
app::refraction_demo::update(const float /*delta_ms*/)
{
}

void
app::refraction_demo::key_event(const int32_t /*key_code*/, const int32_t /*action*/, const int32_t /*mods*/)
{
}

void
app::refraction_demo::init()
{

    _drawprogram = []() {
        const GLuint compiled_shaders[] = { make_shader(gl::VERTEX_SHADER, "shaders/cap5/refraction/shader.vert"),
                                            make_shader(gl::FRAGMENT_SHADER, "shaders/cap5/refraction/shader.frag") };
...

    {
        ...
        _indexbuffer_skybox = [data = raw_ptr(skybox_mesh.indices),
                               buff_size = bytes_size(raw_ptr(skybox_mesh.indices), skybox_mesh.index_count)]() {
            GLuint ibuff{};
    ...
    _vertexarray_skybox = [vb = raw_handle(_vertexbuffer_skybox), ib = raw_handle(_indexbuffer_skybox)]() {
        GLuint vao{};
    ...
    _skybox_texture = []() {
        ...
        for (const auto& face : cube_faces) {
            texture_loader tex_ldr{ c_str_ptr(xr_app_config->texture_path(face.file_name)),
