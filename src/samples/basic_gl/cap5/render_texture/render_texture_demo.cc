...
bool
import_light(xray::base::config_file* cfile, const char* cfg_path, app::light_source4* ls)
{
    char tmp_name[512];
...
void
app::render_texture_demo::compose_ui()
{
    {
        ...
        ImGui::ColorEdit3("Normal start color", _dbg_opts.normal_color_beg.components);
        ImGui::ColorEdit3("Normal end color", _dbg_opts.normal_color_end.components);
        ImGui::SliderFloat("Normal line length", &_dbg_opts.normal_line_len, 0.1f, 5.0f, "%3.3f");
        ImGui::SliderFloat3("Light position", _dbg_opts.light_pos.components, -100.0f, +100.0f, "%3.3f");
        ImGui::End();
    }
}

void
app::render_texture_demo::draw(const xray::rendering::draw_context_t& dc)
{
    assert(valid());
    ...
    {
        const auto view_mtx = view_frame::look_at(_rto.cam_pos, vec3f::stdc::zero, vec3f::stdc::unit_y);
        const auto proj_mtx = projection::perspective_symmetric(static_cast<float>(_rto.rendertarget_size.x),
                                                                static_cast<float>(_rto.rendertarget_size.y),
                                                                radians(_rto.fov),
                                                                0.5f,
                                                                100.0f);
        ...
        const matrix_transform_pack tf_pack_spacecraft{ view_mtx, view_mtx, proj_view_mtx };
        ...
        gl::Viewport(
            0, 0, static_cast<GLsizei>(_rto.rendertarget_size.x), static_cast<GLsizei>(_rto.rendertarget_size.y));

        gl::ClearColor(_rto.tex_clear_color.r, _rto.tex_clear_color.g, _rto.tex_clear_color.b, _rto.tex_clear_color.a);
    ...
    {
        const auto cube_world = _dbg_opts.object_rotates ? mat4f{ R3::rotate_y(_cube_r_angle) } : mat4f::stdc::identity;
        const auto model_view = dc.view_matrix * cube_world;
        const auto model_view_proj = dc.proj_view_matrix * cube_world;
        const matrix_transform_pack tf_pack_cube{ model_view, model_view, model_view_proj };

        const light_source4 scene_lights[] = {
            { _rto.lights[0].color, mul_point(dc.view_matrix, _rto.lights[0].position) },
            { _rto.lights[1].color, mul_point(dc.view_matrix, _rto.lights[1].position) },
        };

        gl::BindTextureUnit(0, raw_handle(_fbo_texture));
        gl::Viewport(0, 0, static_cast<GLsizei>(dc.window_width), static_cast<GLsizei>(dc.window_height));
        gl::ClearColor(
            _rto.scene_clear_color.r, _rto.scene_clear_color.g, _rto.scene_clear_color.b, _rto.scene_clear_color.a);
        gl::Clear(gl::COLOR_BUFFER_BIT | gl::DEPTH_BUFFER_BIT);
        ...
        if (_dbg_opts.debug_normals) {
            ...
            _drawprogram_debug.set_uniform("normal_length", _dbg_opts.normal_line_len);
            _drawprogram_debug.set_uniform("start_point_color", _dbg_opts.normal_color_beg);
            _drawprogram_debug.set_uniform("end_point_color", _dbg_opts.normal_color_end);
...
void
app::render_texture_demo::update(const float /*delta_ms*/)
{
...
void
app::render_texture_demo::key_event(const int32_t /*key_code*/, const int32_t /*action*/, const int32_t /*mods*/)
{
}

void
app::render_texture_demo::init()
{

    _drawprogram = []() {
        const GLuint compiled_shaders[] = { make_shader(gl::VERTEX_SHADER, "shaders/cap5/render_texture/shader.vert"),
    ...
    _drawprogram_debug = []() {
        const GLuint compiled_shaders[] = { make_shader(gl::VERTEX_SHADER, "shaders/dbg/debug.vs"),
    ...
    _fbo = [tex = raw_handle(_fbo_texture), depth_buffer = raw_handle(_fbo_depthbuffer)]() {
        GLuint fbo{};
        gl::CreateFramebuffers(1, &fbo);
        gl::NamedFramebufferTexture(fbo, gl::COLOR_ATTACHMENT0, tex, 0);
        gl::NamedFramebufferRenderbuffer(fbo, gl::DEPTH_ATTACHMENT, gl::RENDERBUFFER, depth_buffer);
...

    {
        ...
        demo_cfg.lookup_value("app.params.render_tex_size.x", _rto.rendertarget_size.x);
        demo_cfg.lookup_value("app.params.render_tex_size.y", _rto.rendertarget_size.y);
        ...
        demo_cfg.lookup_vector_or_matrix("app.params.clear_color.texture", _rto.tex_clear_color);
        demo_cfg.lookup_vector_or_matrix("app.params.clear_color.scene", _rto.scene_clear_color);
    }

    {
        const char* mdl_file{ nullptr };
        demo_cfg.lookup_value("app.params.model", mdl_file);
        _spacecraft_mesh = simple_mesh{ vertex_format::pnt, c_str_ptr(xr_app_config->model_path(mdl_file)) };
    ...
    _cube_mesh = [&demo_cfg, this]() {
        ...
        if (!strcmp("torus", shape_type)) {
            demo_cfg.lookup_value("app.shape_config.torus.outer_radius", torus.outer_radius);
            demo_cfg.lookup_value("app.shape_config.torus.inner_radius", torus.inner_radius);
            ...
            geometry_factory::torus(torus.outer_radius, torus.inner_radius, torus.sides, torus.rings, &obj_mesh);
            ...
            demo_cfg.lookup_value("app.shape_config.sphere.max_subdiv", sph.max_subdivisions);
    ...
    _spacecraft_material = [&demo_cfg]() {
        ...
        texture_loader tex_ldr{ c_str_ptr(xr_app_config->texture_path(material)), texture_load_options::flip_y };
        ...
        GLuint texh{};
        gl::CreateTextures(gl::TEXTURE_2D, 1, &texh);
        gl::TextureStorage2D(texh, 1, tex_ldr.depth() == 4 ? gl::RGBA8 : gl::RGB8, tex_ldr.width(), tex_ldr.height());
    ...
    _null_texture = []() {
        ...
        gl::TextureSubImage2D(texh, 0, 0, 0, 1, 1, gl::RGB, gl::UNSIGNED_BYTE, white_pixel);
        return texh;
    }();
