...
void
app::textures_demo::draw(const xray::rendering::draw_context_t& dc)
{
    assert(valid());
...
void
app::textures_demo::update(const float /*delta_ms*/)
{
}

void
app::textures_demo::key_event(const int32_t /*key_code*/, const int32_t /*action*/, const int32_t /*mods*/)
{
}

void
app::textures_demo::init()
{
    {
        const GLuint compiled_shaders[] = { make_shader(gl::VERTEX_SHADER, "shaders/cap5/textures/shader.vert"),
...
const app::shape_name_id_tuple AVAILABLE_SHAPES[] = { MAKE_SHAPE_TUPLE(tetrahedron), MAKE_SHAPE_TUPLE(hexahedron),
...
app::textures_demo::compose_ui()
{
    ...
    ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_AlwaysAutoResize);
    {
        ...
        ImGui::Combo("Shapes",
                     &sel_item_idx,
                     &combo_filler::func,
                     (void*)AVAILABLE_SHAPES,
                     XR_I32_COUNTOF__(AVAILABLE_SHAPES),
                     5);
...
void
app::textures_demo::update_shape(const uint32_t index)
{
    ...
    switch (shape_data.s_type) {
        ...
        case shape_type::cylinder:
            geometry_factory::cylinder(static_cast<uint32_t>(_cyl_params.ring_tess_factor),
                                       static_cast<uint32_t>(_cyl_params.rings),
                                       _cyl_params.height,
                                       _cyl_params.radius,
                                       &mesh);
            break;
        ...
        case shape_type::sphere:
            geometry_factory::geosphere(
                _sphere_params.radius, static_cast<int32_t>(_sphere_params.subdivisions), &mesh);
            break;
    ...
    if (size_vertexbuffer <= _bytes_vertexbuffer) {
        ...
        gl::NamedBufferSubData(raw_handle(_vertex_buffer), 0, size_vertexbuffer, &vertices[0]);
    } else {
        //
        // reallocate buffer data store
        gl::NamedBufferData(raw_handle(_vertex_buffer), size_vertexbuffer, &vertices[0], gl::DYNAMIC_DRAW);
    ...
    if (size_indexbuffer <= _bytes_indexbuffer) {
        //
        // reuse data store
        gl::NamedBufferSubData(raw_handle(_index_buffer), 0, size_indexbuffer, raw_ptr(mesh.indices));
    } else {
        //
        // reallocate data store
        gl::NamedBufferData(raw_handle(_index_buffer), size_indexbuffer, raw_ptr(mesh.indices), gl::DYNAMIC_DRAW);
...
void
app::textures_demo::shape_specific_ui()
{
    ...
    if (stype == shape_type::cylinder) {
        ...
        update_needed |= ImGui::SliderInt("Horizontal tesselation", &_cyl_params.ring_tess_factor, 3, 128);
        update_needed |= ImGui::SliderInt("Vertical tesselation", &_cyl_params.rings, 3, 128);
        update_needed |= ImGui::SliderFloat("Height", &_cyl_params.height, 0.2f, 4.0f, "%3.3f");
        update_needed |= ImGui::SliderFloat("Radius", &_cyl_params.radius, 0.2f, 5.0f, "%3.3f");
    } else if (stype == shape_type::sphere) {
        ImGui::Separator();

        update_needed |= ImGui::SliderInt("Subdivisions", &_sphere_params.subdivisions, 1, 5);
        update_needed |= ImGui::SliderFloat("Radius", &_sphere_params.radius, 0.1f, 5.0f, "%3.3f");
    }
