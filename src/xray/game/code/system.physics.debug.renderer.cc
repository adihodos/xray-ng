...
PhysicsEngineDebugRenderer::PhysicsEngineDebugRenderer(
	PhysDebugRenderResourcesBundle res_lines,
...
void PhysicsEngineDebugRenderer::DrawLine(JPH::RVec3Arg inFrom, JPH::RVec3Arg inTo, JPH::ColorArg inColor) {
	XR_LOG_INFO("Draw line");
	std::unique_lock lock{_lines_lock};
	if (_lines_count + 2 >= MAX_LINES) return;
...
void PhysicsEngineDebugRenderer::DrawTriangle(
	...
	if (_yftriangles + 3 >= MAX_YF_TRIANGLES) return;
...
void PhysicsEngineDebugRenderer::DrawText3D(
	JPH::RVec3Arg inPosition, const std::string_view& inString, JPH::ColorArg inColor, float inHeight
) {
	XR_LOG_INFO("{} is NOT IMPLEMENTED YET!", XRAY_FUNCTION_NAME);
}

PhysicsEngineDebugRenderer::Batch PhysicsEngineDebugRenderer::CreateTriangleBatch(
...
void PhysicsEngineDebugRenderer::DrawGeometry(
	...
	auto push_triangle_batch_fn =
		[batch,
		 &inModelColor,
		 &inModelMatrix](std::mutex& triangle_lock, size_t& triangle_count, xray::base::MemoryArena& triangle_arena) {
...
xray::base::unique_pointer<PhysicsEngineDebugRenderer> PhysicsEngineDebugRenderer::create(const InitContext& ctx) {
	...
	const size_t block_size			  = round_up(block_size_lines + block_size_triangles * 2, os_get_page_size());
	XR_LOG_INFO("Allocating {} Kb/{} Mb for lines and triangles", block_size / 1024, block_size / (1024 * 1024));

	std::span<std::byte> memory_block{static_cast<std::byte*>(os_alloc_mem(block_size)), block_size};
	if (memory_block.empty()) return nullptr;

	auto gpu_lines = VulkanBuffer::create(
		*ctx.renderer,
	...
	if (!gpu_lines) return nullptr;

	auto gpu_yftris = VulkanBuffer::create(
		*ctx.renderer,
		VulkanBufferCreateInfo{
	...
	auto gpu_filled = VulkanBuffer::create(
		*ctx.renderer,
		VulkanBufferCreateInfo{
	...
	auto [primitive_align, primitive_size] = []() constexpr -> std::pair<size_t, size_t> {
		if constexpr (std::is_same_v<PrimitiveType, PhysicsEngineDebugRenderer::Line>) {
			return std::pair<size_t, size_t>{
				alignof(PhysicsEngineDebugRenderer::Line), sizeof(PhysicsEngineDebugRenderer::Line)
			};
...
void PhysicsEngineDebugRenderer::draw(const RenderEvent& e) {
	using namespace xray::rendering;
