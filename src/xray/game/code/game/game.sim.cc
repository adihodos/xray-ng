...
#if defined(XR_ENABLE_PROFILING)
...
#include "xray/game/code/init_context.hpp"
...
#include "xray/game/code/bindless.pipeline.config.hpp"
#include "xray/game/code/events.hpp"
#include "xray/game/code/system.physics.hpp"
#include "xray/game/code/push.constant.packer.hpp"
#include "xray/game/code/terrain.hpp"
#include "xray/game/code/sprite.ids.hud.symbology.hpp"
#include "xray/game/code/hud.config.hpp"
...
B5::GameSimulation::SimState::SimState(const InitContext& init_context)
	: arcball_cam{xray::math::vec3f::stdc::zero, 1.0f, {init_context.surface_width, init_context.surface_height}},
	  flightcam{
		  .params = FlightCameraParams::from_file(
			  *init_context.perm,
			  xray::base::string_view_from_c_str(
				  init_context.config_sys->config_path("flight.camera.params.conf").c_str()
			  )
		  ),
	  } {
...
B5::GameSimulation::InputStateTracker::InputStateTracker(
	...
	last_axis_events.reserve(static_cast<U8>(xray::ui::GamepadAxis::Count));
	for (U8 i = static_cast<U8>(GamepadAxis::First); i < static_cast<U8>(GamepadAxis::Count); ++i) {
		last_axis_events.push_back(
			GamepadAxisEvent{.axis = static_cast<GamepadAxis>(i), .i32 = 0, .f32 = 0.0f, .timestamp = 0}
		);
	}
...
xray::base::unique_pointer<B5::GameSimulation> B5::GameSimulation::create(const InitContext& init_ctx) {
	...
	std::span<std::byte> arena_temp = std::span{static_cast<std::byte*>(os_alloc_mem(kTempArenaSize)), kTempArenaSize};
...
void B5::GameSimulation::update_ship_data() {
	...
	sd->compass_bearing = std::fmod(450.0f - sd->yaw_angle * F32Consts::OneEightyOverPi, 360.0f);
}
...
void B5::GameSimulation::loop_event(const RenderEvent& render_event) {
...

	{
		...
		auto copy_lights_to_gpu_fn = [&]<typename LightType, typename LightFN>(
			...
			[[maybe_unused]] const auto maybe_err =
...
vec3f angles_from_rotation(const JPH::Quat& rotation) {
	...
	if (ship_up.GetY() < 0.0f) {
		roll = std::copysign(F32Consts::Pi, roll) - roll;
	}
