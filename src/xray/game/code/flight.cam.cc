...
FlightCameraParams FlightCameraParams::from_file(
	xray::base::MemoryArena& arena, const xray::base::xrStringView_t file_path
) {
	FlightCameraParams loaded_params{};
	if (!xray::base::deserialize_from_file(arena, loaded_params, file_path)) {
		XR_LOG_INFO("Failed to load camera parameters: file: %" PRI_xrStringView_t, FMT_xrStringView_t(file_path));
	}

	return loaded_params;
}

xray::math::MatrixWithInvertedMatrixPair4f FlightCamera::update(const JPH::Body& target) noexcept {
	...
	const JPH::Quat rotation_from_input{
		JPH::Quat::sEulerAngles(JPH::Vec3{radians(-look_average_angle.y), radians(look_average_angle.x), 0.0f})
	};
	...
	if (params.fixed_cam && pivot_mode) {
		...
		cam_pos =
			ship_position + cam_rot * JPH::Vec3{params.camera_offset.x, params.camera_offset.y, params.camera_offset.z};
	} else {
		...
		const JPH::Quat rotation_from_angular_vel{JPH::Quat::sEulerAngles(
			JPH::Vec3{
				radians(-angular_velocity_average.x),
