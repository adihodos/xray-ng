...
#endif
  _collector_thread = tbb::tbb_thread{[this]() { this->run(); }};
}
...
void xray::base::stats_thread::run() {
  ...
  static constexpr const char* const PERF_COUNTER_NAMES[] = {
      "% Processor Time", "Working Set",        "Thread Count",
      "Virtual Bytes",    "Virtual Bytes Peak", "Working Set Peak"};

  {
    ...
    for (uint32_t i = 0; i < counter_type::last; ++i) {
      ...
      const auto result =
          PdhAddCounter(raw_handle(_proc_stats.query), counter_path_buff, 0,
  ...
  PdhCollectQueryDataEx(raw_handle(_proc_stats.query), 1,
      ...
      for (uint32_t counter_idx = 0; counter_idx < counter_type::last;
        ...
        const auto           result =
            PdhGetFormattedCounterValue(_proc_stats.counters[counter_idx],
                                        PDH_FMT_DOUBLE, &type, &counter_val);
...
xray::base::stats_thread::process_stats() const noexcept {
  ...
  psi.vbytes_peak = static_cast<uint32_t>(
      _proc_stats.values[counter_type::virtual_bytes_peak]);
  psi.virtual_bytes =
