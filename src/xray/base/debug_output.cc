...
static void output_dbg_string_impl(
    const char* fmt_string, va_list args_ptr)
{
    const auto req_chars_no_null_term = vsnprintf(
        nullptr, 0, fmt_string, args_ptr);
...
void xray::base::output_debug_string(
    const char* format, ...)
{
    fputc('\n', stderr);
