...
FileSystemWatcher::~FileSystemWatcher() {
	...
	for (const auto& [file_path, watch_fd] : _fs_watched_paths) {
		syscall_wrapper(inotify_rm_watch, raw_handle(_inotify_handle), watch_fd);
	}
#endif
}
...
void FileSystemWatcher::add_watch(
	const std::filesystem::path& path, FileSystemObserverDelegate observer, const bool recursive
) {
#if defined(XRAY_OS_IS_POSIX_FAMILY)
	...
	for (const fs::directory_entry& dir_entry : fs::recursive_directory_iterator(path)) {
		if (!fs::is_directory(dir_entry.path())) continue;
