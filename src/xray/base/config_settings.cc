...
int32_t
xray::base::detail::setting_type_to_library_type(const xray::base::config_entry_type st) noexcept
{
    switch (st) {
...
std::string
xray::base::config_file::error() const noexcept
{
    ...
    return fmt::format(
        "Config parse error {}:{}:{}", config_error_text(&conf_), config_error_file(&conf_), config_error_line(&conf_));
}
