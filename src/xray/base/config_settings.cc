...
int32_t xray::base::detail::setting_type_to_library_type(const xray::base::config_entry_type st) noexcept {
	switch (st) {
...
xray::base::config_entry_type xray::base::detail::library_type_to_setting_type(const int32_t lt) noexcept {
	switch (lt) {
...
std::string xray::base::config_file::error() const noexcept {
	...
	if (err_type == CONFIG_ERR_NONE) return "No error";

	if (err_type == CONFIG_ERR_FILE_IO) return "File IO error";

	char tmp_buff[1024];
	format_to_n(
		tmp_buff,
		"Config parse error %s:%s:%d",
		config_error_text(&conf_),
		config_error_file(&conf_),
