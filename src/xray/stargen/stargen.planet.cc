...
constexpr auto UNDEFINED_TILT = 360.0;
...
SG_Planet::SG_Planet(SG_Star* sun, int planet_no, long double a, long double e, long double mass) {
	...
	mAtmosphere		 = std::make_shared<SG_Atmosphere>(this);
	mHigh_temp		 = 0;
	mLow_temp		 = 0;
	mMax_temp		 = 0;
	mMin_temp		 = 0;
...
long double SG_Planet::estimateDensity(long double orbit_radius, long double r_ecosphere, bool gas_giant) {
	long double temp;
	temp = pow1_8(to_em(mMass)) * pow1_4(r_ecosphere / orbit_radius);
...
long double SG_Planet::calculateDayLength() {
	...
	change_in_angular_velocity =
		CHANGE_IN_EARTH_ANG_VEL * (mDensity / EARTH_DENSITY) * (equatorial_radius_in_cm / EARTH_RADIUS) *
		(EARTH_MASS_IN_GRAMS / planetary_mass_in_grams) * pow2(mPrimaryStar->mMass) * (1.0 / pow(m_a, 6.0));
...
*/
int SG_Planet::calculateInclination(long double orb_radius, StargenHelper& helper) {
	...
	Inclination *= helper.about(EARTH_AXIAL_TILT, 0.4);
...
long double SG_Planet::calculateCloudFraction(long double smallest_MW_retained, long double hydro_fraction) {
	...
	if (smallest_MW_retained > WATER_VAPOR)
		...
		surf_area		  = 4.0 * std::numbers::pi * pow2(mRadius);
...
long double SG_Planet::calculateIceFraction(long double hydro_fraction) {
	...
	surf_temp = std::min<double>(328.0, mSurf_temp);
...
@sa calculateSurfaceTemperature
*/
long double SG_Planet::calculateAlbedo(
	long double water_fraction, long double cloud_fraction, long double ice_fraction
) {
...
void SG_Planet::calculateSurfaceClimate(
	...
	if (mGreenhouse_effect && (mMax_temp < mBoiling_point)) {
		...
		mVolatile_gas_inventory = this->calculateVolatilGasRatio(
			mEsc_velocity, mPrimaryStar->mMass, mGreenhouse_effect, (mGas_mass / mMass) > 0.000001
		);
	...
	if ((mGreenhouse_effect) && (mSurf_pressure > 0.0)) mCloud_cover = 1.0;
...
void SG_Planet::calculateGlobalClimate() {
	...
	*/
	...
	for (int count = 0; count <= 25; count++) {
		...
		if (fabs(mSurf_temp - last_temp) < 0.25) break;
	}
...
void SG_Planet::calculateTemperatureRange() {
	...
	long double tiltmod	 = fabs(cos(mAxial_tilt * std::numbers::pi / 180.0) * pow2(1 + m_e));
...
long double SG_Planet::calculateOrbitPeriod(long double star_mass) {
	const long double period_in_years = sqrt(pow3(m_a) / (mMass + star_mass));
...
long double SG_Planet::getSphereDensity(long double mass, long double radius) {
	const long double mass_grams = mass * SOLAR_MASS_IN_GRAMS;
	const long double radius_cm	 = radius * CM_PER_KM;
	const long double volume	 = (4.0 / 3.0) * std::numbers::pi * pow3(radius_cm);
...
long double SG_Planet::getSphereRadius(long double mass, long double density) {
	const long double mass_grams = mass * SOLAR_MASS_IN_GRAMS;
	const long double volume	 = mass_grams / density;
	const long double radius	 = pow1_3((3 * volume) / (4 * std::numbers::pi)) / CM_PER_KM;
	return (radius);
}
...
long double SG_Planet::getSphereMass(long double density, long double radius) {
	const long double radius_cm = radius * CM_PER_KM;
	const long double volume	= (4.0 / 3.0) * std::numbers::pi * pow3(radius_cm);
	const long double mass		= volume * density / SOLAR_MASS_IN_GRAMS;
	return (mass);
}
...
bool SG_Planet::calculateMetal() {
	if ((mDensity > 4) && (mMolec_weight < 250)) return true;
	return false;
}
...
void SG_Planet::calculatePlanet(StargenHelper& helper) {
	mEarthlike = false;

	// SG_Utils::writeLog("Planet No " + std::to_string(mPlanet_no));
	// SG_Utils::writeLog("Orbit    = " + std::to_string(m_a) + "AU");

	if (mGas_giant) this->calculateGasPlanet(helper);
	if (!mGas_giant) this->calculateSolidPlanet(helper);
	...
	if ((mSurf_grav >= 0.8) &&		 // between 0.8 and 1.2 g
		...
		(!mFaceLocked) && (!mResonant_period) &&
		(mBreathe == AtmosphereType::Breathable))  // and, of course, breathable
	{
		mEarthlike = true;
	}
...
*/
void SG_Planet::calculateGasPlanet(StargenHelper& helper) {
	...
	mGreenhouse_rise		= 0;
	mAlbedo					= helper.about(GAS_GIANT_ALBEDO, 0.1);
	...
	mAxial_tilt		 = this->calculateInclination(m_a, helper);
	...
	/************************************************************************/
	if ((to_em(mMass) > 1) && (mGas_mass > 0.05 * mMass) && (mMolec_weight <= HELIUM)) {
		mGas_planet = true;
	} else {
...
they are using values calculated by the calls of previous functions.
*/
void SG_Planet::calculateSolidPlanet(StargenHelper& helper) {
	/************************************************************************/
	/* Calculation of physical characteristics                              */
	/************************************************************************/
	mOrbit_zone		 = this->getOrbitalZone(mPrimaryStar->mLum, m_a);
	...
	/* Calculation of the atmosphere's composition                      */
	/********************************************************************/
	// removed by DDL to calculate all atmospheres: if ((mMax_temp >= FREEZING_POINT_OF_WATER) && (mMin_temp <=
	// mBoiling_point))
