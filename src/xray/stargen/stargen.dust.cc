#include "xray/stargen/stargen.dust.hpp"
...
SG_Stardust::SG_Stardust(SG_Star* star, int max_planet) {
	...
	for (int index = 0; index < mMaxPlanet; index++) mPlanetList[index] = nullptr;
...
*/
long double SG_Stardust::getInnerEffectLimit(const protoPlanet* protoplanet) const noexcept {
	const long double a = protoplanet->orbit;
	const long double e = protoplanet->eccnt;
	const long double m = protoplanet->reducedmass;

	const long double R = std::max<long double>(0.0, a * (1 - e) * (1 - m) / (1.0 + mCloudEccentricity));
	return R;
}
...
*/
long double SG_Stardust::getOuterEffectLimit(const protoPlanet* protoplanet) const noexcept {
	const long double a = protoplanet->orbit;
	const long double e = protoplanet->eccnt;
	const long double m = protoplanet->reducedmass;
...
*/
bool SG_Stardust::isDustAvailable(long double inside_range, long double outside_range) const noexcept {
	dust_band* current_dust_band = mDust_head;
	bool dust_here				 = false;

	current_dust_band = mDust_head;
	while ((current_dust_band != nullptr) && (current_dust_band->outer_edge < inside_range))
	...
	while ((current_dust_band != nullptr) && (current_dust_band->inner_edge < outside_range)) {
...
@param protoplanet The protoplanet. The protoplanet has a band limited by its innerEffect and outerEffect, called the
"protoplanet'band".
*/
/* ------------------------------------------------------------------------- */
void SG_Stardust::updateDustLanes(protoPlanet* protoplanet) {
	...
	bool gasLeft = (protoplanet->mass < protoplanet->criticalmass);
	mDust_left	 = false;
	...
	while ((node1 != nullptr)) {
		...
		if ((node1->inner_edge < protoplanet->innereffect) && (node1->outer_edge > protoplanet->outereffect)) {
		...
		else if ((node1->inner_edge < protoplanet->outereffect) && (node1->outer_edge > protoplanet->outereffect)) {
			// Creation on a new dustband after the protoplanet path
			node2				= new dust_band;
		...
		else if ((node1->inner_edge < protoplanet->innereffect) && (node1->outer_edge > protoplanet->innereffect)) {
			// Creation on a new dustband on the protoplanet path
			node2				= new dust_band;
			node2->next_band	= node1->next_band;
			node2->dust_present = false;
		...
		else if ((node1->inner_edge >= protoplanet->innereffect) && (node1->outer_edge <= protoplanet->outereffect)) {
			// Adjust the existing band (on the protoplanet path)
			if (node1->gas_present) node1->gas_present = gasLeft;
			node1->dust_present = false;
			// Next dustband to analyse
		...
		else if ((node1->outer_edge < protoplanet->innereffect) || (node1->inner_edge > protoplanet->outereffect))
			// Next dustband to analyse
			node1 = node1->next_band;
	}
	...
	dust_band* nextBand = nullptr;
	while ((currBand != nullptr)) {
		...
		if ((currBand->dust_present) && (currBand->outer_edge >= mPlanet_inner_bound) &&
...
void SG_Stardust::collectDust(protoPlanet* protoplanet, dust_band* dustband) {
	...
	if ((Inner_effect < dustband->outer_edge) && (dustband->inner_edge < Outer_effect)) {
		...
		volume =
			4 * std::numbers::pi * pow2(protoplanet->orbit) * (1.0 - protoplanet->eccnt * (temp1 - temp2) / bandwidth);
...
*/
long double SG_Stardust::getCriticalMass(const protoPlanet* protoplanet) const noexcept {
	const long double perihelion_dist = protoplanet->orbit * (1 - protoplanet->eccnt);
	const long double temp			  = perihelion_dist * std::sqrt(mSun->mLum);
...
void SG_Stardust::generatePlanets(StargenHelper& helper) {
	...
	while ((mDust_left) && (mPlanetIndex < mMaxPlanet)) {
		...
		//
		// TODO : useless heap alloc here ?!
		protoPlanet* proto	 = new protoPlanet;
		proto->orbit		 = mSun->getBodePlanetOrbit(BodeIndex++, helper);
		proto->eccnt		 = helper.random_eccentricity();
...
void SG_Stardust::sortPlanetTable() {
	for (int i = 1; i < mMaxPlanet; i++) {
		...
		const auto pc = mPlanetList[i]->getOrbit();
