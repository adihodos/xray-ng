...
*/
long double SG_Gas::getBoilingPoint(long double pressure) const noexcept {
...
*/
long double SG_Gas::getMeltingPoint(long double pressure) const noexcept {
...
*/
GasType SG_Gas::getState(long double temperature, long double pressure) const noexcept {
	long double BP = this->getBoilingPoint(pressure);
	long double MP = this->getMeltingPoint(pressure);

	if (temperature < MP) return GasType::Solid;
	if (temperature < BP) return GasType::Liquid;
	return GasType::Gas;
}

/// This function returns the aboundance of the Gas in the Sun.
/** @return the solar aboundance of the Gas. */
long double SG_Gas::getAbound() const noexcept { return mAbunds; }
...
@sa getPres2
*/
long double SG_Gas::getReact(long double temperature, long double pressure, long double years) const noexcept {
	...
	// Carbon Dioxid
		...
		return (1.5 * pow(1 / (1 + mReactivity), std::sqrt(bil_years / 2.0) * pres2));
	}

	// Other gases
	else {
		return std::pow(1 / (1 + mReactivity), bil_years / 2.0 * pres2);
	}
}
...
@sa getReact
*/
long double SG_Gas::getPres2(long double temperature, long double pressure, long double years) const noexcept {
	long double bil_years = years * 1e-9;
	long double pressureB = pressure / MILLIBARS_PER_BAR;  // unit = bar
	long double pres2;
...
*/
long double SG_Gas::getRMSvelocity(long double temperature) const noexcept {
...
*/
long double SG_Gas::getPVRMS(long double temperature, long double Escape_velocity, long double years) const noexcept {
	long double bil_years = years * 1e-9;
	long double vrms	  = this->getRMSvelocity(temperature);
	return std::pow(1 / (1 + vrms / Escape_velocity), bil_years);
}
...
*/
long double SG_Gas::getFract(long double Molec_weight) const noexcept { return (1 - (Molec_weight / mWeight)); }

/// This function returns the atomic weight for the gas molecule.
long double SG_Gas::getWeight() const noexcept { return (mWeight); }

/// This function returns the atomic number (AN) of the gas molecule.
int SG_Gas::getAtomicNumber() const noexcept { return mNum; }

/// Set the amount of the gas in the atmosphere.
void SG_Gas::setAmount(long double amount) { mAmount = amount; }

/// Get the amount of the gas in the atmosphere.
long double SG_Gas::getAmount() const noexcept { return mAmount; }

/// This function return the chemical symbol of the gas.
std::string_view SG_Gas::getSymbol() const noexcept { return mSymbol; }

/// This function indicates if the element is radioactive or not.
bool SG_Gas::isRadioactive() const noexcept { return (mWeight >= 84); }

/// This function indicates if the gas a reached a toxical concentration in the atmosphere.
bool SG_Gas::isToxic(long double surf_pressure) const noexcept {
	bool toxicity;
...
std::string_view SG_Gas::getName() const noexcept { return mName; }
 ...
 */
long double SG_Gas::getMaxIPP() const noexcept { return mMax_ipp; }
...
*/
long double SG_Gas::getInspiredPartialPressure(long double surf_pressure) const noexcept {
...
void SG_Gas::setPartialPressure(long double Partial_pressure) { mPartialPressure = Partial_pressure; }

/// This function returns the Partial Pressure of the gas in the atmosphere.
long double SG_Gas::getPartialPressure() const noexcept { return mPartialPressure; }

/// This function set the percentage of the gas in the atmosphere.
/** @param pourcentage The percentage should be a value in [0..1].          */
void SG_Gas::setPartialPercentage(long double pourcentage) { mPartialPercentage = pourcentage; }

/// This function returns the percentage of the gas in the atmosphere.
/** @return The percentage is a value in [0..1].                            */
long double SG_Gas::getPartialPercentage() const noexcept { return mPartialPercentage; }
...
**/
SG_Gas::SG_colour SG_Gas::getGasColour() const noexcept { return mGasColour; }
