...
/* ------------------------------------------------------------------------- */
void SG_Star::setRandomStar(StargenHelper& helper) {
	mMass		 = helper.random_number(0.7, 7);
	mLum		 = this->calculateLuminosity(mMass);
	mR_ecosphere = this->calculateEcosphere(mLum);
	mLife		 = this->calculateLife();
	mAge		 = helper.random_number(1, 6) * 1e9;  // age of the star: 1 to 6 billions years
...
*/
/* ------------------------------------------------------------------------- */
void SG_Star::setEcosphere(long double ecosphere) { mR_ecosphere = ecosphere; }
...
*/
/* ------------------------------------------------------------------------- */
void SG_Star::setLife(long double life) { mLife = life; }
...
*/
/* ------------------------------------------------------------------------- */
long double SG_Star::calculateEcosphere(long double luminosity) { return (sqrt(luminosity)); }
...
*/
/* ------------------------------------------------------------------------- */
long double SG_Star::calculateLife() { return ((mMass / mLum) * 10e9); }
...
long double SG_Star::getBodyTemperature(long double orbit_radius, long double albedo) {
	return (sqrt(mR_ecosphere / orbit_radius) * pow1_4((1.0 - albedo) / (1.0 - EARTH_ALBEDO)) * EARTH_AVERAGE_KELVIN);
}
...
long double SG_Star::getEffectiveTemperature(long double orbit_radius, long double albedo) {
	return (sqrt(mR_ecosphere / orbit_radius) * pow1_4((1.0 - albedo) / (1.0 - EARTH_ALBEDO)) * EARTH_EFFECTIVE_TEMP);
}
...
/** @return unit = UA */
/* ------------------------------------------------------------------------- */
long double SG_Star::getNearestPlanetOrbit() { return (0.3 * pow1_3(mMass)); }

/* ------------------------------------------------------------------------- */
/// Renvoie l'orbite maximale où peut se former une planete.
/** @return unit = UA */
/* ------------------------------------------------------------------------- */
long double SG_Star::getFarthestPlanetOrbit() { return (50.0 * pow1_3(mMass)); }
...
/* ------------------------------------------------------------------------- */
long double SG_Star::getStellarDustLimit() { return (200.0 * pow1_3(mMass)); }
...
*/
/* ------------------------------------------------------------------------- */
long double SG_Star::getBodePlanetOrbit(int index, StargenHelper& helper) {
	long double orbit;

	if (index < -1) {
		// If index is a negative value, the function returns a random orbit.
		orbit = helper.random_number(getNearestPlanetOrbit(), getFarthestPlanetOrbit());
	} else if (index == -1) {
