...
#include "xray/stargen/stargen.planet.hpp"
#include "xray/stargen/stargen.star.hpp"
...
*/
SG_Atmosphere::SG_Atmosphere(SG_Planet* myPlanet) : mPlanet(myPlanet) {
	/// At-Nb Symbol At-Weight Melt Boil   Density   Abound-S  Rea Max-IPP
	mGasList.push_back(SG_Gas(AN_H, "H", 1.0079, 14.06, 20.40, 8.99e-05, 27925.4000 /*91.00%*/, 1, 0, "Hydrogen"));
	//		(AN_H, "H2", 2.0158, 14.06, 20.40, 8.99e-05, 27925.4000 /*91.00%*/, 1, 0, "Hydrogen");
	mGasList.push_back(
		SG_Gas(AN_HE, "He", 4.0026, 3.46, 4.20, 0.000178, 2722.70000 /*7.100%*/, 0, 61000 * MMHG_TO_PPM, "Helium")
	);
	mGasList.push_back(SG_Gas(
	...
	mGasList.push_back(
		SG_Gas(AN_O, "O", 16, 54.80, 90.20, 0.001429, 23.8232000 /*0.050%*/, 10, 400 * MMHG_TO_PPM, "Oxygen")
	);
	//		(AN_O, "O2",32,      54.80, 90.20, 0.001429, 23.8232000 /*0.050%*/, 10, 400*MMHG_TO_PPM,"Oxygen");
	mGasList.push_back(SG_Gas(AN_O3, "O3", 48, 80.16, 161.16, 0.001000, 0.00000100, 2 /*0.001*/, 0.1, "Ozone"));
	mGasList.push_back(SG_Gas(AN_NE, "Ne", 20.17, 24.53, 27.10, 0.000900, 3.44350e-5, 0, 3900 * MMHG_TO_PPM, "Neon"));
	mGasList.push_back(SG_Gas(AN_AR, "Ar", 39.948, 84.00, 87.30, 0.001782, 0.10092500, 0, 1220 * MMHG_TO_PPM, "Argon"));
	mGasList.push_back(SG_Gas(AN_KR, "Kr", 83.8, 116.60, 119.70, 0.003708, 4.4978e-05, 0, 350 * MMHG_TO_PPM, "Krypton")
	);
	mGasList.push_back(SG_Gas(AN_XE, "Xe", 131.3, 161.30, 165.00, 0.005880, 4.6989e-06, 0, 160 * MMHG_TO_PPM, "Xenon"));
	mGasList.push_back(SG_Gas(AN_NH3, "NH3", 17, 195.46, 239.66, 0.001000, 0.00010000, 1 /*0.001*/, 100, "Ammonia"));
	mGasList.push_back(SG_Gas(AN_H2O, "H2O", 18, 273.16, 373.16, 1.000000, 0.00100000, 0, 0, "Water"));
	mGasList.push_back(
		SG_Gas(AN_CO2, "CO2", 44, 194.66, 194.66, 0.001000, 0.00400000, 0, 7 * MMHG_TO_PPM, "CarbonDiOxide")
	);
	mGasList.push_back(SG_Gas(AN_CH4, "CH4", 16, 90.16, 109.16, 0.010000, 0.00010000, 1 /*0*/, 5000, "Methane"));

	// Additional elements
	mGasList.push_back(SG_Gas(AN_F, "F", 18.99, 53.58, 85.10, 0.001696, 0.00084333, 50, 0.1, "Fluor"));
	mGasList.push_back(SG_Gas(AN_CL, "Cl", 35.45, 172.22, 239.20, 0.003214, 0.00523600, 40, 1, "Chlore"));
	mGasList.push_back(SG_Gas(AN_C, "C", 12.01, 3820.0, 5100.0, 2.260000, 6.89323800 /*0.033%*/, 0, 0, "Carbon"));
	mGasList.push_back(SG_Gas(AN_S, "S", 32.06, 386.0, 717.8, 2.070000, 0.42000000 /*0.002%*/, 0, 2, "Sulfur"));
	mGasList.push_back(SG_Gas(AN_SO2, "SO2", 64.06, 197.0, 262.9, 3.049000, 0.00020000, 4, 2, "Sulfur Dioxid")
	); /* abound uncertain */
	mGasList.push_back(SG_Gas(AN_CO, "CO", 28.01, 0.0, 81.4, 0.968000, 0.00001000, 0, 25, "Carbon Monoxid"));

	/// Metals (here or not ?) - TODO
	mGasList.push_back(SG_Gas(AN_FE, "Fe", 55.85, 1809.0, 3023.0, 7.800000, 0.63000000 /*0.003%*/, 0, 0.0, "Iron"));
	mGasList.push_back(SG_Gas(AN_CU, "Cu", 63.54, 1356.6, 2840.0, 8.960000, 0.00001000, 0, 0, "Copper"));
	mGasList.push_back(SG_Gas(AN_NI, "Ni", 58.69, 1726.0, 2647.0, 8.900000, 0.00001000, 0, 0, "Nickel"));
	...
	mTotalIPP = lz::accumulate(
		mGasList | lz::filter([](const SG_Gas& gas) { return gas.getWeight() >= AN_N && gas.getMaxIPP() < 1e9; }),
		0.0,
		[](const double total_ipp, const SG_Gas& gas) { return total_ipp + gas.getMaxIPP(); }
	);
...
int SG_Atmosphere::estimateComposition() {
	...
	for (SG_Gas& gas : mGasList) {
		partial_amount = this->getGasAmount(&gas);
...
*/
AtmosphereType SG_Atmosphere::getBreathability() {
	SG_Planet* myPlanet = static_cast<SG_Planet*>(mPlanet);

	if (mGases == 0) return AtmosphereType::None;
...
*/
std::string SG_Atmosphere::getGasName(int rank, const uint32_t stateMask) {
	if (rank >= 0 && rank < static_cast<int>(mGasList.size())) {
		const GasType state = mGasList[rank].getState(mPlanet->mSurf_temp, mPlanet->mSurf_pressure);
		if (static_cast<uint32_t>(state) & stateMask) {
			return std::string{mGasList[rank].getName()};
		}
	}
...
*/
std::string SG_Atmosphere::getGasSymbol(int rank, const uint32_t stateMask) {
	if (rank >= 0 && rank < static_cast<int>(mGasList.size())) {
		const GasType state = mGasList[rank].getState(mPlanet->mSurf_temp, mPlanet->mSurf_pressure);
		if (static_cast<uint32_t>(state) & stateMask) {
			return std::string{mGasList[rank].getSymbol()};
		}
	}
	return std::string{};
}
...
*/
GasType SG_Atmosphere::getGasState(int rank) {
	if (rank >= 0 && rank < static_cast<int>(mGasList.size())) {
		return mGasList[rank].getState(mPlanet->mSurf_temp, mPlanet->mSurf_pressure);
	}
...
long double SG_Atmosphere::getGasRatio(int rank) {
	if (rank >= 0 && rank < static_cast<int>(mGasList.size())) {
		return mGasList[rank].getPartialPercentage();
	}
...
*/
SG_Gas* SG_Atmosphere::getGas(int rank, const uint32_t stateMask) {
	if (rank >= 0 && rank < static_cast<int>(mGasList.size())) {
		const GasType state = mGasList[rank].getState(mPlanet->mSurf_temp, mPlanet->mSurf_pressure);
		if (static_cast<uint32_t>(state) & stateMask) {
...
void SG_Atmosphere::sortGasTable() {
	std::sort(mGasList.begin(), mGasList.end(), [](const SG_Gas& g0, const SG_Gas& g1) {
		return g0.getPartialPercentage() > g1.getPartialPercentage();
	});
}
...
@sa getBreathability
*/
std::string SG_Atmosphere::getToxicGasList() { return mPoisonedBy; }
