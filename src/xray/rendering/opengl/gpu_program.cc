...
void xray::rendering::gpu_program::bind_to_pipeline() {
  ...
  for_each(begin(uniform_blocks_), end(uniform_blocks_), [this](auto& u_blk) {
    if (u_blk.dirty) {
      ...
      {
        scoped_resource_mapping ubuff_mapping{
          raw_handle(u_blk.gl_buff), gl::MAP_WRITE_BIT, u_blk.size};
  ...
  {
    auto stages =
      gsl::span<pipeline_stage_subroutine_uniform_data>{stage_subroutine_ufs_};
    ...
    for (uint8_t stage      = static_cast<uint8_t>(pipeline_stage::vertex),
      ...
      gl::UniformSubroutinesuiv(
        ...
        static_cast<GLsizei>(indices_buff.size()),
        indices_buff.data());
    }
  }
}
...
bool xray::rendering::gpu_program::collect_uniform_blocks() {
  ...
  gl::GetProgramInterfaceiv(
    phandle, gl::UNIFORM_BLOCK, gl::ACTIVE_RESOURCES, &num_uniform_blocks);

  GLint max_name_len{};
  gl::GetProgramInterfaceiv(
    phandle, gl::UNIFORM_BLOCK, gl::MAX_NAME_LENGTH, &max_name_len);
  ...
  for (uint32_t blk_idx = 0;
    ...
    gl::GetProgramResourceName(phandle,
                               gl::UNIFORM_BLOCK,
                               blk_idx,
                               static_cast<GLsizei>(txt_buff.size()),
                               nullptr,
                               txt_buff.data());
  ...
  // compute number of required bytes for blocks and allocate storage.
  {
    ...
    for_each(begin(ublocks), end(ublocks), [kBufferCreateFlags](auto& blk) {
      ...
      gl::NamedBufferStorage(
        raw_handle(blk.gl_buff), blk.size, nullptr, kBufferCreateFlags);
    });

    //
    // abort if creation of any buffer failed.
    const auto any_fails = any_of(begin(ublocks),
                                  end(ublocks),
...
bool xray::rendering::gpu_program::collect_uniforms() {
  ...
  GLint num_uniforms{};
  gl::GetProgramInterfaceiv(
    phandle, gl::UNIFORM, gl::ACTIVE_RESOURCES, &num_uniforms);
  ...
  GLint u_max_len{};
  gl::GetProgramInterfaceiv(
    phandle, gl::UNIFORM, gl::MAX_NAME_LENGTH, &u_max_len);
  ...
  for (std::uint32_t u_idx = 0; u_idx < static_cast<uint32_t>(num_uniforms);
    ...
    gl::GetProgramResourceName(phandle,
                               gl::UNIFORM,
                               u_idx,
                               static_cast<GLsizei>(tmp_buff.size()),
                               nullptr,
                               tmp_buff.data());
    ...
    const GLuint props_to_get[] = {gl::BLOCK_INDEX,
                                   gl::OFFSET,
                                   gl::TYPE,
                                   gl::ARRAY_SIZE,
                                   gl::LOCATION,
  ...
  sort(begin(uniforms_), end(uniforms_), [](const auto& u0, const auto& u1) {
    return u0.name < u1.name;
  });
...
bool xray::rendering::gpu_program::collect_subroutines_and_uniforms() {
  ...
  for (const auto sub_uf_ifc : SUBROUTINE_UNIFORM_INTERFACES_IDS) {
    ...
    gl::GetProgramInterfaceiv(
      phandle, sub_uf_ifc, gl::ACTIVE_RESOURCES, &active_uniforms);
    ...
    gl::GetProgramInterfaceiv(
      phandle, sub_uf_ifc, gl::MAX_NAME_LENGTH, &max_name_length);
  ...
  sort(begin(subroutine_uniforms_),
  ...
  // Fill per stage subroutine uniform data.
  {
    ...
    for (size_t idx = 0, uniform_count = subroutine_uniforms_.size();
      ...
      gl::GetProgramStageiv(phandle,
                            pipeline_stage_to_gl_enum(uf_data.ssu_stage),
  ...
  for (const auto sub_ifc : SUBROUTINE_INTERFACE_IDS) {
    ...
    gl::GetProgramInterfaceiv(
      phandle, sub_ifc, gl::ACTIVE_RESOURCES, &subroutines_count);
    ...
    gl::GetProgramInterfaceiv(
      phandle, sub_ifc, gl::MAX_NAME_LENGTH, &max_name_len);
    ...
    for (int32_t idx = 0; idx < subroutines_count; ++idx) {
      ...
      gl::GetProgramResourceName(
        phandle, sub_ifc, idx, max_name_len, &name_len, name_buff.data());
...
void xray::rendering::gpu_program::set_uniform_block(const char*  block_name,
  ...
  memcpy(raw_ptr(ublocks_datastore_) + blk_iter->store_offset,
         block_data,
         byte_count);
...
void xray::rendering::gpu_program::set_subroutine_uniform(
  ...
  auto itr_subroutine = find_if(
    begin(subroutines_), end(subroutines_), [subroutine_name](const auto& sub) {
...
bool xray::rendering::detail::gpu_program_helpers::reflect(
  ...
  if (!collect_uniform_blocks(
        program, refdata->prg_ublocks, &refdata->prg_datastore_size)) {
    return false;
  }

  if (!collect_uniforms(
        program, *refdata->prg_ublocks, refdata->prg_uniforms)) {
    return false;
  }
...
bool xray::rendering::detail::gpu_program_helpers::collect_uniforms(
  ...
  //
  // sort uniforms by name.
  sort(begin(*uniforms), end(*uniforms), [](const auto& u0, const auto& u1) {
    return u0.name < u1.name;
  });
...
bool xray::rendering::detail::gpu_program_helpers::
  ...
  const int32_t subroutines_count = [phandle, api_subroutine_interface_name]() {
    GLint cnt{};
    gl::GetProgramInterfaceiv(
      phandle, api_subroutine_interface_name, gl::ACTIVE_RESOURCES, &cnt);
  ...
  const int32_t max_namelen_subroutine = [phandle,
    ...
    GLint len{};
    gl::GetProgramInterfaceiv(
      phandle, api_subroutine_interface_name, gl::MAX_NAME_LENGTH, &len);
...
void xray::rendering::detail::gpu_program_helpers::set_uniform(
  ...
  case gl::FLOAT:
    PROG_UNIFORM_FV(
      program_id, gl::ProgramUniform1fv, u_location, count, u_data);
    break;

  case gl::UNSIGNED_INT:
    PROG_UNIFORM_UIV(
      program_id, gl::ProgramUniform1uiv, u_location, count, u_data);
    break;

  case gl::FLOAT_VEC2:
    PROG_UNIFORM_FV(
      program_id, gl::ProgramUniform2fv, u_location, count, u_data);
    break;

  case gl::FLOAT_VEC3:
    PROG_UNIFORM_FV(
      program_id, gl::ProgramUniform3fv, u_location, count, u_data);
    break;

  case gl::FLOAT_VEC4:
    PROG_UNIFORM_FV(
      program_id, gl::ProgramUniform4fv, u_location, count, u_data);
    break;

  case gl::FLOAT_MAT2:
    PROG_UNIFORM_MAT_FV(
      program_id, gl::ProgramUniformMatrix2fv, u_location, count, u_data);
    break;

  case gl::FLOAT_MAT2x3:
    PROG_UNIFORM_MAT_FV(
      program_id, gl::ProgramUniformMatrix2x3fv, u_location, count, u_data);
    break;

  case gl::FLOAT_MAT3x2:
    PROG_UNIFORM_MAT_FV(
      program_id, gl::ProgramUniformMatrix3x2fv, u_location, count, u_data);
    break;

  case gl::FLOAT_MAT3:
    PROG_UNIFORM_MAT_FV(
      program_id, gl::ProgramUniformMatrix3fv, u_location, count, u_data);
    break;

  case gl::FLOAT_MAT4:
    PROG_UNIFORM_MAT_FV(
      program_id, gl::ProgramUniformMatrix4fv, u_location, count, u_data);
    break;
  ...
  case gl::SAMPLER_CUBE:
    PROG_UNIFORM_IV(
      program_id, gl::ProgramUniform1iv, u_location, count, u_data);
    break;
...
xray::rendering::detail::gpu_program_base::gpu_program_base(
  ...
  _valid =
    detail::gpu_program_helpers::reflect(base::raw_handle(_handle),
                                         api_subroutine_uniform_interface_name,
                                         api_subroutine_interface_name,
                                         &reflection_info);
...
void xray::rendering::detail::gpu_program_base::flush_uniforms() {
  ...
  if (!subroutine_uniforms_.empty()) {
    ...
    gl::UniformSubroutinesuiv(
      _stage, static_cast<GLsizei>(indices_buff.size()), indices_buff.data());
  }
}
