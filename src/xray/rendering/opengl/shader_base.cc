...
GLuint xray::rendering::make_shader(const uint32_t     shader_type,
  ...
  gl::ShaderSource(
    raw_handle(tmp_handle), strings_count, src_code_strings, nullptr);
  ...
  //  Get compile status
  {
    ...
    gl::GetShaderiv(
      raw_handle(tmp_handle), gl::COMPILE_STATUS, &compile_status);
  ...
  //  Get compile error
  {
    ...
    if (log_length > 0) {
      ...
      gl::GetShaderInfoLog(
        raw_handle(tmp_handle), log_length, nullptr, err_buff.data());
...
GLuint xray::rendering::make_shader(const uint32_t shader_type,
  ...
  try {
    ...
    XR_LOG_ERR(
      "Failed to open shader file [{}], error [{}]", source_file, ex.what());
