...
tl::expected<SpriteSystem, SpriteAtlasCreationError> SpriteSystem::from_file(
	...
	ScratchPadArena scratch = ThreadLocalContext::acquire_scratchpad({&arena});

	TextureAtlasData atlas_data;
	if (!deserialize_from_file(*scratch.arena, atlas_data, string_view_from_c_str(path.c_str()) )) {
		return tl::make_unexpected(
			SpriteAtlasError{
				.what = "serialization error"
	...
	auto atlas_image = VulkanImage::from_file(
		renderer,
		VulkanImageLoadInfo{
	...
	auto map_buffer_fn = [device = renderer.device()](const VulkanBuffer& vb) noexcept {
		void* mapped_addr{};
		const VkResult map_result = vkMapMemory(device, vb.memory_handle(), 0, VK_WHOLE_SIZE, 0, &mapped_addr);
...
tl::optional<SpriteAtlasEntry> SpriteSystem::get_sprite_by_name(const std::string_view sprite) const noexcept {
	const uint64_t hashed_name = FNV::fnv1a(sprite.data());
	return get_sprite_by_id(SpriteHandleType{hashed_name});
}

tl::optional<SpriteAtlasEntry> SpriteSystem::get_sprite_by_id(const SpriteHandleType id) const noexcept {
...
void SpriteSystem::draw_sprite(const SpriteEntry& sprite, const float ox, const float oy, const uint32_t color) {
	using namespace xray::math;
...
void SpriteSystem::render(const SpriteSystemRenderContext& rctx) {
	if (_cursor.index != 0) {
...
void SpriteSystem::add_sprite_quad(const std::span<const SpriteVertex> quad) {
	assert(quad.size() == 4);
