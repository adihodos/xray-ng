...
struct DebugDrawSystem::RenderStateVulkan {
	...
	~RenderStateVulkan() {
		if (mVertexBuffer.memory_handle()) vkUnmapMemory(mDevice, mVertexBuffer.memory_handle());
	}
...
tl::expected<DebugDrawSystem::RenderStateVulkan, VulkanError> DebugDrawSystem::RenderStateVulkan::create(
	const DebugDrawSystem::InitContext& init
) {
...
void DebugDrawSystem::RenderStateDeleter::operator()(DebugDrawSystem::RenderState* r) const noexcept { delete r; }

DebugDrawSystem::DebugDrawSystem(
	PrivateConstructionToken, std::unique_ptr<RenderState, RenderStateDeleter>&& render_state
)
	: mRenderState{std::move(render_state)} {}

DebugDrawSystem::~DebugDrawSystem() {}

void DebugDrawSystem::draw_line(const math::vec3f& from, const math::vec3f& to, const rgb_color& color) {
	*mRenderState->mBufferPtr++ = vertex_pc{from, color};
	*mRenderState->mBufferPtr++ = vertex_pc{to, color};
}

void DebugDrawSystem::draw_box(const std::span<const math::vec3f> points, const rgb_color& c) {
	assert(points.size() == 8);
	constexpr const uint8_t indices[] = {0, 1, 1, 2, 2, 3, 3, 0, 4, 5, 5, 6, 6, 7, 7, 4, 0, 4, 1, 5, 2, 6, 3, 7};
	static_assert(std::size(indices) % 2 == 0);

	for (size_t i = 0; i < std::size(indices) / 2; ++i) {
		draw_line(points[indices[i * 2 + 0]], points[indices[i * 2 + 1]], c);
	}
}

void DebugDrawSystem::draw_frustrum(const math::MatrixWithInvertedMatrixPair4f& mtx, const rgb_color& color) {
	// NDC coords near + far
...
void DebugDrawSystem::draw_oriented_box(
...
void DebugDrawSystem::draw_axis_aligned_box(
	const math::vec3f& minpoint, const math::vec3f& maxpoint, const rgb_color& c
) {
...
void DebugDrawSystem::draw_cross(
	const math::vec3f& o,
	const math::vec3f& u,
	const math::vec3f& v,
	const math::vec3f& w,
...
void DebugDrawSystem::draw_coord_sys(
	const math::vec3f& o,
	const math::vec3f& u,
	const math::vec3f& v,
	const math::vec3f& w,
...
void DebugDrawSystem::draw_grid(
...
void DebugDrawSystem::draw_arrow(const math::vec3f& from, const math::vec3f& to, const rgb_color& c) {
...
void DebugDrawSystem::draw_directional_light(const math::vec3f& dir, const float origin_dist, const rgb_color& c) {
...
void DebugDrawSystem::draw_plane(
	const math::vec3f& origin, const math::vec3f& normal, const float scale, const rgb_color& c
) {
...
void DebugDrawSystem::draw_circle(
	const math::vec3f& origin,
	...
	for (uint32_t idx = 0; idx < kSteps; ++idx) {
		...
		if (draw_options & Draw_CircleSegments) draw_line(origin, v0, c);
		draw_line(v0, v1, c);
	}

	if (draw_options & Draw_CircleNormal) draw_arrow(origin, origin + normal * radius * 0.5f, c);
}

void DebugDrawSystem::draw_sphere(const math::vec3f& origin, const float radius, const rgb_color& color) {
...
void DebugDrawSystem::draw_cone(
	const math::vec3f origin,
	const math::vec3f& normal,
	const float height,
	const float base,
...
void DebugDrawSystem::draw_vector(
	const math::vec3f& org, const math::vec3f& dir, const float length, const rgb_color& c
) {
	draw_arrow(org, org + dir * length, c);
}

tl::expected<DebugDrawSystem, DebugDrawSystem::ErrorType> DebugDrawSystem::create(const InitContext& init) {
#if defined(XRAY_GRAPHICS_API_VULKAN)
...
void DebugDrawSystem::new_frame(const uint32_t frame_id) noexcept {
#if defined(XRAY_GRAPHICS_API_VULKAN)
	mRenderState->mBufferPtr = mRenderState->mVertexMemoryMappings[frame_id];
#endif
}

void DebugDrawSystem::render(const DebugDrawSystem::RenderContext& rc) noexcept {
#if defined(XRAY_GRAPHICS_API_VULKAN)
	...
	vkCmdPushConstants(
		rc.frd->cmd_buf, mRenderState->mGraphicsPipeline.layout(), VK_SHADER_STAGE_ALL, 0, sizeof(uint32_t), &rc.frd->id
	);
