 ...
 */
VkFormat ktxTexture2_GetVkFormat(ktxTexture2* This) { return static_cast<VkFormat>(This->vkFormat); }
 ...
 */
static KTX_error_code optimalTilingCallback(
 ...
 * @copydetails PFNKTXITERCB
 */
KTX_error_code optimalTilingPadCallback(
	int miplevel, int face, int width, int height, int depth, ktx_uint64_t faceLodSize, void* pixels, void* userdata
) {
	user_cbdata_optimal* ud = (user_cbdata_optimal*)userdata;
...
tl::expected<xray::rendering::VulkanImage, xray::rendering::VulkanError> xray::rendering::VulkanImage::from_memory(
	...
	const VkMemoryAllocateInfo mem_alloc_info{
		...
		.memoryTypeIndex = renderer.find_allocation_memory_type(
			mem_requirements.memoryRequirements.memoryTypeBits, create_info.memory_flags
		),
	};
	...
	if (create_info.pixels.size() != 0) {
		...
		for (size_t i = 0, bytes_copied = 0, count = create_info.pixels.size(); i < count;
			...
			memcpy(
				reinterpret_cast<void*>(staging_buffer_ptr + bytes_copied),
	...
	return VulkanImage{
		xrUniqueImageWithMemoryAndView{
			renderer.device(),
...
tl::expected<xray::rendering::VulkanImage, xray::rendering::VulkanError> xray::rendering::VulkanImage::from_file(
	...
	const KTX_error_code tex_load_result = ktxTexture2_CreateFromMemory(
		reinterpret_cast<const ktx_uint8_t*>(texf.data()),
		texf.size(),
	...
	if (load_info.tiling == VK_IMAGE_TILING_OPTIMAL) {
...

		{
			ktx_internal_details::user_cbdata_optimal cbData{
				...
				.dest		   = reinterpret_cast<uint8_t*>(staging_buff_ptr),
			...
			if (loaded_ktx->pData) {
				...
				// buffer.
				memcpy(reinterpret_cast<void*>(staging_buff_ptr), loaded_ktx->pData, loaded_ktx->dataSize);
			} else {
				/* Load the image data directly into the staging buffer. */
				const ktxResult kResult = ktxTexture_LoadImageData(
					ktxTexture(loaded_ktx), reinterpret_cast<ktx_uint8_t*>(staging_buff_ptr), (ktx_size_t)textureSize
				);
		...
		for (VkBufferImageCopy& bcpy : copy_regions) {
		...
		const VkMemoryAllocateInfo mem_alloc_info = {
			...
			.memoryTypeIndex = renderer.find_allocation_memory_type(
				mem_req.memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
			),
		};
		...
		VkCommandBuffer cmdbuf = load_info.cmd_buf;
...
VkImageMemoryBarrier2 xray::rendering::make_image_layout_memory_barrier(
	const VkImage image,
...
void xray::rendering::set_image_layout(
	...
	const VkImageMemoryBarrier2 img_mem_barrier{
		make_image_layout_memory_barrier(image, initial_layout, final_layout, subresource_range)
	};
