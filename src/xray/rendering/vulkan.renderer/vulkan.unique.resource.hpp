...
struct VulkanResourceDeleterBase {
	...
	explicit VulkanResourceDeleterBase(
		VulkanResourceOwner owner, const VkAllocationCallbacks* alloc_cb = nullptr
	) noexcept
		: _owner{owner}, _alloc_cb{alloc_cb} {}
...
void dispatch_destructor_func(
	VulkanResource resource, VulkanResourceOwner owner, DestructorFn destroy_fn, const VkAllocationCallbacks* alloc_cb
) {
	if constexpr (std::is_same_v<VulkanResourceOwner, NotOwnedVulkanResource>) {
		destroy_fn(resource, alloc_cb);
	} else {
		destroy_fn(owner, resource, alloc_cb);
	}
}
...
struct UniqueMemoryMapping {
	...
	UniqueMemoryMapping(UniqueMemoryMapping&& rhs) {
		memcpy(reinterpret_cast<void*>(this), &rhs, sizeof(*this));
		memset(reinterpret_cast<void*>(&rhs), 0, sizeof(rhs));
	}
	...
	static tl::expected<UniqueMemoryMapping, VulkanError> map_memory(
...
struct VkResourceDeleter<VkImage> {
	void operator()(VkDevice device, VkImage image, const VkAllocationCallbacks* alloc_cb) const noexcept {
		if (image) vkDestroyImage(device, image, alloc_cb);
	}
};
...
struct VkResourceDeleter<VkImageView> {
	void operator()(VkDevice device, VkImageView image_view, const VkAllocationCallbacks* alloc_cb) const noexcept {
		if (image_view) vkDestroyImageView(device, image_view, alloc_cb);
	}
};
...
struct VkResourceDeleter<VkDeviceMemory> {
	void operator()(
		VkDevice device, VkDeviceMemory device_memory, const VkAllocationCallbacks* alloc_cb
	) const noexcept {
		if (device_memory) vkFreeMemory(device, device_memory, alloc_cb);
	}
};
...
struct VkResourceDeleter<VkBuffer> {
	void operator()(VkDevice device, VkBuffer buffer, const VkAllocationCallbacks* alloc_cb) const noexcept {
		if (buffer) vkDestroyBuffer(device, buffer, alloc_cb);
	}
};
...
struct VkResourceDeleter<VkDescriptorPool> {
	void operator()(VkDevice device, VkDescriptorPool dpool, const VkAllocationCallbacks* alloc_cb) const noexcept {
		if (dpool) vkDestroyDescriptorPool(device, dpool, alloc_cb);
	}
};
...
struct VkResourceDeleter<VkPipelineLayout> {
	void operator()(
		VkDevice device, VkPipelineLayout pipeline_layout, const VkAllocationCallbacks* alloc_cb
	) const noexcept {
		if (pipeline_layout) vkDestroyPipelineLayout(device, pipeline_layout, alloc_cb);
	}
};
...
struct UniqueVulkanResourcePack {
	UniqueVulkanResourcePack(Owner owner, Resources... args) noexcept
		: _owner{owner}, _resources{std::forward<Resources>(args)...} {}
	...
	UniqueVulkanResourcePack(UniqueVulkanResourcePack&& rhs) noexcept
		: _owner{rhs._owner}, _resources{std::move(rhs._resources)} {
...
void free_multiple_resources(ResourceDeleter deleter, Containers&&... containers) {
	(
