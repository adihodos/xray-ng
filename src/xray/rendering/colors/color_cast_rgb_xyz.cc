...
xray::rendering::xyz_to_rgb(const xyz_color& xyz) noexcept {
  ...
  const float r_linear =
      math::clamp(3.2406f * x - 1.5372f * y - 0.4986f * z, 0.0f, 1.0f);
  const float g_linear =
      math::clamp(-0.9689f * x + 1.8758f * y + 0.0415f * z, 0.0f, 1.0f);
  const float b_linear =
      math::clamp(0.0557f * x - 0.2040f * y + 1.0570f * z, 0.0f, 1.0f);
