...
{
    assert((hsl.h >= 0.0f) && (hsl.h <= 360.0f) && "Invalid hue value, must be in [0, 360] range");

    assert((hsl.s >= 0.0f) && (hsl.s <= 1.0f) && "Invalid saturation value, must be in [0, 1] range");

    assert((hsl.l >= 0.0f) && (hsl.l <= 1.0f) && "Invalid luminance value, must be in [0, 1] range");

    const auto c_val = (1.0f - std::fabs(2.0f * hsl.l - 1.0f)) * hsl.s;
    const auto x_val = c_val * (1.0f - std::fabs(std::fmod((hsl.h / 60.0f), 2.0f) - 1.0f));
    const auto m_val = hsl.l - c_val * 0.5f;

    const auto selector = static_cast<int32_t>(hsl.h / 60.0f);

    const float clr_values[] = { c_val, x_val, 0.0f,  x_val, c_val, 0.0f,  0.0f,  c_val, x_val,
                                 0.0f,  x_val, c_val, x_val, 0.0f,  c_val, c_val, 0.0f,  x_val };
...
xray::rendering::rgb_to_hsl(const rgb_color& rgb) noexcept
{
    ...
    hsl.s = math::is_zero(delta) ? 0.0f : delta / (1.0f - std::fabs(2.0f * (hsl.l) - 1.0f));
