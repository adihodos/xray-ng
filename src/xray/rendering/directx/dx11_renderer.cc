...
static const char*
dxgi_error_code_to_string(const HRESULT dxgi_errcode)
{
    switch (dxgi_errcode) {
...
xray::rendering::dx11_renderer::dx11_renderer(const renderer_init_params& init_params)
{
    ...
    //  Create the DirectX device and device context objects.
    {
        ...
        com_ptr<IDXGIFactory1> factory;
        {
            ...
            {
                const auto result = dx_device_->QueryInterface(__uuidof(IDXGIDevice1),
            ...
            {
                const auto result = dxgi_device->GetParent(__uuidof(IDXGIAdapter1),
            ...
            {
                const auto result =
                    dxgi_adapter->GetParent(__uuidof(IDXGIFactory1), reinterpret_cast<void**>(raw_ptr_ptr(factory)));
        ...
        maybe<DXGI_MODE_DESC> closest_mode_match{ nothing{} };
        {
            ...
            if (primary_output->FindClosestMatchingMode(&desired_mode, &closest_match, nullptr) == S_OK) {
        ...
        if (!closest_mode_match) {
            XR_LOG_ERR(
                "No display mode found with requested properties : {}x{}", output_wnd_width_, output_wnd_height_);
            return;
        }
        ...
        //  Create the swap chain.
        {
            ...
            const auto create_swapchain_res =
                factory->CreateSwapChain(raw_ptr(dx_device_), &swap_chain_desc, raw_ptr_ptr(swap_chain_));
...
void
xray::rendering::dx11_renderer::handle_resize(const uint32_t width, const uint32_t height) noexcept
{
    if (!valid())
        return;
    ...
    //  stencil views.
    {
        const auto resize_result = swap_chain_->ResizeBuffers(1, 0, 0, backbuffer_format_, 0);

        if (FAILED(resize_result)) {
            XR_LOG_CRITICAL("Failed to resize buffers, error {0}", dxgi_error_code_to_string(resize_result));
            return;
        }
...
void
xray::rendering::dx11_renderer::clear_render_target_view(const float red,
                                                         const float green,
                                                         const float blue,
...
void
xray::rendering::dx11_renderer::clear_depth_stencil()
{
    assert(valid());
    devcontext_->ClearDepthStencilView(
        raw_ptr(depth_stencil_view_), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0xFF);
}

void
xray::rendering::dx11_renderer::create_rtvs() noexcept
{
    assert(device_valid());
    ...
    //  the pipeline.
    {
        ...
        swap_chain_->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(raw_ptr_ptr(backbuffer_tex)));
        ...
        const auto ret_code =
            dx_device_->CreateRenderTargetView(raw_ptr(backbuffer_tex), nullptr, raw_ptr_ptr(render_target_view_));
    ...
    //  Create the depth stencil texture and the associated view.
    {
        ...
        {
            const auto ret_code =
                dx_device_->CreateTexture2D(&tex_stencil_desc, nullptr, raw_ptr_ptr(depth_stencil_texture_));
...

        {
            const auto result_create_dsv = dx_device_->CreateDepthStencilView(
                raw_ptr(depth_stencil_texture_), nullptr, raw_ptr_ptr(depth_stencil_view_));
    ...
    //  output merger stage.
    {
        ...
        devcontext_->OMSetRenderTargets(
            XR_U32_COUNTOF(bound_render_targets), bound_render_targets, raw_ptr(depth_stencil_view_));
    }
...
HRESULT
xray::rendering::dx11_renderer::swap_buffers(const bool test_for_occlusion /*= false*/) const noexcept
{
    if (!valid())
        return E_FAIL;
...
void
xray::rendering::dx11_renderer::toggle_fullscreen() noexcept
{
    if (!valid())
        return;
