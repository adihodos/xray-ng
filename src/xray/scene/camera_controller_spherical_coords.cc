...
void
xray::scene::camera_controller_spherical_coords::input_event(const ui::window_event& input_evt)
{
    using namespace xray::ui;

    assert(ui::is_input_event(input_evt));

    if ((input_evt.type == event_type::mouse_motion) && (input_evt.event.motion.button1)) {
        ...
        // positions.
        if (!_last_mouse_pos) {
            _last_mouse_pos = vec2f{ static_cast<float>(input_evt.event.motion.pointer_x),
    ...
    if (input_evt.type == event_type::mouse_wheel) {
        ...
        _params.radius_ =
            clamp(_params.radius_ + delta * _params.speed_move_, _params.radius_min_, _params.radius_max_);
        return;
    }
}

void
xray::scene::camera_controller_spherical_coords::mouse_moved(const float x_pos, const float y_pos) noexcept
{
