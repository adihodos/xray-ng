...
static xray::ui::KeySymbol map_x11_key_symbol(const xkb_keysym_t key_sym) noexcept {
	...
	const U32 x11_key = static_cast<U32>(key_sym);
	//
...
struct GamepadAxisMapping {
	xray::I32 native;
	xray::ui::GamepadAxis translated;
};

struct GamepadButtonMapping {
	xray::I32 native;
	xray::ui::GamepadButton translated;
};

struct PlatformGamepad {
	static constexpr const GamepadAxisMapping AXIS_MAPPING[] = {
		{ABS_X, xray::ui::GamepadAxis::LeftX},
		{ABS_Y, xray::ui::GamepadAxis::LeftY},
		{ABS_RX, xray::ui::GamepadAxis::RightX},
		{ABS_RY, xray::ui::GamepadAxis::RightY},
		{ABS_Z, xray::ui::GamepadAxis::LeftZ},
		{ABS_RZ, xray::ui::GamepadAxis::RightZ},
	};

	static constexpr const GamepadButtonMapping BUTTON_MAPPING[] = {
		{BTN_TL, xray::ui::GamepadButton::Left},
		{BTN_TL2, xray::ui::GamepadButton::Left2},
		{BTN_TR, xray::ui::GamepadButton::Right},
		{BTN_TR2, xray::ui::GamepadButton::Right2},
	};
...
void PlatformWindow_HandleGamepadEvent(
	const input_event& evt, const xray::ui::PlatformWindow::PlatformImpl* impl, xray::ui::WindowCommonCore* core
) noexcept {
	(void)impl;
	(void)core;
	//
...
tl::expected<PolledDevicesState, std::error_condition> PolledDevicesState::create(Display* dpy) {
	const I32 x11_fd = XConnectionNumber(dpy);
	if (x11_fd == -1) {
		return tl::make_unexpected(std::error_code{errno, std::system_category()}.default_error_condition());
	}

	const I32 epoll_fd = epoll_create(32);
	if (epoll_fd == -1) {
		return tl::make_unexpected(std::error_code{errno, std::system_category()}.default_error_condition());
	}

	epoll_event e{.events = EPOLLIN | EPOLLERR, .data = {.fd = x11_fd}};
	if (const I32 add_fd_result = epoll_ctl(epoll_fd, EPOLL_CTL_ADD, x11_fd, &e); add_fd_result != 0) {
		return tl::make_unexpected(std::error_code{errno, std::system_category()}.default_error_condition());
	}
	...
	auto gamepads_range =
		...
		std::views::transform(
			[epoll_fd](const fs::directory_entry& de) -> tl::expected<PlatformGamepad, std::error_condition> {
				XR_LOG_INFO("gamepad device %s", de.path().c_str());
				I32 gamepad_fd = open(de.path().generic_string().c_str(), O_RDONLY | O_NONBLOCK);
				if (gamepad_fd == -1) {
				...
				for (const GamepadAxisMapping axis_mapping : PlatformGamepad::AXIS_MAPPING) {
					...
					const I32 res = syscall_wrapper(ioctl, gamepad_fd, EVIOCGABS(axis_mapping.native), &abs_info);
					if (res == -1) {
						return tl::make_unexpected(
							std::error_code{errno, std::system_category()}.default_error_condition()
						);
				...
				const I32 add_res = epoll_ctl(epoll_fd, EPOLL_CTL_ADD, gamepad_fd, &e);
				if (add_res == -1) {
					return tl::make_unexpected(
						std::error_code{errno, std::system_category()}.default_error_condition()
					);
	...
	for (auto gp : gamepads_range) {
		XR_LOG_INFO("Gamepad %s -> %d", gp->_device_path.generic_string().c_str(), gp->_device_id);
...
void PlatformWindow_HandleXkbEvent(
	const XkbEvent* xkb_evt, xray::ui::PlatformWindow::PlatformImpl* impl, xray::ui::WindowCommonCore* core
) {
	if (xkb_evt->any.xkb_type == XkbMapNotify) {
		impl->_input_helper.xkb_keymap = {};
		impl->_input_helper.xkb_keymap = xray::ui::detail::unique_xkb_keyman{xkb_x11_keymap_new_from_device(
			raw_ptr(impl->_input_helper.xkb_ctx),
			impl->_input_helper.xcb_con,
			impl->_input_helper.device_id,
			XKB_KEYMAP_COMPILE_NO_FLAGS
		)};

		XRAY_ASSERT(impl->_input_helper.xkb_keymap, "Failed to update Keyboard map!");
		XR_LOG_INFO("Keyboard map updated!");
		return;
	}

	if (xkb_evt->any.xkb_type == XkbStateNotify) {
		auto get_xkb_mod_mask_fn = [xi = &impl->_input_helper](const U32 in) {
			U32 ret = 0;
			if ((in & ShiftMask) && xi->mod_index.shift_mod != XKB_MOD_INVALID) ret |= (1 << xi->mod_index.shift_mod);
			if ((in & LockMask) && xi->mod_index.caps_mod != XKB_MOD_INVALID) ret |= (1 << xi->mod_index.caps_mod);
			if ((in & ControlMask) && xi->mod_index.ctrl_mod != XKB_MOD_INVALID) ret |= (1 << xi->mod_index.ctrl_mod);
			if ((in & Mod1Mask) && xi->mod_index.alt_mod != XKB_MOD_INVALID) ret |= (1 << xi->mod_index.alt_mod);
			if ((in & Mod2Mask) && xi->mod_index.num_mod != XKB_MOD_INVALID) ret |= (1 << xi->mod_index.num_mod);

			// mod3 - scroll lock, don’t need it for now
			// if ((in & Mod3Mask) && xi->mod_index.mod3_mod != XKB_MOD_INVALID)
			// 	ret |= (1 << xi->mod_index.mod3_mod);

			if ((in & Mod4Mask) && xi->mod_index.logo_mod != XKB_MOD_INVALID) ret |= (1 << xi->mod_index.logo_mod);
		...
		xkb_state_update_mask(
			raw_ptr(impl->_input_helper.xkb_state),
...
void PlatformWindow_HandleMouseButtonEvent(const XButtonEvent* x11evt, xray::ui::WindowCommonCore* core) {
	struct x11_button_mapping {
		U32 x11_btn;
		xrMouseButton_t xray_btn;
	};

	static constexpr x11_button_mapping REGULAR_BUTTON_MAPPINGS[] = {
		{Button1, xrMouseButton_t::button1},
		{Button2, xrMouseButton_t::button2},
		{Button3, xrMouseButton_t::button3},
	};

	bool was_regular_button = false;
	for (const x11_button_mapping& btn_map : REGULAR_BUTTON_MAPPINGS) {
		if (btn_map.x11_btn == x11evt->button) {
			const xrButton_t prev_state = core->mouse.buttons[static_cast<U8>(btn_map.xray_btn)];
			core->mouse.buttons[static_cast<U8>(btn_map.xray_btn)] = xrButton_t{
				.is_down	  = x11evt->type == ButtonPress,
				.was_pressed  = prev_state.was_pressed == false,
				.was_released = x11evt->type == ButtonRelease,
			};

			was_regular_button = true;
			break;
		}
	}

	if (!was_regular_button) {
		struct WheelButtonToDeltaMapping {
			U32 nativeid;
			F32 delta;
		} constexpr WHEEL_BUTTON_MAPPINGS[] = {
			WheelButtonToDeltaMapping{
				.nativeid = Button4,
				.delta	  = +1.0f,
			},
			WheelButtonToDeltaMapping{
		...
		for (const WheelButtonToDeltaMapping& btn_map : WHEEL_BUTTON_MAPPINGS) {
			if (btn_map.nativeid == x11evt->button) {
				core->mouse.delta_mousewheel = btn_map.delta;
				break;
			}
		}
	}
...
void PlatformWindow_HandleKeyEvent(
	const XKeyEvent* x11evt, const xray::ui::PlatformWindow::PlatformImpl* impl, xray::ui::WindowCommonCore* core
) {
	const xkb_keycode_t key_code{x11evt->keycode};
	const xkb_keysym_t key_sym{xkb_state_key_get_one_sym(raw_ptr(impl->_input_helper.xkb_state), key_code)};

	if (const KeySymbol mapped_key = map_x11_key_symbol(key_sym); mapped_key != KeySymbol::unknown) {
		const ISIZE key_sym_idx = static_cast<ISIZE>(mapped_key);

		if (core->key_names[key_sym_idx].len == 0) {
			if (const I32 bytes_len = xkb_keysym_get_name(
					key_sym, core->key_names[key_sym_idx].name, sizeof(core->key_names[key_sym_idx].name)
				);
				bytes_len >= 0) {
				core->key_names[key_sym_idx].len = bytes_len;
			}
		}

		const xrButton_t prev_state = core->keys[key_sym_idx];
		core->keys[key_sym_idx]		= xrButton_t{
				.is_down	  = x11evt->type == KeyPress,
				.was_pressed  = !prev_state.was_pressed,
				.was_released = x11evt->type == KeyRelease,
		};

		// XR_LOG_INFO(
		// 	"Key event: %" PRI_xrStringView_t ", down %s, was pressed %s, was released %s",
		// 	FMT_xrStringView_t(
		// 		slice_from_ptr_and_len(core->key_names[key_sym_idx].name, core->key_names[key_sym_idx].len)
		// 	),
		// 	core->keys[key_sym_idx].is_down ? "yes" : "no",
		// 	core->keys[key_sym_idx].was_pressed ? "yes" : "no",
		// 	core->keys[key_sym_idx].was_released ? "yes" : "no"
		// );
	}
	...
	if (x11evt->type == KeyPress) {
		...
		if (core->key_sym_handling == InputKeySymHandling::Unicode) {
			const U32 unicode_point = xkb_state_key_get_utf32(raw_ptr(impl->_input_helper.xkb_state), key_code);

			if (unicode_point != 0 && core->typing.s_len < xr_size_of(core->typing_buff)) {
				core->typing_buff[core->typing.s_len++] = static_cast<C32>(unicode_point);
			}
		} else {
			KeySym x11_key_sym;
			U32 mod_return{};
			XkbLookupKeySym(raw_ptr(impl->_display), x11evt->keycode, x11evt->state, &mod_return, &x11_key_sym);

			//
			// Not entirely sure about this, MonkaS ...
			C8 input_buff[128];
			const I32 num_bytes = XkbTranslateKeySym(
				raw_ptr(impl->_display), &x11_key_sym, x11evt->state, input_buff, sizeof(input_buff), nullptr
			);
			input_buff[num_bytes > 0 ? num_bytes : 0] = 0;

			if (num_bytes > 0) {
				for (I32 idx = 0; idx < num_bytes && core->typing.s_len < xr_size_of(core->typing_buff); ++idx) {
					core->typing_buff[core->typing.s_len++] = input_buff[idx];
				}
			}
		}
	}
...
void PlatformWindow_HandleClientMessageEvent(
	const XClientMessageEvent* x11evt,
	const xray::ui::PlatformWindow::PlatformImpl* impl,
	xray::ui::WindowCommonCore* core
) {
	if (static_cast<U32>(x11evt->data.l[0]) == impl->_window_delete_atom) {
		XR_LOG_INFO("Quit received, exiting message loop...");
		core->state.flags.quit = true;
	}
}

void PlatformWindow_HandleConfigureEvent(const XConfigureEvent* x11evt, xray::ui::WindowCommonCore* core) {
	core->state.size.width	= x11evt->width;
	core->state.size.height = x11evt->height;
}

void PlatformWindow_HandleMotionNotifyEvent(const XMotionEvent* x11evt, xray::ui::WindowCommonCore* core) {
	const vec2i32 prev_pos{core->mouse.position.x, core->mouse.position.y};
	const vec2i32 curr_pos{x11evt->x, x11evt->y};
	const vec2i32 delta_pos = curr_pos - prev_pos;

	core->mouse.delta_position.x = delta_pos.x;
	core->mouse.delta_position.y = delta_pos.y;
	core->mouse.position.x		 = curr_pos.x;
	core->mouse.position.y		 = curr_pos.y;
}

xray::ui::PlatformWindow* xray::ui::PlatformWindow::create(
	xray::base::MemoryArena& arena, const xray::ui::window_params_t& win_params
) {
	using namespace xray::ui::detail;

	I32 ver_major{XkbMajorVersion};
	I32 ver_minor{XkbMinorVersion};
	I32 err_code{};
	I32 xkb_event_base{};
	I32 xkb_error_base{};
	...
	if (!_display) {
		XR_LOG_CRITICAL("Failed to open display, error %d", err_code);
		return nullptr;
	}

	constexpr const U32 xkb_details_mask =
	...
	const I32 device_id = xkb_x11_get_core_keyboard_device_id(connection);
	XR_LOG_INFO("Core keyboard id %d", device_id);
	...
	const I32 _default_screen = XDefaultScreen(raw_ptr(_display));
	...
	window_attribs.background_pixel	 = 0xff32a852;
	...
	//  set window properties
	{
		...
		XRAY_SCOPE_EXIT_NOEXCEPT { XFree(wnd_name.value); };
		...
		XRAY_SCOPE_EXIT_NOEXCEPT { XFree(icon_name.value); };
	...
	[[maybe_unused]] const Atom name_atoms[] = {
	...
	const U32 mod_shift = xkb_keymap_mod_get_index(raw_ptr(keymap), XKB_MOD_NAME_SHIFT);
	const U32 mod_caps	= xkb_keymap_mod_get_index(raw_ptr(keymap), XKB_MOD_NAME_CAPS);
	const U32 mod_alt	= xkb_keymap_mod_get_index(raw_ptr(keymap), XKB_MOD_NAME_ALT);
	const U32 mod_ctrl	= xkb_keymap_mod_get_index(raw_ptr(keymap), XKB_MOD_NAME_CTRL);
	const U32 mod_num	= xkb_keymap_mod_get_index(raw_ptr(keymap), XKB_MOD_NAME_NUM);
	const U32 mod_logo	= xkb_keymap_mod_get_index(raw_ptr(keymap), XKB_MOD_NAME_LOGO);

	auto polled_state = detail::PolledDevicesState::create(raw_ptr(_display));
	if (!polled_state) {
		XR_LOG_ERR("Failed to query polled devices, error %s", polled_state.error().message().c_str());
		return nullptr;
	}

	PlatformWindow* pw = new (arena.alloc_align<PlatformWindow>()) PlatformWindow{
...
void xray::ui::PlatformWindow::tick() noexcept {
	core.frame += 1;
	core.typing.s_ptr = core.typing_buff;
	core.typing.s_len = 0;

	epoll_event queued_events[32];
	const ISIZE events_count = syscall_wrapper(epoll_wait, _platform->_poll_state._epoll_fd, queued_events, 32, 0);

	if (events_count == -1) {
		XR_LOG_CRITICAL("epoll_wait error %d", errno);
		return;
	}

	for (ISIZE event = 0; event < events_count && core.state.flags.quit == false; ++event) {
		const epoll_event& e = queued_events[event];

		//
		// handle X11 event
		if (e.data.fd == _platform->_poll_state._x11_display_fd) {
			while ((core.state.flags.quit == false) && XEventsQueued(raw_ptr(_platform->_display), QueuedAfterFlush)) {
				XEvent window_event;
				...
				if (window_event.type == _platform->_input_helper.xkb_event_base) {
					...
					PlatformWindow_HandleXkbEvent(xkb_evt, raw_ptr(_platform), &core);
					continue;
				}

				if ((window_event.type == ButtonPress) || (window_event.type == ButtonRelease)) {
					PlatformWindow_HandleMouseButtonEvent(&window_event.xbutton, &core);
					continue;
				}

				if (window_event.type == MotionNotify) {
					PlatformWindow_HandleMotionNotifyEvent(&window_event.xmotion, &core);
					continue;
				}

				if (window_event.type == ClientMessage) {
					PlatformWindow_HandleClientMessageEvent(&window_event.xclient, raw_ptr(_platform), &core);
					continue;
				}

				if ((window_event.type == KeyPress) || (window_event.type == KeyRelease)) {
					PlatformWindow_HandleKeyEvent(&window_event.xkey, raw_ptr(_platform), &core);
					continue;
				}

				if (window_event.type == ConfigureNotify) {
					PlatformWindow_HandleConfigureEvent(&window_event.xconfigure, &core);
				}
			}
			continue;
		}
		...
		auto itr_gamepad = ranges::find_if(_platform->_poll_state._gamepads, [&](const PlatformGamepad& gamepad) {
		...
		if (itr_gamepad != std::cend(_platform->_poll_state._gamepads)) {
			...
			for (;;) {
				...
				if (bytes_read <= 0) {
					if (errno != EAGAIN) {
						XR_LOG_ERR(
							"Failed to read from gamepad %s, error %d",
							itr_gamepad->_device_path.generic_string().c_str(),
							errno
						);
					}
					break;
				...
				for (ISIZE i = 0; i < (bytes_read / xr_size_of(input_event)); ++i) {
					PlatformWindow_HandleGamepadEvent(gamepad_events[i], raw_ptr(_platform), &core);
				}
			}
			continue;
		}
...
void xray::ui::PlatformWindow::quit() noexcept { core.state.flags.quit = true; }

xray::base::xrSlice_t<const xray::ui::GamepadAxisInfo> xray::ui::PlatformWindow::gamepad_axis_info() const noexcept {
	if (_platform->_poll_state._gamepads.empty()) {
		return xrSlice_t<const GamepadAxisInfo>{};
	} else {
		return base::slice_from_seq_container(_platform->_poll_state._gamepads[0]._axis_info);
	}
}
