...
static const TCHAR* const APP_WINDOW_CLASS_NAME = _T("__##@@%%axaxaxa_!!!_Win32XrayWndClass_##@@__");

static constexpr auto swp_flags = SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED;
...
xray::ui::application_window::application_window(const xray::ui::window_params_t& init_params)
{
...
LRESULT WINAPI
xray::ui::application_window::wnd_proc_stub(HWND wnd, UINT msg, WPARAM w_param, LPARAM l_param)
{
    if (msg == WM_CREATE) {
        auto create_data = reinterpret_cast<CREATESTRUCT*>(l_param);
        auto wnd_object = reinterpret_cast<application_window*>(create_data->lpCreateParams);

        SetWindowLongPtrW(wnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(wnd_object));
        return true;
    }

    application_window* wptr =
        reinterpret_cast<application_window*>(static_cast<LONG_PTR>(GetWindowLongPtrW(wnd, GWLP_USERDATA)));
...
void
xray::ui::application_window::wm_size_event(const int32_t size_request,
                                            const int32_t width,
...
void
xray::ui::application_window::run() noexcept
{
    if (!is_valid()) {
        return;
    }
...
void
xray::ui::application_window::make_frame()
{
...
void
xray::ui::application_window::key_event(const int32_t key_code, const bool down /*= true*/) noexcept
{
...
void
xray::ui::application_window::syskeydown_event(const WPARAM w_param, const LPARAM l_param) noexcept
{
...
void
xray::ui::application_window::mouse_button_event(WPARAM /*w_param*/,
                                                 LPARAM l_param,
...
void
xray::ui::application_window::mouse_move_event(WPARAM /*w_param*/, LPARAM l_param) noexcept
{
...
void
xray::ui::application_window::mouse_wheel_event(WPARAM w_param, LPARAM l_param) noexcept
{
    assert(is_valid());
...
LRESULT
xray::ui::application_window::window_procedure(UINT msg, WPARAM w_param, LPARAM l_param)
{
    switch (msg) {
        case WM_CLOSE: {
        ...
        case WM_SIZE:
            wm_size_event(static_cast<int32_t>(w_param), LOWORD(l_param), HIWORD(l_param));
            break;
