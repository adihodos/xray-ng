...
#elif defined(XRAY_RENDERER_OPENGL)
...
static ImGuiKey translate_key(const KeySymbol key_sym_val) {
...
xray::ui::user_interface::user_interface() noexcept {
	...
	// TerminessNerdFontMono-Regular.ttf
	set_current();
	load_fonts({});
}
...
xray::ui::user_interface::user_interface(const FontsLoadBundle& font_packages) {
	...
	for (size_t i = 0, count = font_packages.info.size(); i < count; ++i) {
		...
		for (const uint8_t font_size : curr_font.sizes) {
			base::format_to_n(scratch_buffer, "%s_%u", base_font_name.c_str(), font_size);
			...
			ImFont* font_handle = ImGui::GetIO().Fonts->AddFontFromMemoryTTF(
				(void*)mmaped_font_data.data(),
			...
			if (!font_handle) {
				XR_LOG_INFO("Failed to load font %s", curr_font.path.string().c_str());
				continue;
			}
	...
	for_each(begin(_rendercontext.fonts), end(_rendercontext.fonts), [](const loaded_font& fi) {
		XR_LOG_INFO(
			"added font %s, size %3.3f, handle %p ...",
			fi.name.c_str(),
			fi.pixel_size,
			static_cast<const void*>(fi.font)
		);
	});
	...
	XR_LOG_INFO("Font atlas %ux%u", _rendercontext.atlas_width, _rendercontext.atlas_height);
}
...
static constexpr const char IMGUI_SHADER[] =
...
void xray::ui::user_interface::init(const std::span<const font_info> font_list) {
#if defined(XRAY_RENDERER_DIRECTX)
...

	{
		...
		if (!_rendercontext.vertex_buffer) return;
	}

	{
		...
		if (!_rendercontext.index_buffer) return;
	}

	{
		...
		if (!_rendercontext.font_texture) return;
...

	{
		...
		if (!_rendercontext.blend_state) return;
	}

	{
		...
		if (!_rendercontext.depth_stencil_state) return;
	}

	{
		...
		if (!_rendercontext.raster_state) return;
	}

	{
		...
		if (!_rendercontext.input_layout) return;
	}
...
void xray::ui::user_interface::load_fonts(const std::span<const font_info> font_list) {
...
tl::optional<xray::ui::UserInterfaceRenderContext> xray::ui::user_interface::draw() {
	ImGui::Render();
...

	{
		...
		dc->IASetIndexBuffer(
			raw_ptr(_rendercontext.index_buffer),
	...
	if (fb_width == 0 || fb_height == 0) return tl::nullopt;
...
void xray::ui::user_interface::new_frame(const int32_t wnd_width, const int32_t wnd_height) {
	ImGuiIO& io	   = ImGui::GetIO();
...
bool xray::ui::user_interface::input_event(const xray::ui::window_event& in_evt) {
	if (in_evt.type == event_type::key) {
		...
		if (translated_key != ImGuiKey_None) io.AddKeyEvent(translated_key, ke.type == event_action_type::press);
...
void xray::ui::user_interface::tick(const float delta) { ImGui::GetIO().DeltaTime = delta / 1000.0f; }
...
xray::ui::user_interface::loaded_font* xray::ui::user_interface::find_font(const char* name) {
	if (name == nullptr) {
		name = "Default";
	}
	...
	if (fentry == end(_rendercontext.fonts)) {
		XR_LOG_INFO("Trying to set non existent font %s", name);
		return nullptr;
	}
...
xray::ui::UserInterfaceBackendCreateInfo xray::ui::user_interface::render_backend_create_info() noexcept {
