...
/*
 ...
 - FREQUENTLY ASKED QUESTIONS (FAQ), TIPS
   ...
   - Can I have multiple widgets with the same label? Can I have widget without
a label? (Yes) / A primer on the use of labels/IDs in ImGui.
...
examples/opengl_example/ first at it is the simplest.
...
you need to upload that your your graphic       
...
functions anytime
   ...
   - after calling ImGui::NewFrame() you can read back flags from the IO
structure to tell how ImGui intends to use your inputs.
...
may want to discard/hide the inputs from the rest of your application.
     When 'io.WantInputsCharacters' is set to may want to notify your OS to
popup an on-screen keyboard, if available.
...
OpenPopup() to actually set the "opened" state of a popup. BeginPopup() returns
...
"Click"
   ...
   - When working with trees, ID are used to preserve the opened/closed state of
each tree node.
...
load. ImGui will support UTF-8 encoding across the board.
    Character input depends on you passing the right character code to
...
requested/discussed by users (at that point it usually happens on the github)
 ...
 - window: maximum window size settings (per-axis). for large popups in
particular user may not want the popup to fill all space.
...
limits)
 ...
 - tree node: clarify spacing, perhaps provide API to query exact spacing.
provide API to draw the primitive. same with Bullet().
 - tree node: tree-node/header right-most side doesn't take account of
horizontal scrolling.
 - tree node: add treenode/treepush int variants? because (void*) cast from int
warns on some platforms/settings
...
auto-resize context (tooltip, etc.) (git issue #249)
...
scaling easier.
 - font: helper to add glyph redirect/replacements (e.g. redirect alternate
apostrophe unicode code points to ascii one, etc.)
...
!- demo: custom render demo pushes a clipping rectangle past parent window
bounds. expose ImGui::PushClipRect() from imgui_internal.h?
...
#pragma clang diagnostic ignored                                               \
    "-Wmissing-noreturn" // warning : function xx could be declared with
                         // attribute 'noreturn' warning    //
                         // GetDefaultFontData() asserts which some
                         // implementation makes it never return.
#pragma clang diagnostic ignored                                               \
...
// You can swap the pointer to a different context by calling
// ImGui::SetInternalState()
static ImGuiState GImDefaultState;
ImGuiState*       GImGui = &GImDefaultState;

// Statically allocated default font atlas. This is merely a maneuver to keep
...
bool ImGui::ItemAdd(const ImRect& bb, const ImGuiID* id) {
  ...
  if (IsMouseHoveringRect(bb.Min, bb.Max)) {
    // Matching the behavior of IsHovered() but ignore if
...
bool ImGui::FocusableItemRegister(ImGuiWindow* window, bool is_active,
  ...
  if (tab_stop && window->FocusIdxAllRequestNext == IM_INT_MAX &&
      window->FocusIdxTabRequestNext == IM_INT_MAX && is_active &&
...
void ImGui::NewFrame() {
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
    if (g.IO.MouseClicked[i])
      g.IO.MouseDownOwned[i] =
          (g.HoveredWindow != NULL) || (!g.OpenedPopupStack.empty());
  ...
  if (g.CaptureMouseNextFrame != -1)
    ...
    g.IO.WantCaptureMouse =
        ...
        (g.ActiveId != 0) || (!g.OpenedPopupStack.empty());
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list,
  ...
  IM_ASSERT((int) draw_list->_VtxCurrentIdx ==
            draw_list->VtxBuffer.Size); // Sanity check. Bug or mismatch between
                                        // PrimReserve() calls and incrementing
...
void ImGui::PushClipRect(const ImVec2& clip_rect_min,
                         ...
                         bool          intersect_with_existing_clip_rect) {
  ImGuiWindow* window = GetCurrentWindow();
  ...
  window->DrawList->PushClipRect(
      ImVec4(cr.Min.x, cr.Min.y, cr.Max.x, cr.Max.y));
}
...
void ImGui::RenderCollapseTriangle(ImVec2 p_min, bool opened, float scale,
                                   bool shadow) {
  ImGuiState&  g      = *GImGui;
  ImGuiWindow* window = GetCurrentWindow();
...
void ImGui::OpenPopupEx(const char* str_id, bool reopen_existing) {
  ...
  if (g.OpenedPopupStack.Size < current_stack_size + 1)
    g.OpenedPopupStack.push_back(popup_ref);
           ...
           g.OpenedPopupStack[current_stack_size].PopupID != id) {
  ...
  if (g.FocusedWindow) {
    for (n = 0; n < g.OpenedPopupStack.Size; n++) {
      ...
      for (int m = n; m < g.OpenedPopupStack.Size && !has_focus; m++)
        has_focus = (g.OpenedPopupStack[m].Window &&
                     g.OpenedPopupStack[m].Window->RootWindow ==
                         g.FocusedWindow->RootWindow);
      if (!has_focus)
        break;
    }
  }
  if (n < g.OpenedPopupStack.Size) // This test is not required but it allows to
...
void ImGui::CloseCurrentPopup() {
  ...
  if (popup_idx < 0 || popup_idx > g.OpenedPopupStack.Size ||
      g.CurrentPopupStack[popup_idx].PopupID !=
          g.OpenedPopupStack[popup_idx].PopupID)
    return;
  while (popup_idx > 0 && g.OpenedPopupStack[popup_idx].Window &&
         (g.OpenedPopupStack[popup_idx].Window->Flags &
...
bool ImGui::BeginPopup(const char* str_id) {
  if (GImGui->OpenedPopupStack.Size <=
...
bool ImGui::BeginPopupModal(const char* name, bool* p_opened,
  ...
  if (!opened || (p_opened && !*p_opened)) // Opened can be 'false' when the
...
bool ImGui::Begin(const char* name, bool* p_opened,
  ...
  if (first_begin_of_the_frame) {
    ...
    // sheering). Only valid for root windows.
    ...
    window->FocusIdxAllRequestCurrent =
        (window->FocusIdxAllRequestNext == IM_INT_MAX ||
    ...
    if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
      ...
      ImVec2 clip_max =
          ImVec2(window->Pos.x + window->Size.x -
                     (p_opened ? title_bar_rect.GetHeight() - 3
      ...
      RenderTextClipped(text_min, text_max, name, NULL, &text_size,
                        style.WindowTitleAlign, NULL, &clip_max);
    }
  ...
  ImRect       clip_rect;
  clip_rect.Min.x = title_bar_rect.Min.x + 0.5f +
                    ImMax(border_size, window->WindowPadding.x * 0.5f);
  clip_rect.Min.y =
      title_bar_rect.Max.y + window->MenuBarHeight() + 0.5f + border_size;
  clip_rect.Max.x = window->Pos.x + window->Size.x - window->ScrollbarSizes.x -
                    ImMax(border_size, window->WindowPadding.x * 0.5f);
  clip_rect.Max.y =
      window->Pos.y + window->Size.y - border_size - window->ScrollbarSizes.y;
  ...
  if (flags & ImGuiWindowFlags_ChildWindow) {
    ...
    if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) &&
        window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0)
      window->Collapsed |=
          (window->ClippedWindowRect.Min.x >= window->ClippedWindowRect.Max.x ||
           window->ClippedWindowRect.Min.y >= window->ClippedWindowRect.Max.y);
...
static void Scrollbar(ImGuiWindow* window, bool horizontal) {
  ...
  if (horizontal)
    ...
    window_rounding_corners =
        ((window->Flags & ImGuiWindowFlags_NoTitleBar) ? 2 : 0) |
        (other_scrollbar ? 0 : 4);
...
// FIXME-OPT: Could cache and maintain it (pretty much only change on columns
// change)
ImVec2 ImGui::GetContentRegionMax() {
  ImGuiWindow* window = GetCurrentWindowRead();
  ImVec2       content_region_size =
      ImVec2(window->SizeContentsExplicit.x
                 ? window->SizeContentsExplicit.x
...
ImVec2 ImGui::GetWindowContentRegionMin() {
  ImGuiWindow* window = GetCurrentWindowRead();
  return ImVec2(-window->Scroll.x, -window->Scroll.y +
  ...
  ImVec2       content_region_size =
      ImVec2(window->SizeContentsExplicit.x ? window->SizeContentsExplicit.x
             ...
             window->SizeContentsExplicit.y ? window->SizeContentsExplicit.y
                                            : window->Size.y);
  ImVec2 m = content_region_size - window->Scroll - window->WindowPadding -
             window->ScrollbarSizes;
  return m;
}
...
// FIXME: Split into CollapsingHeader(label, default_open?) and
// TreeNodeBehavior(label), obsolete the 4 parameters function.
bool ImGui::CollapsingHeader(const char* label, const char* str_id,
                             bool display_frame, bool default_open) {
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
  ...
  if (label == NULL)
    label = str_id;
  const bool label_hide_text_after_double_hash =
      (label == str_id); // Only search and hide text after ## if we have passed
                         // label and ID separately, otherwise allow "##" within
                         // format string.
  const ImGuiID id = window->GetID(str_id);
  const ImVec2  label_size =
      CalcTextSize(label, NULL, label_hide_text_after_double_hash);
  ...
  const ImVec2 text_pos =
      bb.Min + padding + ImVec2(collapser_width, text_base_offset_y);
  if (display_frame) {
    // Framed type
    RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
...
void ImGui::Bullet() {
  ...
  window->DrawList->AddCircleFilled(
      ...
      bullet_size, GetColorU32(ImGuiCol_Text));
...
bool ImGui::VSliderInt(const char* label, const ImVec2& size, int* v, int v_min,
  ...
  if (!display_format)
    display_format    = "%.0f";
  float v_f           = (float) *v;
  bool  value_changed = ImGui::VSliderFloat(label, size, &v_f, (float) v_min,
                                           (float) v_max, display_format, 1.0f);
  *v = (int) v_f;
  return value_changed;
}
...
bool ImGui::DragFloatN(const char* label, float* v, int components,
  ...
  for (int i = 0; i < components; i++) {
    ImGui::PushID(i);
    value_changed |= ImGui::DragFloat("##v", &v[i], v_speed, v_min, v_max,
...
bool ImGui::DragInt(const char* label, int* v, float v_speed, int v_min,
  ...
  if (!display_format)
    display_format    = "%.0f";
  float v_f           = (float) *v;
  bool  value_changed = ImGui::DragFloat(label, &v_f, v_speed, (float) v_min,
...
bool ImGui::DragIntN(const char* label, int* v, int components, float v_speed,
  ...
  for (int i = 0; i < components; i++) {
    ImGui::PushID(i);
    value_changed |=
        ImGui::DragInt("##v", &v[i], v_speed, v_min, v_max, display_format);
    ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
    ImGui::PopID();
    ImGui::PopItemWidth();
  }
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size,
  ...
  ImVec2       size       = CalcItemSize(
      ...
      (is_multiline ? ImGui::GetTextLineHeight() * 8.0f : label_size.y) +
          style.FramePadding.y *
  ...
  if (focus_requested || user_clicked || user_scrolled) {
    if (g.ActiveId != id) {
      ...
      edit_state.InitialText.resize(buf_size + 1); // utf-8. we use +1 to make
                                                   ...
                                                   // so it doesn't crash.
      ImFormatString(edit_state.InitialText.Data, edit_state.InitialText.Size,
                     "%s", buf);
  ...
  if (g.ActiveId == id || (edit_state.Id == id && is_multiline &&
    ...
    draw_window->DrawList->AddText(
        ...
        GetColorU32(ImGuiCol_Text), buf, buf + edit_state.CurLenA, 0.0f,
    ...
    ImRect cursor_screen_rect(cursor_screen_pos.x,
                              ...
                              cursor_screen_pos.x, cursor_screen_pos.y - 1.5f);
    if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
      draw_window->DrawList->AddLine(cursor_screen_rect.Min,
                                     cursor_screen_rect.Max,
    ...
    if (is_multiline)
      text_size =
          ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf, &buf_end) *
                             g.FontSize); // We don't need width
    draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos,
                                   GetColorU32(ImGuiCol_Text), buf, buf_end,
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size,
  ...
  bool ret = InputTextEx(label, buf, (int) buf_size, ImVec2(0, 0), flags,
                         callback, user_data);
  return ret;
}
...
bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size,
                               ...
                               void*                 user_data) {
  bool ret =
      InputTextEx(label, buf, (int) buf_size, size,
...
bool ImGui::InputFloatN(const char* label, float* v, int components,
  ...
  for (int i = 0; i < components; i++) {
    ImGui::PushID(i);
    value_changed |=
        ImGui::InputFloat("##v", &v[i], 0, 0, decimal_precision, extra_flags);
    ImGui::SameLine(0, g.Style.ItemInnerSpacing.x);
    ImGui::PopID();
    ImGui::PopItemWidth();
  }
...
bool ImGui::Combo(const char* label, int* current_item,
  ...
  RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max,
              GetColorU32(popup_opened || hovered ? ImGuiCol_ButtonHovered
                                                  : ImGuiCol_Button),
...
bool ImGui::BeginMainMenuBar() {
  ...
  ImGui::SetNextWindowSize(ImVec2(
      g.IO.DisplaySize.x, g.FontBaseSize + g.Style.FramePadding.y * 2.0f));
  ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
...
bool ImGui::BeginMenuBar() {
  ...
  PushClipRect(
      ImVec2(rect.Min.x + 0.5f, rect.Min.y - 0.5f + window->BorderSize),
      ImVec2(rect.Max.x + 0.5f, rect.Max.y - 0.5f), false);
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
  ...
  bool menuset_opened =
      !(window->Flags & ImGuiWindowFlags_Popup) &&
      (g.OpenedPopupStack.Size > g.CurrentPopupStack.Size &&
       g.OpenedPopupStack[g.CurrentPopupStack.Size].ParentMenuSet ==
           window->GetID("##menus"));
  if (menuset_opened)
    g.FocusedWindow = window;
  ...
  if (window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu)) {
    ...
    if (g.HoveredWindow == window &&
      ...
      if (ImGuiWindow* next_window =
              g.OpenedPopupStack[g.CurrentPopupStack.Size].Window) {
    ...
    want_close =
        (opened && !hovered && g.HoveredWindow == window &&
  ...
  } else if (opened && pressed &&
             menuset_opened) // menu-bar: click open menu to close
  {
    want_close = true;
    want_open = opened = false;
  } else if (pressed || (hovered && menuset_opened && !opened)) // menu-bar:
                                                                // first click
                                                                // to open, then
                                                                // hover to open
                                                                // others
  ...
  if (opened) {
    ...
    ImGuiWindowFlags flags =
        ...
        ((window->Flags & (ImGuiWindowFlags_Popup | ImGuiWindowFlags_ChildMenu))
             ...
             : ImGuiWindowFlags_ChildMenu);
    opened = BeginPopupEx(label, flags); // opened can be 'false' when the popup
                                         // is completely clipped (e.g. zero
                                         // size display)
  }
...
void ImGui::EndMenu() { ImGui::EndPopup(); }
...
bool ImGui::ColorButton(const ImVec4& col, bool small_height,
  ...
  if (hovered)
    ImGui::SetTooltip("Color:\n(%.2f,%.2f,%.2f,%.2f)\n#%02X%02X%02X%02X", col.x,
                      col.y, col.z, col.w, IM_F32_TO_INT8(col.x),
                      IM_F32_TO_INT8(col.y), IM_F32_TO_INT8(col.z),
                      IM_F32_TO_INT8(col.z));
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool alpha) {
  ...
  if (ImGui::IsItemHovered())
    ImGui::SetTooltip("Color:\n(%.2f,%.2f,%.2f,%.2f)\n#%02X%02X%02X%02X",
                      col[0], col[1], col[2], col[3], IM_F32_TO_INT8(col[0]),
                      IM_F32_TO_INT8(col[1]), IM_F32_TO_INT8(col[2]),
...
void ImGui::Columns(int columns_count, const char* id, bool border) {
  ...
  const float content_region_width = window->SizeContentsExplicit.x
                                         ? window->SizeContentsExplicit.x
                                         : window->Size.x;
...
void ImGui::ShowMetricsWindow(bool* opened) {
  if (ImGui::Begin("ImGui Metrics", opened)) {
    ...
    struct Funcs {
      static void NodeDrawList(ImDrawList* draw_list, const char* label) {
        ...
        for (const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin();
          ...
          bool draw_opened = ImGui::TreeNode(
          ...
          if (!draw_opened)
            continue;
          for (int i = elem_offset; i + 2 < elem_offset + (int) pcmd->ElemCount;
               i += 3) {
            ImVec2 triangles_pos[3];
            char   buf[300], *buf_p = buf;
            for (int n = 0; n < 3; n++) {
              ImDrawVert& v =
                  draw_list->VtxBuffer[(draw_list->IdxBuffer.Size > 0)
                                           ? draw_list->IdxBuffer.Data[i + n]
                                           : i + n];
              triangles_pos[n] = v.pos;
              buf_p += sprintf(buf_p, "vtx %04d { pos = (%8.2f,%8.2f), uv = "
                                      "(%.6f,%.6f), col = %08X }\n",
            ...
            if (ImGui::IsItemHovered())
              overlay_draw_list->AddPolyline(triangles_pos, 3,
                                             ImColor(255, 255, 0), true, 1.0f,
                                             false); // Add triangle without AA,
                                                     // more readable for
                                                     // large-thin triangle
          }
          ImGui::TreePop();
    ...
    if (ImGui::TreeNode("Popups", "Opened Popups Stack (%d)",
                        g.OpenedPopupStack.Size)) {
