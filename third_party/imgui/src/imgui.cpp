...
// is done separately and is up to you. Tips: if you need to change your scale
// multiple times, prefer calling this on a freshly initialized ImGuiStyle
// structure rather than scaling multiple times (because floating point
// multiplications are lossy).
  ...
  if (g.Style.Alpha > 0.0f) {
    ...
    for (int i = 1; i < IM_ARRAYSIZE(g.RenderDrawLists); i++) {
      ...
      if (layer.empty())
        continue;
      memcpy(
        &g.RenderDrawLists[0][n], &layer[0], layer.Size * sizeof(ImDrawList*));
      n += layer.Size;
    }
...
void ImGui::OpenPopupEx(ImGuiID id, bool reopen_existing) {
  ...
  ImGuiPopupRef popup_ref          = ImGuiPopupRef(
    id, window, window->GetID("##menus"), g.IO.MousePos); // Tagged as new ref
                                                          // because constructor
                                                          // sets Window to NULL
                                                          // (we are passing the
                                                          // ParentWindow info
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window) {
  ...
  if ((flags & ImGuiWindowFlags_Tooltip) != 0) {
    ...
    size_auto_fit =
      ImClamp(window->SizeContents + window->WindowPadding,
              style.WindowMinSize,
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
  ...
  if (g.SetNextWindowCollapsedCond) {
    ...
    SetWindowCollapsed(
      window, g.SetNextWindowCollapsedVal, g.SetNextWindowCollapsedCond);
  ...
  if (first_begin_of_the_frame) {
    ...
    if (window->Collapsed) {
      ...
      // Title bar
      const bool is_focused = g.NavWindow && window->RootNonPopupWindow ==
                                               g.NavWindow->RootNonPopupWindow;
      if (!(flags & ImGuiWindowFlags_NoTitleBar))
        window->DrawList->AddRectFilled(
          title_bar_rect.GetTL(),
...
bool ImGui::Begin(const char*      name,
  ...
  // parameter. This is actually tricky to reproduce manually because: (1) there
  // are multiple variants of WindowBg (popup, tooltip, etc.) and (2) you can't
  // call PushStyleColor before Begin and PopStyleColor just after Begin()
  // because of how CheckStackSizes() behave. The user-side solution is to do
  // backup = GetStyleColorVec4(ImGuiCol_xxxBG), PushStyleColor(ImGuiCol_xxxBg),
  // Begin, PushStyleColor(ImGuiCol_xxxBg, backup), [...], PopStyleColor(),
  // End(); PopStyleColor() - which is super awkward. The alpha override was
  // rarely used but for now we'll leave the Begin() variant around for a bit.
  // We may either lift the constraint on CheckStackSizes() either add a
...
static inline void DataTypeFormatString(ImGuiDataType data_type,
  ...
  if (data_type == ImGuiDataType_Int) {
    ...
    if (decimal_precision < 0)
      ImFormatString(
        buf, buf_size, "%f", *(float*) data_ptr); // Ideally we'd have a minimum
                                                  // decimal precision of 1 to
                                                  // visually denote that it is
                                                  // a float, while hiding
...
bool ImGui::BeginCombo(const char* label,
  ...
  float popup_y2 = ImClamp(popup_y1 + popup_size.y,
                           popup_y1,
  ...
  if ((popup_y2 - popup_y1) <
    ...
    SetNextWindowPos(ImVec2(frame_bb.Min.x, frame_bb.Min.y),
                     ImGuiCond_Always,
                     ImVec2(0.0f, 1.0f));
  } else {
    // Position our combo below
    SetNextWindowPos(ImVec2(frame_bb.Min.x, frame_bb.Max.y),
                     ImGuiCond_Always,
                     ImVec2(0.0f, 0.0f));
  }
...
bool ImGui::Selectable(const char*          label,
  ...
  RenderTextClipped(
    bb.Min, bb_with_spacing.Max, label, NULL, &label_size, ImVec2(0.0f, 0.0f));
  if (flags & ImGuiSelectableFlags_Disabled)
    PopStyleColor();
                                ...
                                // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
  {
    ...
    if (g.HoveredWindow == window &&
      ...
      if (ImGuiWindow* next_window =
        ...
        float extra = ImClamp(
          fabsf(ta.x - tb.x) * 0.30f, 5.0f, 30.0f); // add a bit of extra slack.
...
bool ImGui::ColorButton(const char*         desc_id,
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f) {
    ...
    if (col_source.w < 1.0f)
      ...
      window->DrawList->AddRectFilled(
        bb_inner.Min, bb_inner.Max, GetColorU32(col_source), rounding, ~0);
  }
  if (window->Flags & ImGuiWindowFlags_ShowBorders)
...
bool ImGui::ColorPicker4(const char*         label,
  ...
  if (flags & ImGuiColorEditFlags_PickerHueWheel) {
    ...
    draw_list->AddCircle(hue_cursor_pos,
                         ...
                         hue_cursor_segments);
    draw_list->AddCircle(
      hue_cursor_pos, hue_cursor_rad, IM_COL32_WHITE, hue_cursor_segments);
...
float ImGui::GetColumnWidth(int column_index) {
  ...
  return OffsetNormToPixels(
    window,
    window->DC.ColumnsData[column_index + 1].OffsetNorm -
...
void ImGui::PushColumnClipRect(int column_index) {
  ...
  PushClipRect(window->DC.ColumnsData[column_index].ClipRect.Min,
               window->DC.ColumnsData[column_index].ClipRect.Max,
               false);
}
            ...
            for (int prim  = clipper.DisplayStart,
              ...
              if (ImGui::IsItemHovered())
                overlay_draw_list->AddPolyline(triangles_pos,
                                               ...
                                               true,
                                               1.0f,
                                               false); // Add triangle without
                                                       // AA, more readable for
                                                       // large-thin triangle
            }
          ImGui::TreePop();
        }
      ...
      static void NodeWindow(ImGuiWindow* window, const char* label) {
        ...
        if (ImGui::IsItemHovered())
          GImGui->OverlayDrawList.AddRect(window->Pos,
                                          ...
                                          IM_COL32(255, 255, 0, 255));
        ImGui::BulletText(
          "Scroll: (%.2f,%.2f)", window->Scroll.x, window->Scroll.y);
        ImGui::BulletText(
          "Active: %d, Accessed: %d", window->Active, window->Accessed);
    ...
    if (ImGui::TreeNode("Basic state")) {
      ...
      ImGui::Text("HoveredId: 0x%08X/0x%08X",
                  ...
                  g.HoveredIdPreviousFrame); // Data is "in-flight" so depending
                                             ...
                                             // information or not
      ImGui::Text(
        "ActiveId: 0x%08X/0x%08X", g.ActiveId, g.ActiveIdPreviousFrame);
