...
#pragma clang diagnostic ignored                                               \
  "-Wdeprecated-declarations" // warning : 'xx' is deprecated: The POSIX name
                              // for this item.. // for strdup used in demo code
...
void ImGui::ShowTestWindow(bool* p_open) {
  ...
  if (show_app_about) {
    ImGui::Begin(
      "About ImGui", &show_app_about, ImGuiWindowFlags_AlwaysAutoResize);
  ...
  if (ImGui::CollapsingHeader("Widgets")) {
    ...
    if (ImGui::TreeNode("Color/Picker Widgets")) {
      ...
      ImGui::ColorEdit4(
        "MyColor##2", (float*) &color, ImGuiColorEditFlags_HSV | misc_flags);
      ...
      ImGui::SameLine();
      ShowHelpMarker("With the ImGuiColorEditFlags_NoInputs flag you can hide "
                     "all the slider/text inputs.\nWith the "
                     "ImGuiColorEditFlags_NoLabel flag you can pass a "
                     "non-empty label which will only be used for the tooltip "
                     "and picker popup.");
      ...
      ImGui::SameLine();
      ShowHelpMarker("SetColorEditOptions() is designed to allow you to set "
                     "boot-time default.\nWe don't have Push/Pop functions "
                     "because you can force options on a per-widget basis if "
                     "needed, and the user can change non-forced ones with the "
  ...
  if (ImGui::CollapsingHeader("Inputs & Focus")) {
    ...
    ShowHelpMarker("Request ImGui to render a mouse cursor for you in "
                   "software. Note that a mouse cursor rendered via regular "
                   "GPU rendering will feel more laggy than hardware cursor, "
                   "but will be more in sync with your other visuals.");
    ...
    if (ImGui::TreeNode("Mouse cursors")) {
      ...
      ImGui::SameLine();
      ShowHelpMarker("Your application can render a different mouse cursor "
                     "based on what ImGui::GetMouseCursor() returns. If "
                     "software cursor rendering (io.MouseDrawCursor) is set "
                     "ImGui will draw the right cursor for you, otherwise your "
                     "backend needs to handle it.");
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref) {
  ...
  if (ImGui::TreeNode("Settings")) {
    ...
    ImGui::SliderFloat(
      "WindowRounding", &style.WindowRounding, 0.0f, 16.0f, "%.0f");
    ImGui::SliderFloat(
      "ChildWindowRounding", &style.ChildWindowRounding, 0.0f, 16.0f, "%.0f");
    ImGui::SliderFloat2(
      "FramePadding", (float*) &style.FramePadding, 0.0f, 20.0f, "%.0f");
    ImGui::SliderFloat(
      "FrameRounding", &style.FrameRounding, 0.0f, 16.0f, "%.0f");
    ImGui::SliderFloat2(
    ...
    ImGui::SliderFloat(
      "ScrollbarRounding", &style.ScrollbarRounding, 0.0f, 16.0f, "%.0f");
    ImGui::SliderFloat("GrabMinSize", &style.GrabMinSize, 1.0f, 20.0f, "%.0f");
    ImGui::SliderFloat(
      "GrabRounding", &style.GrabRounding, 0.0f, 16.0f, "%.0f");
  ...
  ShowHelpMarker("Tip: Load fonts with io.Fonts->AddFontFromFileTTF()\nbefore "
                 "calling io.Fonts->GetTex* functions.");
  if (fonts_opened) {
    ...
    for (int i = 0; i < atlas->Fonts.Size; i++) {
      ...
      if (font_details_opened) {
        ...
        ShowHelpMarker(
          ...
          "some flexibility with scaling. You can also render at multiple "
          "sizes and select which one to use at runtime.\n\n(Glimmer of hope: "
...
static void ShowExampleAppCustomRendering(bool* p_open) {
  ...
  ImGui::ColorEdit3("Color", &col.x);
  {
    ...
    for (int n = 0; n < 2; n++) {
      ...
      draw_list->AddCircle(
        ImVec2(x + sz * 0.5f, y + sz * 0.5f), sz * 0.5f, col32, 20, thickness);
      x += sz + spacing;
      draw_list->AddRect(
        ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 0.0f, ~0, thickness);
      x += sz + spacing;
      draw_list->AddRect(
        ImVec2(x, y), ImVec2(x + sz, y + sz), col32, 10.0f, ~0, thickness);
      x += sz + spacing;
