...
public:
	...
	*/
	void setDustDensityRatio(long double ratio = 1) { mDustDensityCoeff *= ratio; }
	void generatePlanets(StargenHelper& helper);

	/// This function returns the coefficient of density of the dust in the cloud.
	long double getDustDensityCoeff() const noexcept { return mDustDensityCoeff; }
...
private:
	...
	bool isDustAvailable(long double inside_range, long double outside_range) const noexcept;
	long double getInnerEffectLimit(const protoPlanet* protoplanet) const noexcept;
	long double getOuterEffectLimit(const protoPlanet* protoplanet) const noexcept;
	long double getCriticalMass(const protoPlanet* protoplanet) const noexcept;
