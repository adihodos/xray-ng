...
struct Reader {
	...
	rfl::Result<InputVarType> get_field_from_array(const size_t _idx, const InputArrayType _arr) const noexcept {
		...
		return rfl::Unexpected{make_config_error("Field @ {} not found", _idx)};
	}
	...
	rfl::Result<InputVarType> get_field_from_object(
		...
		if (s)
			...
			return rfl::Unexpected{make_config_error("Field @ {} not found", _name)};
	}
	...
	rfl::Result<T> to_basic_type(const InputVarType& _var) const noexcept {
		...
		if constexpr (std::is_same_v<T, std::string>) {
			...
			return s ? rfl::Result<T>{std::string{s}} : rfl::Unexpected{rfl::Error{"variable is not a string"}};
	...
	rfl::Result<InputArrayType> to_array(const InputVarType& _var) const noexcept {
		if (config_setting_is_list(_var))
			...
			return rfl::Unexpected{rfl::Error{"Variable is not an array"}};
	}
	...
	rfl::Result<InputObjectType> to_object(const InputVarType& _var) const noexcept {
		if (config_setting_is_group(_var)) return _var;
		return rfl::Unexpected{rfl::Error{"Variable is not an object"}};
	}
	...
	std::optional<Error> read_array(const ArrayReader& _array_reader, const InputArrayType& _arr) const noexcept {
		...
		for (int32_t i = 0; i < count; ++i) {
			...
			if (!e) return make_config_error("Failed to read array element {}", i);
	...
	std::optional<Error> read_object(const ObjectReader& _object_reader, const InputObjectType& _obj) const noexcept {
		if (!config_setting_is_group(_obj)) return make_config_error("Not an object type");

		for (int32_t i = 0, count = config_setting_length(_obj); i < count; ++i) {
			config_setting_t* s = config_setting_get_elem(_obj, i);
			if (!s) return make_config_error("Failed to read member @ {} of object.", i);
