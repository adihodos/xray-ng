...
template<class OutputClass, class InputClass>
inline OutputClass
unsafe_horrible_cast(const InputClass input)
{
    horrible_union<OutputClass, InputClass> u;
    u.in = input;
    return u.out;
...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE + 2 * sizeof(int)>
{
    ...
    inline static GenericClass* Convert(X* pthis, XFuncType function_to_bind, GenericMemFuncType& bound_func)
    {
        ...
        static_cast(sizeof(function_to_bind) == sizeof(u.s) && sizeof(function_to_bind) == sizeof(u.ProbeFunc) &&
...
class FastDelegateImpl
{
  ...
  public:
    ...
    {
        m_Closure.bindstaticfunc(this, &FastDelegateImpl::InvokeStaticFunction, function_to_bind);
    }
    // Invoke the delegate
...
template<typename RetType, typename... Args>
class fast_delegate<RetType(Args...)>
    ...
    : public FastDelegateImpl<RetType, Args...>
{
  public:
    ...
    template<typename X, typename Y>
    fast_delegate(Y* pthis, RetType (X::*function_to_bind)(Args... args))
        : BaseType(pthis, function_to_bind)
    {
    }

    template<typename X, typename Y>
    fast_delegate(const Y* pthis, RetType (X::*function_to_bind)(Args... args) const)
        : BaseType(pthis, function_to_bind)
    {
    }

    fast_delegate(RetType (*function_to_bind)(Args... args))
...
#endif // fast_delegate_ALLOW_FUNCTION_TYPE_SYNTAX
...
//
//			MakeDelegate(&x, &X::func) returns a fast_delegate of the type
...
template<typename RetType, typename... Args>
fast_delegate<RetType(Args...)>
make_delegate(RetType (*func)(Args...))
{
    return fast_delegate<RetType(Args...)>(func);
}

template<typename RetType, typename X, typename Y, typename... Args>
fast_delegate<RetType(Args...)>
make_delegate(Y& x, RetType (X::*func)(Args...))
{
    return fast_delegate<RetType(Args...)>(&x, func);
}

template<typename RetType, typename X, typename Y, typename... Args>
fast_delegate<RetType(Args...)>
make_delegate(Y& x, RetType (X::*func)(Args...) const)
{
    return fast_delegate<RetType(Args...)>(&x, func);
}

// template <typename RetType, typename X, typename Y, typename... Args>
// fast_delegate<RetType (Args...)> make_delegate(const Y& x, RetType (X::*func)(Args...) const) {
//	return fast_delegate<RetType (Args...)>(&x, func);
// }
