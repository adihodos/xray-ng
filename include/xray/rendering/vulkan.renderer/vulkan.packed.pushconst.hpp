...
struct PackedU32PushConstant {
	...
	PackedU32PushConstant(
		const xray::rendering::BindlessResourceHandle_StorageBuffer sbo,
	...
	PackedU32PushConstant(
		const xray::rendering::BindlessResourceHandle_Image image, const uint32_t instance_entry, const uint32_t frame
	) noexcept {
		this->instance_buffer_id = destructure_bindless_resource_handle(image).first;
		this->instance_entry	 = instance_entry;
		this->frame_id			 = frame;
	}

	PackedU32PushConstant(
		const xray::rendering::BindlessResourceHandle_StorageImage image,
		const uint32_t instance_entry,
		const uint32_t frame
	) noexcept {
		this->instance_buffer_id = destructure_bindless_resource_handle(image).first;
	...
	PackedU32PushConstant(const uint32_t frame) noexcept {
